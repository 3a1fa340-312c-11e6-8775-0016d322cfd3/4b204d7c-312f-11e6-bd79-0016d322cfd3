...
#endif
#endif	//ZOT
#define IRQ_RETURN void	//ZOT
#define IRQ_HANDLED //ZOT
//========================================================
#ifdef CONFIG_STAR_NIC_PHY_INTERNAL_PHY
#ifdef O_NETCOR
#define FE_PHY_LED_MODE (0x1 << 12)
#else
#define FE_PHY_LED_MODE (0x2 << 12)
#endif //O_NETCOR
#define CONFIG_INTERNEL_PHY_PATCH
#endif //CONFIG_STAR_NIC_PHY_INTERNAL_PHY

#ifdef CONFIG_INTERNEL_PHY_PATCH
#define INTERNAL_PHY_PATCH_CHECKCNT	16
#define INTERNAL_PHY_PATCH_CHECK_PERIOD	1000 //ms

//ZOT static struct timer_list internal_phy_timer;

cyg_handle_t PHY_SysClk;//ZOT
cyg_handle_t PHY_Counter;//ZOT
cyg_handle_t PHY_Alarm; //ZOT 
cyg_alarm PHY_timerAlarm;//ZOT

extern BYTE MyPhysNodeAddress[];	//ZOT

static void internal_phy_patch_check(int);
//ZOT static void internal_phy_update(unsigned long data);
void internal_phy_update(cyg_handle_t handle, cyg_addrword_t ptr);	//ZOT

//ZOT_phy_update Task create information definition
#define ZOT_phy_update_TASK_PRI         20	//ZOT716u2
#define ZOT_phy_update_TASK_STACK_SIZE	 1024 //ZOT716u2 3072
static uint8 			ZOT_phy_update_Stack[ZOT_phy_update_TASK_STACK_SIZE];
static cyg_thread       ZOT_phy_update_Task;
static cyg_handle_t     ZOT_phy_update_TaskHdl;

extern void ZOT_phy_update(cyg_addrword_t data);
...
//ZOT static struct proc_dir_entry *star_nic_proc_entry;
...
struct star_nic_private *STAR_PRIV;	//ZOT

extern void LanRecv(unsigned char *data, unsigned int len);	//ZOT

#ifdef CONFIG_STAR_NIC_NAPI
static void star_nic_receive_packet(int mode, int *work_done, int work_to_do);
#else
void star_nic_receive_packet(int mode, int work_to_do);
#endif
...
#else	//ZOT
static void star_nic_phy_powerdown(void);
static void star_nic_phy_powerup(void);
#endif	//ZOT
...
//ZOT static int star_nic_dma_config(struct net_device *dev)
static int star_nic_dma_config(void)	//ZOT
{
	u32 dma_config = 0;
	...
	// 4N+2(for Linux)
//ZOT	dma_config &= ~(0x1 << 16);
	// 4N
	dma_config |= (0x1 << 16);	//ZOT
...
//ZOT static int star_nic_mac_config(struct net_device *dev)
static int star_nic_mac_config(void)	//ZOT
{
	u32 mac_config;
...
//ZOT static int star_nic_fc_config(struct net_device *dev)
static int star_nic_fc_config(void)	//ZOT
{
	u32 fc_config;
...
//ZOT static int star_nic_phy_config(struct net_device *dev)
static int star_nic_phy_config(void)	//ZOT
{
//ZOT	struct star_nic_private *priv = netdev_priv(dev);
	struct star_nic_private *priv = STAR_PRIV;	//ZOT
...
//=========================================================
	...
	NIC_PHY_CONTROL_REG1 = phy_config;
#if 1
//ZOT	star_nic_phy_powerdown(dev);
	star_nic_phy_powerdown();	//ZOT
#endif

	return 0;
}

//ZOT static int star_nic_vlan_config(struct net_device *dev)
static int star_nic_vlan_config(void)	//ZOT
{
	u32 vlan_id;
...
//ZOT static int star_nic_arl_config(struct net_device *dev)
static int star_nic_arl_config(void)	//ZOT
{
	u32 arl_config;
...
static void star_nic_interrupt_enable(void)
{
	...
	HAL_INTC_ENABLE_INTERRUPT_SOURCE(INTC_NIC_TXTC_BIT_INDEX);
...
//ZOT static void star_nic_phy_powerdown(struct net_device *dev)
static void star_nic_phy_powerdown(void)	//ZOT
{
//ZOT	struct star_nic_private *priv = netdev_priv(dev);	
	struct star_nic_private *priv = STAR_PRIV;	//ZOT
	u16 phy_data = 0;
...
//ZOT static void star_nic_phy_powerup(struct net_device *dev)
static void star_nic_phy_powerup(void)	//ZOT
{
//ZOT	struct star_nic_private *priv = netdev_priv(dev);
	struct star_nic_private *priv = STAR_PRIV;	//ZOT
	u16 phy_data = 0;
...
//ZOT static void star_nic_enable(struct net_device *dev)
static void star_nic_enable(void) //ZOT
{
#if 0
	// enable NIC clock
...
#if 1
//ZOT	star_nic_phy_powerup(dev);
	star_nic_phy_powerup();	//ZOT
#endif
#ifdef CONFIG_INTERNEL_PHY_PATCH
	printk("%s: starting patch check.\n", __FUNCTION__);
	internal_phy_patch_check(1);
//ZOT	mod_timer(&internal_phy_timer, jiffies + INTERNAL_PHY_PATCH_CHECK_PERIOD / 10);
    //ZOT
//Jesse    PHY_SysClk = cyg_real_time_clock();

//Jesse    cyg_clock_to_counter(PHY_SysClk, &PHY_Counter);
//Jesse    cyg_alarm_create(PHY_Counter, (cyg_alarm_t *)internal_phy_update,
//Jesse                     (cyg_addrword_t) 0,
//Jesse                     &PHY_Alarm, &PHY_timerAlarm);

    /* This creates a periodic timer */
//Jesse    cyg_alarm_initialize(PHY_Alarm, cyg_current_time() + INTERNAL_PHY_PATCH_CHECK_PERIOD / 10, 0); //every setting trigger once
    
    //Create ZOT_phy_update Thread 
    cyg_thread_create(ZOT_phy_update_TASK_PRI,
                  ZOT_phy_update,
                  0,
                  "ZOT_phy_update",
                  (void *) (ZOT_phy_update_Stack),
                  ZOT_phy_update_TASK_STACK_SIZE,
                  &ZOT_phy_update_TaskHdl,
                  &ZOT_phy_update_Task);
	
	//Start ZOT_phy_update Thread
	cyg_thread_resume(ZOT_phy_update_TaskHdl);
...
//ZOT
cyg_interrupt MAC_RXRC_interrupt;
cyg_handle_t  MAC_RXRC_interrupt_handle;
cyg_interrupt MAC_RXQF_interrupt;
cyg_handle_t  MAC_RXQF_interrupt_handle;
cyg_interrupt MAC_STATE_interrupt;
cyg_handle_t  MAC_STATE_interrupt_handle;
//ZOT
cyg_uint32 rxrc_isr(cyg_vector_t vector, cyg_addrword_t data)
{
	cyg_interrupt_mask(vector);
	cyg_interrupt_acknowledge(vector);
	return(CYG_ISR_HANDLED | CYG_ISR_CALL_DSR);	
}
cyg_uint32 rxqf_isr(cyg_vector_t vector, cyg_addrword_t data)
{
	cyg_interrupt_mask(vector);
	cyg_interrupt_acknowledge(vector);
	return(CYG_ISR_HANDLED | CYG_ISR_CALL_DSR);	
...
cyg_uint32 status_isr(cyg_vector_t vector, cyg_addrword_t data)
{
	cyg_interrupt_mask(vector);
	cyg_interrupt_acknowledge(vector);
	return(CYG_ISR_HANDLED | CYG_ISR_CALL_DSR);	
...
IRQ_RETURN star_nic_receive_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	star_nic_receive_packet(0, 32); // Receive Once
...
IRQ_RETURN star_nic_rxqf_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	star_nic_receive_packet(1, 32); // Receive at Queue Full Mode
...
//ZOT static int star_nic_install_isr(struct net_device *dev)
static int star_nic_install_isr(void)	//ZOT
{
	int retval;
...
#else	//ZOT
    cyg_interrupt_create(INTC_NIC_RXRC_BIT_INDEX,
                             0,                     
                             0,                     
                             &rxrc_isr,
                             &star_nic_receive_isr,
                             &MAC_RXRC_interrupt_handle,
                             &MAC_RXRC_interrupt);
    cyg_interrupt_attach(MAC_RXRC_interrupt_handle); 
#endif	//ZOT
...
#else	//ZOT
    cyg_interrupt_create(INTC_NIC_RXQF_BIT_INDEX,
                             0,                     
                             0,                     
                             &rxqf_isr,
                             &star_nic_rxqf_isr,
                             &MAC_RXQF_interrupt_handle,
                             &MAC_RXQF_interrupt);
    cyg_interrupt_attach(MAC_RXQF_interrupt_handle); 
#endif	//ZOT		
#endif	
...
#else
cyg_interrupt_create(INTC_NIC_STATUS_BIT_INDEX,
                             0,                     
                             0,                     
                             &status_isr,
                             &star_nic_status_isr,
                             &MAC_STATE_interrupt_handle,
                             &MAC_STATE_interrupt);
    cyg_interrupt_attach(MAC_STATE_interrupt_handle); 
...
//ZOT static int star_nic_lan_open(struct net_device *dev)
static int star_nic_lan_open(void)	//ZOT
{
...
//ZOT	CUR_NAPI_DEV = dev;

//ZOT	netif_start_queue(dev);
...
//ZOT static inline struct sk_buff *star_nic_alloc_skb(void)
static struct sk_buff *star_nic_alloc_skb(int dir, int id)	//ZOT
{
	struct sk_buff *skb;
#if 0
...
//Z	memset(skb, 0, sizeof(struct sk_buff));
	
//	if (dir == 0)	//Tx
//		skb->data = MAC_TX_BUF_POOL + (id * 0x800);
//	else			//Rx
//		skb->data = MAC_RX_BUF_POOL + (id * 0x800);
	
	skb->data = kaligned_alloc(MAX_PACKET_LEN, 256);
	if (!skb->data)
		return NULL;
	
	memset(skb->data, 0, MAX_PACKET_LEN);
...
//ZOT static void __init star_nic_buffer_free(void)
static void star_nic_buffer_free(void)	//ZOT
{
	int i;
...
//ZOT static int __init star_nic_buffer_alloc(void)
static int star_nic_buffer_alloc(void)	//ZOT
{
...
#else	//ZOT
//	rxring.vir_addr	= MAC_RX_DEC_POOL;
//	rxring.phy_addr = MAC_RX_DEC_POOL;
//	txring.vir_addr = MAC_TX_DEC_POOL;
//	txring.phy_addr = MAC_TX_DEC_POOL;

	rxring.phy_addr = rxring.vir_addr	= kaligned_alloc(STAR_NIC_MAX_RFD_NUM * sizeof(STAR_NIC_RXDESC), 256);
	txring.phy_addr = txring.vir_addr	= kaligned_alloc(STAR_NIC_MAX_TFD_NUM * sizeof(STAR_NIC_TXDESC), 256);
	...
	for (i = 0; i < STAR_NIC_MAX_RFD_NUM; i++, rxdesc_ptr++) {
		...
		skb_ptr = star_nic_alloc_skb(1, i);
		if (!skb_ptr) {
	...
	for (i = 0; i < STAR_NIC_MAX_TFD_NUM; i++, txdesc_ptr++) {
		...
		skb_ptr = star_nic_alloc_skb(0, i);
		if (!skb_ptr) {
			printk("ERROR: Allocate skb Failed!\n");
			err = -ENOMEM;
			goto err_out;
		...
		// Trans Packet from Virtual Memory to Physical Memory
		txring.skb_ptr[i]	= skb_ptr;
		txdesc_ptr->data_ptr	= (u32)virt_to_phys(skb_ptr->data);
		txdesc_ptr->length	= MAX_PACKET_LEN;
		...
		txdesc_ptr->tco		= 0;
#endif
//ZOT		txring.skb_ptr[i] 	= NULL;	// clear txring.skb_ptr
	}
...
static int star_nic_get_rfd_buff(int id)
{
	...
	//TODO: get rxdesc ptr
	rxdesc_ptr = rxring.vir_addr + id;
	skb_ptr = rxring.skb_ptr[id];
#ifdef CONFIG_STAR_JUMBO
...
static int star_nic_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	...
	spin_lock_irqsave(&star_nic_send_lock, &flags);	//ZOT==> spin_lock_irqsave(&star_nic_send_lock, flags);
	...
	for (f = 0; f < (nr_frags + 1); f++) {
		...
		if (txdesc_ptr->cown == 0) {
			spin_unlock_irqrestore(&star_nic_send_lock, &flags);	//ZOT==> spin_unlock_irqrestore(&star_nic_send_lock, flags);
#ifdef FREE_TX_SKB_MULTI
			for (i = 0; i < skb_free_count; i++) {
...
int send_frame(unsigned char *data,int length, int flag)	//ZOT
{
	struct star_nic_private *priv = STAR_PRIV;
//ZOT716u2	STAR_NIC_TXDESC volatile *txdesc_ptr = (txring.vir_addr + txring.cur_index);	
	STAR_NIC_TXDESC volatile *txdesc_ptr;	//ZOT716u2
	struct sk_buff *skb = NULL;
	struct sk_buff *skb_free = NULL;
	unsigned long flags;
	
	cyg_semaphore_wait(&send_frame_mutex);//ZOT716u2
	
	txdesc_ptr = (txring.vir_addr + txring.cur_index);//ZOT716u2

	if (txdesc_ptr->cown == 0) { // This TFD is busy
		// re-queue the skb
		cyg_semaphore_post(&send_frame_mutex);	//ZOT716u2
		return 1;
	}
	...
	skb = txring.skb_ptr[txring.cur_index];

	if(flag)
		cpyfrompbuf(skb->data, data, length);
	else
    	memcpy((void *)skb->data, data, length);
...
//ZOT static void star_nic_set_mac_addr(struct net_device *dev, const char *mac_addr)
static void star_nic_set_mac_addr(const char *mac_addr)	//ZOT
{
//ZOT	memcpy(dev->dev_addr, mac_addr, 6);
...
//ZOT static int star_nic_init(struct net_device *dev)
static int star_nic_init()	//ZOT
{
#if 1
	// set hight
	PWRMGT_SOFTWARE_RESET_CONTROL_REG |= (0x1 << 15);
...
//ZOT	star_nic_mac_config(dev);
	star_nic_mac_config();	//ZOT
//ZOT	star_nic_fc_config(dev);
	star_nic_fc_config();	//ZOT
	
//ZOT	if (star_nic_phy_config(dev) != 0) {
	if (star_nic_phy_config() != 0) {
		star_nic_buffer_free();
		return -1;
	}

//ZOT	star_nic_vlan_config(dev);
	star_nic_vlan_config();	//ZOT
//ZOT	star_nic_arl_config(dev);
	star_nic_arl_config();	//ZOT
//ZOT	star_nic_set_mac_addr(dev, default_mac_addr);
	star_nic_set_mac_addr(MyPhysNodeAddress);	//ZOT
//ZOT	star_nic_mib_reset();

//ZOT	*(u32 volatile *)(SYSVA_MISC_BASE_ADDR+0x0c) = 0x00000125;    //0x00000105 pb0_nic
	*(u32 volatile *)(SYSPA_MISC_BASE_ADDR+0x0c) = 0x00000125;    //0x00000105 pb0_nic	//ZOT
...
//ZOT	star_nic_dma_config(dev);
	star_nic_dma_config();	//ZOT

	return 0;
}

//ZOT static int __init star_nic_probe(int port_type)
static int star_nic_probe(int port_type)	//ZOT
{
//ZOT	struct net_device *netdev;
//ZOT	struct star_nic_private *priv;
	int err;
...
#endif	//ZOT
	STAR_PRIV = malloc(sizeof(struct star_nic_private));		//ZOT
//ZOT
	if (!STAR_PRIV) {
		err = -ENOMEM;
		goto err_alloc_etherdev;
	}	
//Z	memset(STAR_PRIV, 0, sizeof(struct star_nic_private));
//ZOT	spin_lock_init(&priv->lock);
#if 0	//ZOT
	...
	STAR_PRIV->dev_index = LAN_PORT;	//ZOT
	
//ZOT	if ((err = star_nic_init(netdev))) {
	if ((err = star_nic_init())) {
		goto err_nic_init;
	}

	star_nic_lan_open();	//ZOT

//Jesse
	HAL_PWRMGT_ENABLE_MII_PAD_SIGNAL_NOT_BOUNDED();	
	HAL_PWRMGT_SELECT_PAD_DRIVE_STRENGTH_MII_MODE();
...
//ZOT static int __init star_nic_lan_init(void)
int star_nic_lan_init(void)	//ZOT
{
...
void internal_phy_update(cyg_handle_t handle, cyg_addrword_t ptr)	//ZOT
{
	cyg_alarm_delete(PHY_Alarm);
	cyg_clock_delete(PHY_SysClk);
	cyg_counter_delete(PHY_Counter);

	internal_phy_patch_check(0);
//ZOT mod_timer(&internal_phy_timer, jiffies + INTERNAL_PHY_PATCH_CHECK_PERIOD / 10);
	//ZOT
    PHY_SysClk = cyg_real_time_clock();

    cyg_clock_to_counter(PHY_SysClk, &PHY_Counter);
    cyg_alarm_create(PHY_Counter, (cyg_alarm_t *)internal_phy_update,
                     (cyg_addrword_t) 0,
                     &PHY_Alarm, &PHY_timerAlarm);

    /* This creates a periodic timer */
    cyg_alarm_initialize(PHY_Alarm, cyg_current_time() + INTERNAL_PHY_PATCH_CHECK_PERIOD / 10, 0); //every setting trigger once
}

void ZOT_phy_update(cyg_addrword_t data)
{
	while(1)
	{
		internal_phy_patch_check(0);
...
//ZOT static struct timer_list internal_phy_timer =
//ZOT TIMER_INITIALIZER(internal_phy_update, 0, 0);
...
static void internal_phy_patch_check(int init)
{
	...
	if (((phy_data & 0x0004) != 0x0004) && ((phy_data2 & 0x0004) != 0x0004)) { // link down
		...
		LANLightToggle_down++;	//eason 20100809
...
//ZOT716u2
//ZOT==>		if(init)
//ZOT==>			short_cable_agc_detect_count = 1;
		...
		if (short_cable_agc_detect_count) { // short cable
			...
			if (phy_data & 0x0040) { // link up
				link_status = 1;
//Jesse				STAR_MAC_Plugout = 0;	//ZOT
			}
			//ZOT
			else{
//Jesse				STAR_MAC_Plugout = 1;	//ZOT
			}
