...
#else  /* _LINUX_SOURCE_ */
typedef unsigned long       RTMP_OS_PID;
#endif /* !_LINUX_SOURCE_ */
#endif
...
#else /* _LINUX_SOURCE_ */
typedef struct cyg_sem_t    OS_SEM;
#endif /* !_LINUX_SOURCE_ */
...
{                                               \
    cyg_mutex_t* mut_t = (cyg_mutex_t *)lock;   \
    cyg_mutex_release(mut_t);                   \
    cyg_mutex_destroy(mut_t);                   \
} 
...
#define OS_SEM_EVENT_INIT_LOCKED(_pSema) 	cyg_semaphore_init((_pSema), 0)
#define OS_SEM_EVENT_INIT(_pSema)			cyg_semaphore_init((_pSema), 1)
#define OS_SEM_EVENT_DESTORY(_pSema)		do{}while(0)
#define OS_SEM_EVENT_WAIT(_pSema, _status)	cyg_semaphore_wait(_pSema); _status = 0
#define OS_SEM_EVENT_UP(_pSema)				cyg_semaphore_post(_pSema)
...
#else /* _LINUX_SOURCE_ */
#define OS_WAIT(_time) RTMPusecDelay(_time * 1000)
#endif /* !_LINUX_SOURCE_ */
...
#define RTUSB_URB_FREE_BUFFER(_dev, _size, _addr, _dma)	kfree(_addr, 0) 
#endif
...
#define RTUSB_URB_ALLOC_BUFFER(pUsb_Dev, BufSize, pDma_addr)         		kaligned_alloc(BufSize, 4096)
#define RTUSB_URB_FREE_BUFFER(pUsb_Dev, BufSize, pTransferBuf, Dma_addr)  	kaligned_free(pTransferBuf)
#endif
