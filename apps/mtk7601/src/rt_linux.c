...
static inline NDIS_STATUS __RtmpOSTaskKill(OS_TASK *pTask)
{
    ...
    cgy_thread_kill(pTask->thread_handle);
	...
	IN ULONG arg)
{
    ...
    cyg_thread_create (pTask->sched_info,
                       (cyg_thread_entry_t *)fn, 
                       (cyg_addrword_t)pTask,
                       pTask->taskName,
                       (void *)pTask->stack_base,
                       pTask->stack_size,
                       &pTask->thread_handle,
                       &pTask->thread);
    cyg_thread_resume(pTask->thread_handle);
...
#ifndef _LINUX_SOURCE_
    cyg_semaphore_init(&pTask->taskSema, 0);
#endif /* _LINUX_SOURCE_ */
	return NDIS_STATUS_SUCCESS;
}
...
/*	*ppAd = (PVOID)vmalloc(sizeof(RTMP_ADAPTER)); //pci_alloc_consistent(pci_dev, sizeof(RTMP_ADAPTER), phy_addr); */
    ...
    *ppAd = (PVOID) aligned_alloc(SizeOfpAd, 256);
	if (*ppAd) {
	...
	IN VOID *pNetDevOrg)
{
	...
	if (pCfg80211_CB->pCfg80211_Wdev != NULL)
	{
		...
		*/
		...
		kfree(pCfg80211_CB->pCfg80211_Wdev, 0);

		if (pCfg80211_CB->pCfg80211_Channels != NULL)
			kfree(pCfg80211_CB->pCfg80211_Channels, 0);

		if (pCfg80211_CB->pCfg80211_Rates != NULL)
			kfree(pCfg80211_CB->pCfg80211_Rates, 0);
