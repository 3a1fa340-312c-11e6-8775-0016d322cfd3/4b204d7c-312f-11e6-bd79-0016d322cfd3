			...
			if ((size_t)(offset - cfgData) < fileLen)
			{
				...
				else if (strncmp(pTempStr, "NetworkType=", strlen("NetworkType=")) == 0)
				{
					...
					if (pAd->StaCfg.BssType == BSS_ADHOC) {
                        ...
                        EEPROM_Data.WLMode = 2;
                    }
					else {
						snprintf(pTempStr, 512, "NetworkType=Infra");
                        EEPROM_Data.WLMode = 1;
                    }
				}
#endif /* CONFIG_STA_SUPPORT */
		...
		if (pAd->StaCfg.WepStatus == Ndis802_11WEPEnabled)                           
		{
		    if (pCredentail->KeyLength)
			{
                if (pCredentail->KeyLength == 5) {
                    ...
                    switch (pAd->StaCfg.DefaultKeyId+1) {
                        case 1:
...
/*
  ...
  */
#define WSC_TASK_PRI        20
#define WSC_TASK_STACK_SIZE 4096
static uint8 Wsc_Stack[WSC_TASK_STACK_SIZE];
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd)
{
    ...
    #ifndef _LINUX_SOURCE_
    pTask->sched_info = (cyg_addrword_t)WSC_TASK_PRI;
    pTask->stack_base = (void *)Wsc_Stack;
    pTask->stack_size = WSC_TASK_STACK_SIZE;
    #endif /* _LINUX_SOURCE_ */
