...
#define WSET_LH(buf,pos,val) NSET16((((BYTE*)buf)+pos),val)
#define WVAL_LH(buf,pos) NGET16(((BYTE*)buf) + pos)

#ifdef CONST_DATA
extern const unsigned char far nwcryptdata[256];
extern const unsigned char far nwhashdata[256];
#else
...
static uint16 rol16(uint16 i, int16 c)
{
	return ((i << c) & 65535) | ((uint16)(i & 65535) >> (16 - c));
}
static uint16 ror16(uint16 i, int16 c)
{
	return ((uint16)(i & 65535) >> c) | ((i << (16 - c)) & 65535);
}

void nwencrypt(const uint16 *cryptbuf, const BYTE *in, BYTE *out)
{
	int16 i, j;
	register uint16 i1, i2, i3, i4;
	const uint16 *p;
...
void nwcryptinit(uint16 *scryptbuf, const BYTE *key)
{
	...
	for (i = 0; i < 120; i++) {
		cryptbuf[i + 8] =
		nwcryptdata[(BYTE)(cryptbuf[i] + cryptbuf[i + 7]) & 255];
	}

	cryptbuf[128 - 8] = nwcryptdata[(BYTE)cryptbuf[128 - 8] & 255];
	...
	BYTE *outbuf)
{
	...
	while (buflen >= 8) {
		...
		for (i = 0; i < 8; i++, outbuf++) *outbuf ^= p[i];
		buf += 8;
		buflen -= 8;
	}
	memset(cryptbuf, 0, sizeof(cryptbuf));
...
{
	uint16 i, j;
	BYTE *p = hashbuf + hashbuf[0x40];

	p[0x20] = p[0x00] ^ (p[0x10] = c);
	hashbuf[0x41] = (p[0x30] ^= nwhashdata[(c ^ hashbuf[0x41])]);
	if (!(hashbuf[0x40] = (hashbuf[0x40] + 1) & 15)) {
		c = 0;
		for (i = 18; i; i--) {
			for (j = 48, p = hashbuf; j; j--) {
				c = (*(p++) ^= nwhashdata[(c + j) & 255]);
			}
		}
	}
}
...
{
	while (datalen--) nwhash2(hashbuf, *data++);
}
...
void nwhash2end(BYTE *hashbuf)
{
	...
	for(j = i = 16 - hashbuf[0x40]; j; j--)  nwhash2(hashbuf, i);

	for(i = 0x30; i < 0x40; i++) nwhash2(hashbuf, hashbuf[i]);
}
