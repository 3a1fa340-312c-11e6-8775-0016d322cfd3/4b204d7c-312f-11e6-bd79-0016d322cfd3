...
typedef struct _SupportFunction{
		...
		BYTE IPX_enable:1;   	  //bit 24: IPX			
		BYTE WLWEP128_4KEYS:1;	  //bit 25: Wireless WEP 128-bit 4 keys
		BYTE WPS:1;				  //bit 26: WPS
...
// SMB (56) -----------------------------------------------------------------
	BYTE SMBLen;					// (55)
	BYTE SMBWorkGroup[NCBNAMSZ];	// Workgroup Name: 16
	BYTE SMBServiceName[3][SERVICENAME_LENGTH];	// Shared Printer Name: 39 = 3 * 13

// IPX (2) ------------------------------------------------------------------
	BYTE IPXLen;					// (1)
	BYTE IPXDisable;				// IPX enable/disable: 1
	
// Wireless WEP 128-bit (61) ------------------------------------------------
	BYTE WLWEP128Len;				// (61)
...
typedef  struct
{
	...
	BYTE OEMVer;
	BYTE IP[4];		        // IP address
	BYTE Code1Ver[12];      // code1 firmware ver.
	BYTE Code2Ver[12];      // code2 firmware ver.
	BYTE IsDHCP;
	BYTE Model;
	BYTE WLZone;
}PACK VIEW_DATA;
