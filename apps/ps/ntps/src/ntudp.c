...
void NTStartPrintUDP (BYTE               Version,
                      ...
                      BYTE               MaxPackets)
{
    ...
    if (Version == NT_VERSION_4) {    //9/8/99 added
        ...
        if (PortStatus == PORT_PRINTING) {
            PortStatus = 0;    //6/23/99
        }
    } else {
        ...
        switch (NTUDPStatus) {
...
#ifndef SUPPORT_JOB_LOG
                //Port Name
                memcpy(NTPortInfo[PortNumber].PortName,
                       PortName,
                       NT_MAX_PORT_NAME_LENGTH);
#endif SUPPORT_JOB_LOG
            ...
            case NTUsed:
                ...
                if (!NTPortInfo[PortNumber].HaveRecvPacket &&
                    (NTPortInfo[PortNumber].RemoteSocket.sin_port ==
                     fromsock->sin_port) &&
                    (NTPortInfo[PortNumber].RemoteSocket.sin_addr.s_addr ==
                     fromsock->sin_addr.s_addr) &&
    ...
    NTSendAckStartPrintUDP(Version,
                           PortNumber + 1,
                           NTUDPStatus,
                           SocketNumber,
                           &printsock);
...
BYTE NTRequestUDP (BYTE PortNumber, BYTE *Data, int16 *DataLength)
{
    ...
    char           *recvbuf        = (char *)Data - sizeof(NTReqCmdHeader);
    NTReqCmdHeader *NTReqcmdheader = (NTReqCmdHeader *)recvbuf;
    /*
     * sizeof(NTReqCmdHeader) = 11
     */
    char tempbuf[11];
    ...
    *DataLength = 0;    //6/25/99 ONE PACKET ONLY
    ...
    do {
        ...
        for (i = 0; i < RecvPackets; i++) {
             ...
             */
            memcpy(tempbuf, recvbuf, sizeof(NTReqCmdHeader));

            do {
                RecvBytes = recvfrom(peer_socket[PortNumber],
                                     recvbuf,
                                     sizeof(NTRequestData),
                                     0,
                                     (struct sockaddr *)&printsock,
                                     &fromlen);
                ...
                if (IsNTEndPrint[PortNumber]) {           //move from above   12/6/98
                    *DataLength = 0;                      //6/25/99 ONE PACKET ONLY
                     ...
                     */
                    memcpy(recvbuf, tempbuf, sizeof(NTReqCmdHeader));
                    return(PRN_Q_EOF);
                }
                ...
                if ((RecvBytes < 0) || (++RecvRetryCount > NTUDP_RETRY_COUNT)) {
                    ...
                    if (*DataLength) {
                        return(PRN_Q_NORMAL);             //7/05/99 ONE PACKET ONLY
                    }else {
                        return(PRN_Q_ABORT);              //6/05/99 ONE PACKET ONLY
                    }
                }
...
#endif SUPPORT_JOB_LOG
            } while (NTReqcmdheader->BlockNumber !=
            ...
            if ((BYTE)RecvLen) {
                RecvLen -= 0x100;                                      //6/25/99 ONE PACKET ONLY
            }
            ...
            *DataLength += RecvLen;                                    //6/25/99 ONE PACKET ONLY
            ...
            recvbuf       += RecvLen;
            NTReqcmdheader = (NTReqCmdHeader *)recvbuf;

            NTPortInfo[PortNumber].BlockNumber++;
            NTPortInfo[PortNumber].HaveRecvPacket = 1;
        }    /* for */

        //6/25/99 ONE PACKET ONLY
...
//ZOTIPS    armond_printf("open socket cnt = %d \n",util_socket);
    ...
    for (;;) {
        ...
        if(recvfrom(util_socket, 
                    &NTUDPUtilData, 
                    sizeof(NTUDPUtilData), 
                    0,
...
void NTDoCommandUDP(struct sockaddr_in *fromsock)
{
    switch (NTUDPUtilData.Nt2.Cmd[0]) {
        ...
        case CMD0A:    // Start Print
            NTStartPrintUDP(NT_VERSION_4,
                            fromsock,
                            NTUDPUtilData.Nt2.Cmd[1]);
            break;
        ...
        case CMD11:
            UtilReadPrinterDataUDP(NTUDPUtilData.Nt2.Data[0],
                                   NGET16(&NTUDPUtilData.Nt2.Data[1]),
                                   fromsock);
            break;

        case CMD12:
            UtilWritePrinterDataUDP(NTUDPUtilData.Nt2.Data[0],
                                    NGET16(&NTUDPUtilData.Nt2.Data[1]),
                                    fromsock);
            break;
...
void UtilConfigBoxUDP(struct sockaddr_in *fromsock)
{
     ...
     */
    if (memcmp(MyPhysNodeAddress,
               ((LIST_VIEW *)NTUDPUtilData.Nt2.Data)->NodeID,
               6) != 0) {
        return;
    }
...
void UtilPrintStatusUDP(struct sockaddr_in *fromsock)
{
     ...
     * print status
     */
    NTUDPUtilData.Nt2.Data[0] = ReadPrintStatus();
    ...
    for (PortNumber = 0; PortNumber < NUM_OF_PRN_PORT; PortNumber++) {
        ...
        if (NTUDPUtilData.Nt2.Data[PortNumber + 2]) {
            NTUDPUtilData.Nt2.Data[PortNumber + 2]--;
        }
    }
