...
//0507	{ RW_PCVAL(read,inbuf,outbuf,len) \
//0507	DEBUG(5,("%s%04x %s: ", \
//0507             tab_depth(depth), base,string)); \
//0507    if (charmode) print_asc(5, (unsigned char*)(outbuf), (len)); else \
//0507	{ uint32 idx; for (idx = 0; idx < len; idx++) { DEBUG(5,("%02x ", (outbuf)[idx])); } } \
//0507	DEBUG(5,("\n")); } 

#define DBG_RW_PSVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
		RW_PSVAL(read,big_endian,inbuf,outbuf,len)
//0507	{ RW_PSVAL(read,big_endian,inbuf,outbuf,len) \
//0507	DEBUG(5,("%s%04x %s: ", \
//0507             tab_depth(depth), base,string)); \
//0507    if (charmode) print_asc(5, (unsigned char*)(outbuf), 2*(len)); else \
//0507	{ uint32 idx; for (idx = 0; idx < len; idx++) { DEBUG(5,("%04x ", (outbuf)[idx])); } } \
//0507	DEBUG(5,("\n")); }

#define DBG_RW_PIVAL(charmode,string,depth,base,read,big_endian,inbuf,outbuf,len) \
		RW_PIVAL(read,big_endian,inbuf,outbuf,len)
//0507	{ RW_PIVAL(read,big_endian,inbuf,outbuf,len) \
//0507	DEBUG(5,("%s%04x %s: ", \
//0507             tab_depth(depth), base,string)); \
//0507    if (charmode) print_asc(5, (unsigned char*)(outbuf), 4*(len)); else \
//0507	{ uint32 idx; for (idx = 0; idx < len; idx++) { DEBUG(5,("%08x ", (outbuf)[idx])); } } \
//0507	DEBUG(5,("\n")); }

#define DBG_RW_CVAL(string,depth,base,read,inbuf,outbuf) \
		RW_CVAL(read,inbuf,outbuf,0)
//0507	{ RW_CVAL(read,inbuf,outbuf,0) \
//0507	DEBUG(5,("%s%04x %s: %02x\n", \
//0507             tab_depth(depth), base, string, outbuf)); }

#define DBG_RW_SVAL(string,depth,base,read,big_endian,inbuf,outbuf) \
		RW_SVAL(read,big_endian,inbuf,outbuf,0)
//0507	{ RW_SVAL(read,big_endian,inbuf,outbuf,0) \
//0507	DEBUG(5,("%s%04x %s: %04x\n", \
//0507             tab_depth(depth), base, string, outbuf)); }

#define DBG_RW_IVAL(string,depth,base,read,big_endian,inbuf,outbuf) \
		RW_IVAL(read,big_endian,inbuf,outbuf,0)
//0507	{ RW_IVAL(read,big_endian,inbuf,outbuf,0) \
//0507	DEBUG(5,("%s%04x %s: %08x\n", \
//0507             tab_depth(depth), base, string, outbuf)); }
