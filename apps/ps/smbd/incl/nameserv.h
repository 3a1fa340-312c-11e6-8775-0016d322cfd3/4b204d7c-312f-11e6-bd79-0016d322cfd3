...
struct work_record
{
//0718  struct work_record *next;
//0718  struct work_record *prev;

//0718  struct subnet_record *subnet;

//0718  struct server_record *serverlist;

  /* Stage of development from non-local-master up to local-master browser. */
//0718  enum master_state mst_state;

  /* Stage of development from non-domain-master to domain-master browser. */
//0718  enum domain_state dom_state;

  /* Stage of development from non-logon-server to logon server. */
//0718  enum logon_state log_state;

  /* Work group info. */
  fstring work_group;
//0718  int     token;        /* Used when communicating with backup browsers. */
//0718  fstring local_master_browser_name;      /* Current local master browser. */
...
//0718  time_t death_time;  

  /* Election info */
//0718  BOOL    RunningElection;
//0718  BOOL    needelection;
//0718  int     ElectionCount;
//0718  uint32  ElectionCriterion;

  /* Domain master browser info. Used for efficient syncs. */
//0718  struct nmb_name dmb_name;
//0718  struct in_addr dmb_addr;
};
...
struct subnet_record
{
  ...
  char  *subnet_name[30];	/* For Debug identification. */
...
#define CHECK_TIME_MAX_HOST_ANNCE  1 // Ron Modify , In order to let Announce_My_Server_name sooner 2/22/2002.
...
// 0314 #define FIRST_SUBNET subnetlist
#define FIRST_SUBNET &_subnetlist[0]
