...
//from smbutil.c
extern void *Realloc(void *p,uint32 size);
...
int prs_init(prs_struct *ps, uint32 size, uint8 align, int io)
{
//2/8/2002	ZERO_STRUCTP(ps);
	memset((char *)ps ,0, sizeof(prs_struct));		
	ps->io = io;
	...
	if (size != 0) {
		ps->buffer_size = size;
		if((ps->data_p = (char *)malloc((uint32)size)) == NULL) {
...
void prs_give_memory(prs_struct *ps, char *buf, uint32 size, int is_dynamic)
{
...
int prs_grow(prs_struct *ps, uint32 extra_space)
{
	uint32 new_size;
	char *new_data;
...
//0508	if(UNMARSHALLING(ps) || !ps->is_dynamic) {
 ...
 ********************************************************************/
#if 0 //0607
int prs_force_grow(prs_struct *ps, uint32 extra_space)
{
...
int prs_set_offset(prs_struct *ps, uint32 offset)
{
...
int prs_append_prs_data(prs_struct *dst, prs_struct *src)
{
	if(!prs_grow(dst, prs_offset(src)))
		return False;

	memcpy(&dst->data_p[dst->data_offset], prs_data_p(src), (uint32)prs_offset(src));
...
int prs_append_data(prs_struct *dst, char *src, uint32 len)
{
	if(!prs_grow(dst, len))
		return False;

	memcpy(&dst->data_p[dst->data_offset], src, (uint32)len);
	dst->data_offset += len;
...
int prs_align(prs_struct *ps)
{
	...
	if (ps->align != 0 && mod != 0) {
		...
		if(!prs_grow(ps, extra_space))
			return False;
		memset(&ps->data_p[ps->data_offset], '\0', (uint32)extra_space);
...
void prs_switch_type(prs_struct *ps, int io)
{
	if ((ps->io ^ io) == True)
		ps->io=io;
}
...
int prs_uint8(char *name, prs_struct *ps, int depth, uint8 *data8)
{
...
int prs_uint16(char *name, prs_struct *ps, int depth, uint16 *data16)
{
...
int prs_uint32(char *name, prs_struct *ps, int depth, uint32 *data32)
{
	char *q = prs_mem_get(ps, sizeof(uint32));
	if (q == NULL)
		return False;
//0612	DBG_RW_IVAL(name, depth, ps->data_offset, ps->io, ps->bigendian_data, q, *data32)
	if (ps->io){
 ...
 ********************************************************************/
#if 0 //0709
int prs_uint32s(int charmode, char *name, prs_struct *ps, int depth, uint32 *data32s, int len)
{
...
int prs_buffer2(int charmode, char *name, prs_struct *ps, int depth, BUFFER2 *str)
{
...
int prs_string2(int charmode, char *name, prs_struct *ps, int depth, STRING2 *str)
{
...
int prs_unistr2(int charmode, char *name, prs_struct *ps, int depth, UNISTR2 *str)
{
	char *p = (char *)str->buffer;
 ...
 ********************************************************************/
#if 0 //0709
int prs_unistr3(int charmode, char *name, UNISTR3 *str, prs_struct *ps, int depth)
{
...
int prs_unistr(char *name, prs_struct *ps, int depth, UNISTR *str)
{
	int len = 0;
 ...
 ********************************************************************/
#if 0 //0509
int prs_string(char *name, prs_struct *ps, int depth, char *str, int len, int max_buf_size)
{
	char *q;
	uint8 *start;
	int i;
 ...
 ********************************************************************/
#if 0 //0709
int prs_uint16_pre(char *name, prs_struct *ps, int depth, uint16 *data16, uint32 *offset)
{
 ...
 ********************************************************************/
#if 0 //0709
int prs_uint16_post(char *name, prs_struct *ps, int depth, uint16 *data16,
 ...
 ********************************************************************/
#if 0 //0709
int prs_uint32_pre(char *name, prs_struct *ps, int depth, uint32 *data32, uint32 *offset)
{
	(*offset) = ps->data_offset;
	if (UNMARSHALLING(ps)) {
		/* reading. */
 ...
 ********************************************************************/
#if 0 //0709
int prs_uint32_post(char *name, prs_struct *ps, int depth, uint32 *data32,
