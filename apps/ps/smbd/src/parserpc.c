...
extern void prs_set_bigendian_data(prs_struct *ps);
extern int prs_align(prs_struct *ps);


//0504 extern int DEBUGLEVEL;
...
//0509	{ PIPE_LSARPC  , SYNT_LSARPC_V0  , PIPE_LSASS    , TRANS_SYNT_V2 },
//0509	{ PIPE_SAMR    , SYNT_SAMR_V1    , PIPE_LSASS    , TRANS_SYNT_V2 },
//0509	{ PIPE_NETLOGON, SYNT_NETLOGON_V1, PIPE_LSASS    , TRANS_SYNT_V2 },
...
int smb_io_rpc_hdr(char *desc,  RPC_HDR *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr");
	depth++;
...
static int smb_io_rpc_iface(char *desc, RPC_IFACE *ifc, prs_struct *ps, int depth)
{
	if (ifc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_iface");
	depth++;
...
static int smb_io_rpc_addr_str(char *desc,  RPC_ADDR_STR *str, prs_struct *ps, int depth)
{
	if (str == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_addr_str");
	depth++;
	if(!prs_align(ps))
		return False;
...
static int smb_io_rpc_hdr_bba(char *desc,  RPC_HDR_BBA *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_bba");
	depth++;
...
int smb_io_rpc_hdr_rb(char *desc, RPC_HDR_RB *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_rb");
	depth++;
...
static int smb_io_rpc_results(char *desc, RPC_RESULTS *res, prs_struct *ps, int depth)
{
	if (res == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_results");
	depth++;
...
int smb_io_rpc_hdr_ba(char *desc, RPC_HDR_BA *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_ba");
	depth++;
...
int smb_io_rpc_hdr_req(char *desc, RPC_HDR_REQ *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_req");
	depth++;
...
int smb_io_rpc_hdr_resp(char *desc, RPC_HDR_RESP *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_resp");
	depth++;
...
int smb_io_rpc_hdr_fault(char *desc, RPC_HDR_FAULT *rpc, prs_struct *ps, int depth)
{
	if (rpc == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_fault");
	depth++;
...
********************************************************************/
#if 0 //0509
int smb_io_rpc_hdr_autha(char *desc, RPC_HDR_AUTHA *rai, prs_struct *ps, int depth)
{
	if (rai == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_autha");
	depth++;
...
int rpc_hdr_auth_chk(RPC_HDR_AUTH *rai)
{
...
int smb_io_rpc_hdr_auth(char *desc, RPC_HDR_AUTH *rai, prs_struct *ps, int depth)
{
	if (rai == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_hdr_auth");
	depth++;
...
int rpc_auth_verifier_chk(RPC_AUTH_VERIFIER *rav,
				char *signature, uint32 msg_type)
{
//0504	return (strequal(rav->signature, signature) && rav->msg_type == msg_type);
	return (!strcmp(rav->signature, signature) && rav->msg_type == msg_type);
}
...
********************************************************************/
#if 0 //0509
int smb_io_rpc_auth_verifier(char *desc, RPC_AUTH_VERIFIER *rav, prs_struct *ps, int depth)
{
	if (rav == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_auth_verifier");
	depth++;
...
********************************************************************/
#if 0 //0504
int smb_io_rpc_auth_ntlmssp_neg(char *desc, RPC_AUTH_NTLMSSP_NEG *neg, prs_struct *ps, int depth)
{
	uint32 start_offset = prs_offset(ps);
	if (neg == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_auth_ntlmssp_neg");
	depth++;
...
//2/8/2002		ZERO_STRUCTP(neg);
		memset((char *)neg ,0, sizeof(RPC_AUTH_NTLMSSP_NEG));		
...
********************************************************************/
#if 0 //0504
int smb_io_rpc_auth_ntlmssp_chal(char *desc, RPC_AUTH_NTLMSSP_CHAL *chl, prs_struct *ps, int depth)
{
	if (chl == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_auth_ntlmssp_chal");
	depth++;
...
********************************************************************/
#if 0 //0504
int smb_io_rpc_auth_ntlmssp_resp(char *desc, RPC_AUTH_NTLMSSP_RESP *rsp, prs_struct *ps, int depth)
{
	if (rsp == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_auth_ntlmssp_resp");
	depth++;
...
//2/8/2002		ZERO_STRUCTP(rsp);
		memset((char *)rsp, 0,sizeof(RPC_AUTH_NTLMSSP_RESP));
...
********************************************************************/
#if 0 //0709
int rpc_auth_ntlmssp_chk(RPC_AUTH_NTLMSSP_CHK *chk, uint32 crc32, uint32 seq_num)
{
	if (chk == NULL)
		return False;
...
int smb_io_rpc_auth_ntlmssp_chk(char *desc, RPC_AUTH_NTLMSSP_CHK *chk, prs_struct *ps, int depth)
{
	if (chk == NULL)
		return False;

//0607	prs_debug(ps, depth, desc, "smb_io_rpc_auth_ntlmssp_chk");
	depth++;
