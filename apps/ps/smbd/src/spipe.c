...
int setup_fault_pdu(pipes_struct *p);
...
extern int smb_io_rpc_hdr_resp(char *desc, RPC_HDR_RESP *rpc, prs_struct *ps, int depth);
extern void init_rpc_hdr_auth(RPC_HDR_AUTH *rai, uint8 auth_type, uint8 auth_level,	uint8 stub_type_len, uint32 ptr);
extern int smb_io_rpc_hdr_auth(char *desc, RPC_HDR_AUTH *rai, prs_struct *ps, int depth);
extern int smb_io_rpc_auth_ntlmssp_chk(char *desc, RPC_AUTH_NTLMSSP_CHK *chk, prs_struct *ps, int depth);
...
int create_next_pdu(pipes_struct *p)
{
	RPC_HDR_RESP hdr_resp;
	int auth_verify = IS_BITS_SET_ALL(p->ntlmssp_chal_flags, NTLMSSP_NEGOTIATE_SIGN);
	int auth_seal   = IS_BITS_SET_ALL(p->ntlmssp_chal_flags, NTLMSSP_NEGOTIATE_SEAL);
	uint32 data_len;
	...
	if(!data_len_left) {
//0508		DEBUG(0,("create_next_pdu: no data left to send !\n"));
		return False;
	}
...
//0508		DEBUG(0,("create_next_pdu: failed to marshall RPC_HDR.\n"));
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
//0508		DEBUG(0,("create_next_pdu: failed to marshall RPC_HDR_RESP.\n"));
		return False;
	}
...
//0508		DEBUG(0,("create_next_pdu: failed to copy %u bytes of data.\n", (unsigned int)data_len));
		return False;
	}
...
//0508		DEBUG(5,("create_next_pdu: sign: %s seal: %s data %d auth %d\n",
//0508			 BOOLSTR(auth_verify), BOOLSTR(auth_seal), data_len, p->hdr.auth_len));
...
//0508				DEBUG(0,("create_next_pdu: failed to marshall RPC_HDR_AUTH.\n"));
				return False;
			}
		}
...
//0508				DEBUG(0,("create_next_pdu: failed to marshall RPC_AUTH_NTLMSSP_CHK.\n"));
				return False;
			}
 ...
 *******************************************************************/
#if 0 //0508
static int api_pipe_ntlmssp_verify(pipes_struct *p, RPC_AUTH_NTLMSSP_RESP *ntlmssp_resp)
{
	uchar lm_owf[24];
	uchar nt_owf[24];
	fstring user_name;
	...
	fstring domain;
	fstring wks;
	int guest_user = False;
	struct smb_passwd *smb_pass = NULL;
	struct passwd *pass = NULL;
	uchar null_smb_passwd[16];
	uchar *smb_passwd_ptr = NULL;
	
//0508	DEBUG(5,("api_pipe_ntlmssp_verify: checking user details\n"));
...
//0508		fstrcpy(user_name, dos_unistrn2((uint16*)ntlmssp_resp->user, ntlmssp_resp->hdr_usr.str_str_len/2));
//0508		fstrcpy(domain, dos_unistrn2((uint16*)ntlmssp_resp->domain, ntlmssp_resp->hdr_domain.str_str_len/2));
//0508		fstrcpy(wks, dos_unistrn2((uint16*)ntlmssp_resp->wks, ntlmssp_resp->hdr_wks.str_str_len/2));
	} else {
...
//0508	DEBUG(5,("user: %s domain: %s wks: %s\n", user_name, domain, wks));

	memcpy(lm_owf, ntlmssp_resp->lm_resp, sizeof(lm_owf));
	memcpy(nt_owf, ntlmssp_resp->nt_resp, sizeof(nt_owf));

#ifdef DEBUG_PASSWORD
//0508	DEBUG(100,("lm, nt owfs, chal\n"));
//0508 dump_data(100, (char *)lm_owf, sizeof(lm_owf));
//0508	dump_data(100, (char *)nt_owf, sizeof(nt_owf));
//0508	dump_data(100, (char *)p->challenge, 8);
#endif
...
//0508	    fstrcpy(unix_user_name, lp_guestaccount(-1));
//0508	    DEBUG(100,("Null user in NTLMSSP verification. Using guest = %s\n", unix_user_name));
...
//0508		DEBUG(1,("Couldn't find user '%s' in UNIX password database.\n",unix_user_name));
		return(False);
	}

	if(!guest_user) {

//0508		become_root(True);

		if(!(p->ntlmssp_auth_validated = pass_check_smb(unix_user_name, domain,
				      (uchar*)p->challenge, lm_owf, nt_owf, NULL))) {
//0508			DEBUG(1,("api_pipe_ntlmssp_verify: User %s\\%s from machine %s \
//0508failed authentication on named pipe %s.\n", domain, unix_user_name, wks, p->name ));
//0508			unbecome_root(True);
			return False;
		}

		if(!(smb_pass = getsmbpwnam(unix_user_name))) {
//0508			DEBUG(1,("api_pipe_ntlmssp_verify: Cannot find user %s in smb passwd database.\n",
//0508				unix_user_name));
//0508			unbecome_root(True);
			return False;
		}
...
//0508		unbecome_root(True);
		...
		if (smb_pass == NULL) {
//0508			DEBUG(1,("api_pipe_ntlmssp_verify: Couldn't find user '%s' in smb_passwd file.\n", 
//0508				unix_user_name));
			return(False);
		}
...
//0508			DEBUG(1,("Account for user '%s' was disabled.\n", unix_user_name));
			return(False);
		}

		if(!smb_pass->smb_nt_passwd) {
//0508			DEBUG(1,("Account for user '%s' has no NT password hash.\n", unix_user_name));
			return(False);
		}
...
{
//0508    { "lsarpc",   "lsass",   api_ntlsa_rpc },
//0508    { "samr",     "lsass",   api_samr_rpc },
    { "srvsvc",   "ntsvcs",  api_srvsvc_rpc },
    { "wkssvc",   "ntsvcs",  api_wkssvc_rpc },
//0508    { "NETLOGON", "lsass",   api_netlog_rpc },
...
*******************************************************************/
#if 0 //0508
int api_pipe_bind_auth_resp(pipes_struct *p, prs_struct *rpc_in_p)
{
	RPC_HDR_AUTHA autha_info;
	RPC_AUTH_VERIFIER auth_verifier;
	RPC_AUTH_NTLMSSP_RESP ntlmssp_resp;

//0508	DEBUG(5,("api_pipe_bind_auth_resp: decode request. %d\n", __LINE__));

	if (p->hdr.auth_len == 0) {
//0508		DEBUG(0,("api_pipe_bind_auth_resp: No auth field sent !\n"));
		return False;
	}
...
//0508		DEBUG(0,("api_pipe_bind_auth_resp: unmarshall of RPC_HDR_AUTHA failed.\n"));
		return False;
	}

	if (autha_info.auth_type != NTLMSSP_AUTH_TYPE || autha_info.auth_level != NTLMSSP_AUTH_LEVEL) {
//0508		DEBUG(0,("api_pipe_bind_auth_resp: incorrect auth type (%d) or level (%d).\n",
//0508			(int)autha_info.auth_type, (int)autha_info.auth_level ));
		return False;
	}

	if(!smb_io_rpc_auth_verifier("", &auth_verifier, rpc_in_p, 0)) {
//0508		DEBUG(0,("api_pipe_bind_auth_resp: unmarshall of RPC_AUTH_VERIFIER failed.\n"));
		return False;
	}
...
//0508		DEBUG(0,("api_pipe_bind_auth_resp: rpc_auth_verifier_chk failed.\n"));
		return False;
	}

//	if(!smb_io_rpc_auth_ntlmssp_resp("", &ntlmssp_resp, rpc_in_p, 0)) {
//0508		DEBUG(0,("api_pipe_bind_auth_resp: Failed to unmarshall RPC_AUTH_NTLMSSP_RESP.\n"));
//		return False;
//	}
...
*******************************************************************/
#if 0 //0509
static int setup_bind_nak(pipes_struct *p)
{
...
//0508		DEBUG(0,("setup_bind_nak: marshalling of RPC_HDR failed.\n"));
		return False;
	}
...
//0508		DEBUG(0,("setup_fault_pdu: marshalling of RPC_HDR failed.\n"));
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
//0508		DEBUG(0,("setup_fault_pdu: failed to marshall RPC_HDR_RESP.\n"));
		return False;
	}

	if(!smb_io_rpc_hdr_fault("fault", &fault_resp, &outgoing_pdu, 0)) {
//0508		DEBUG(0,("setup_fault_pdu: failed to marshall RPC_HDR_FAULT.\n"));
		return False;
	}
...
****************************************************************************/
#if 0 //0509
int api_pipe_auth_process(pipes_struct *p, prs_struct *rpc_in)
{
	/*
...
//0508		DEBUG(0,("api_pipe_auth_process: Incorrect auth_len %d.\n", auth_len ));
		return False;
	}
...
//0508	DEBUG(5,("api_pipe_auth_process: sign: %s seal: %s data %d auth %d\n",
//0508		 BOOLSTR(auth_verify), BOOLSTR(auth_seal), data_len, auth_len));
...
//		NTLMSSPcalc_p(p, (uchar*)data, data_len);
//0508		crc32 = crc32_calc_buffer(data, data_len);
	}
...
//0508			DEBUG(0,("api_pipe_auth_process: cannot move offset to %u.\n",
//0508				(unsigned int)old_offset + data_len ));
			return False;
		}

		if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, rpc_in, 0)) {
//0508			DEBUG(0,("api_pipe_auth_process: failed to unmarshall RPC_HDR_AUTH.\n"));
			return False;
		}
	}
...
//0508		DEBUG(5,("api_pipe_auth_process: auth %d\n", prs_offset(rpc_in) + 4));
...
//0508			DEBUG(0,("api_pipe_auth_process: missing %d bytes in buffer.\n",
//0508				RPC_AUTH_NTLMSSP_CHK_LEN - 4 ));
			return False;
		}

//		NTLMSSPcalc_p(p, (uchar*)req_data, RPC_AUTH_NTLMSSP_CHK_LEN - 4);
		if(!smb_io_rpc_auth_ntlmssp_chk("auth_sign", &ntlmssp_chk, rpc_in, 0)) {
//0508			DEBUG(0,("api_pipe_auth_process: failed to unmarshall RPC_AUTH_NTLMSSP_CHK.\n"));
			return False;
		}

		if (!rpc_auth_ntlmssp_chk(&ntlmssp_chk, crc32, p->ntlmssp_seq_num)) {
//0508			DEBUG(0,("api_pipe_auth_process: NTLMSSP check failed.\n"));
			return False;
		}
	}
...
//0508		DEBUG(0,("api_pipe_auth_process: failed to set offset back to %u\n",
//0508			(unsigned int)old_offset ));
		return False;
	}
...
int api_pipe_request(pipes_struct *p)
{
	int i = 0;
	int ret = False;
//0709	int changed_user_id = False;

	if (p->ntlmssp_auth_validated) {

//0508		if(!become_authenticated_pipe_user(p)) {
//			prs_mem_free(&p->out_data.rdata);
//			return False;
//		}
...
//		changed_user_id = True;
	...
	for (i = 0; api_fd_commands[i].pipe_clnt_name; i++) {
		if (!strcmp(api_fd_commands[i].pipe_clnt_name, p->name) &&
		    api_fd_commands[i].fn != NULL) {
//0508			DEBUG(3,("Doing \\PIPE\\%s\n", api_fd_commands[i].pipe_clnt_name));
			ret = api_fd_commands[i].fn(p, &p->in_data.data);
		}
	}

//0508	if(changed_user_id)
...
int api_rpcTNP(pipes_struct *p, char *rpc_name, struct api_struct *api_rpc_cmds,
				prs_struct *rpc_in)
{
	int fn_num;

	/* interpret the command */
//0508	DEBUG(4,("api_rpcTNP: %s op 0x%x - ", rpc_name, p->hdr_req.opnum));

	for (fn_num = 0; api_rpc_cmds[fn_num].name; fn_num++) {
		if (api_rpc_cmds[fn_num].opnum == p->hdr_req.opnum && api_rpc_cmds[fn_num].fn != NULL) {
//0508			DEBUG(3,("api_rpcTNP: rpc command: %s\n", api_rpc_cmds[fn_num].name));
			break;
		}
	}
...
//0508		DEBUG(0,("api_rpcTNP: %s: failed.\n", rpc_name));
		prs_mem_free(&p->out_data.rdata);
		return False;
	}

//0508	DEBUG(5,("api_rpcTNP: called %s successfully\n", rpc_name));
