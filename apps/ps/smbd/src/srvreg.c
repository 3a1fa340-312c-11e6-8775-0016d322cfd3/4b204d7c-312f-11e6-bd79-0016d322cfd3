...
//from util_hnd.c
extern int find_lsa_policy_by_hnd(POLICY_HND *hnd);

//from parsereg.c
extern void init_reg_r_info(REG_R_INFO *r_r, uint32 level, char *os_type, uint32 unknown_0, uint32 unknown_1,	uint32 status);
extern int reg_io_r_info(char *desc, REG_R_INFO *r_r, prs_struct *ps, int depth);
extern int reg_io_q_info(char *desc,  REG_Q_INFO *r_q, prs_struct *ps, int depth);
 ...
 ********************************************************************/
#if 0 //0705
static int api_reg_close( uint16 vuid, prs_struct *data,
				    prs_struct *rdata )
{
	REG_Q_CLOSE q_r;
 ...
 ********************************************************************/
#if 0 //0705
static int api_reg_open( uint16 vuid, prs_struct *data,
				    prs_struct *rdata )
{
	REG_Q_OPEN_HKLM q_u;
...
//0606	DEBUG(5,("reg_open_entry: %d\n", __LINE__));
	...
	fstrcpy(name, dos_unistrn2((uint16 *)(q_u->uni_name.buffer), q_u->uni_name.uni_str_len));

	if (status == 0x0)
	{
//0606		DEBUG(5,("reg_open_entry: %s\n", name));
		/* lkcl XXXX do a check on the name, here */
//0606		if (!strequal(name, "SYSTEM\\CurrentControlSet\\Control\\ProductOptions"))
		if (strcmp(name, "SYSTEM\\CurrentControlSet\\Control\\ProductOptions"))
		{
 ...
 ********************************************************************/
#if 0 //0705
static int api_reg_open_entry( uint16 vuid, prs_struct *data,
				    prs_struct *rdata )
{
...
/*******************************************************************
 api_reg_info
 ********************************************************************/
static int api_reg_info( uint16 vuid, prs_struct *data,
				    prs_struct *rdata )
{
	REG_Q_INFO q_u;
 ...
 ********************************************************************/
int api_reg_rpc(pipes_struct *p, prs_struct *data)
{
