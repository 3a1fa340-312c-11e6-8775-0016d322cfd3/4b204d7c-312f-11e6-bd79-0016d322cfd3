...
#define UNIXERROR(x,y)	0 
#define slprintf

//-------------------------------------------

//Jesse extern int DEBUGLEVEL;
//Jesse extern int Protocol;
extern int Protocol[];
//Jesse extern BOOL case_sensitive;
BOOL case_sensitive;
//Jesse extern int Client;
int Client;
extern int smb_read_error;
//Jesse extern fstring local_machine;
//Jesse extern int global_oplock_break;
...
static int send_trans2_replies(char *outbuf, int bufsize, char *params, 
			       int paramsize, char *pdata, int datasize, int ProcSockID)
{
...
//Jesse  if (((params_to_send % 4) != 0) && (data_to_send != 0))
//Jesse    data_alignment_offset = 4 - (params_to_send % 4);
...
//Jesse    SSVAL(outbuf,smb_proff,((smb_buf(outbuf)+alignment_offset) - smb_base(outbuf)));
SSVAL(outbuf,smb_proff,((smb_buf(outbuf)+alignment_offset) - smb_base(outbuf))-2);
...
//Jesse      SSVAL(outbuf,smb_droff,((smb_buf(outbuf)+alignment_offset) - 
//Jesse            smb_base(outbuf)) + params_sent_thistime + data_alignment_offset);
...
//Jesse    DEBUG(9,("t2_rep: params_sent_thistime = %d, data_sent_thistime = %d, useable_space = %d\n",
//Jesse          params_sent_thistime, data_sent_thistime, useable_space));
//Jesse    DEBUG(9,("t2_rep: params_to_send = %d, data_to_send = %d, paramsize = %d, datasize = %d\n",
//Jesse          params_to_send, data_to_send, paramsize, datasize));

    /* Send the packet */
//Jesse    send_smb(Client,outbuf);
    ...
    if(params_to_send < 0 || data_to_send < 0)
    {
//Jesse      DEBUG(0,("send_trans2_replies failed sanity check pts = %d, dts = %d\n!!!",
//Jesse            params_to_send, data_to_send));
      return -1;
    }
  }
...
//Jesse
  ...
  SMB_INO_T inode = 0;
//Jesse  SMB_STRUCT_STAT sbuf;
  int smb_action = 0;
  BOOL bad_path = False;
//Jesse  files_struct *fsp;

  StrnCpy(fname,pname,namelen);

//Jesse  DEBUG(3,("trans2open %s mode=%d attr=%d ofun=%d size=%d\n",
//Jesse	   fname,open_mode, open_attr, open_ofun, open_size));
...
//Jesse  fsp = file_new();
//Jesse  if (!fsp)
//Jesse    return(ERROR(ERRSRV,ERRnofids));
...
//Jesse  open_file_shared(fsp,conn,fname,open_mode,open_ofun,unixmode,
//Jesse		   oplock_request, &rmode,&smb_action);
      
//Jesse  if (!fsp->open)
//Jesse  {
//Jesse    if((errno == ENOENT) && bad_path)
//Jesse    {
//Jesse      unix_ERR_class = ERRDOS;
//Jesse      unix_ERR_code = ERRbadpath;
//Jesse    }
//Jesse    file_free(fsp);
//Jesse    return(UNIXERROR(ERRDOS,ERRnoaccess));
//Jesse  }

//Jesse  if (sys_fstat(fsp->fd_ptr->fd,&sbuf) != 0) {
//Jesse    close_file(fsp,False);
//Jesse    return(UNIXERROR(ERRDOS,ERRnoaccess));
//Jesse  }
...
//Jesse    DEBUG(8,("get_lanman2_dir_entry:readdir on dirptr 0x%lx now at offset %d\n",
    ...
    if(got_match)
    {
      ...
      {
//Jesse        DEBUG(5,("get_lanman2_dir_entry:Couldn't stat [%s] (%s)\n",pathreal,strerror(errno)));
        continue;
      }
...
//Jesse        DEBUG(5,("[%s] attribs didn't match %x\n",fname,dirtype));
        continue;
      }
...
//Jesse      DEBUG(5,("get_lanman2_dir_entry found %s fname=%s\n",pathreal,fname));
  ...
  if (PTR_DIFF(p,pdata) > space_remaining) {
    ...
    *out_of_space = True;
//Jesse    DEBUG(9,("get_lanman2_dir_entry: out of space\n"));
...
//Jesse  DEBUG(3,("call_trans2findfirst: dirtype = %d, maxentries = %d, close_after_first=%d, close_if_end = %d requires_resume_key = %d level = %d, max_data_bytes = %d\n",
//Jesse	   dirtype, maxentries, close_after_first, close_if_end, requires_resume_key,
//Jesse	   info_level, max_data_bytes));
...
//Jesse  DEBUG(4,("dptr_num is %d, wcard = %s, attr = %d\n",dptr_num, wcard, dirtype));

  /* We don't need to check for VOL here as this is returned by 
     a different TRANS2 call. */
  
//Jesse  DEBUG(8,("dirpath=<%s> dontdescend=<%s>\n",
//Jesse	   conn->dirpath,lp_dontdescend(SNUM(conn))));
  ...
  {
//Jesse    DEBUG(5,("call_trans2findfirst - (2) closing dptr_num %d\n", dptr_num));
    dptr_close(&dptr_num);
  }
...
//Jesse  DEBUG( 4, ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
//Jesse	    smb_fn_name(CVAL(inbuf,smb_com)), 
//Jesse	    mask, directory, dirtype, numentries ) );
...
//Jesse  DEBUG(3,("call_trans2findnext: dirhandle = %d, max_data_bytes = %d, maxentries = %d, \
//Jesse close_after_request=%d, close_if_end = %d requires_resume_key = %d \
//Jesse resume_key = %d resume name = %s continue=%d level = %d\n",
//Jesse	   dptr_num, max_data_bytes, maxentries, close_after_request, close_if_end, 
//Jesse	   requires_resume_key, resume_key, resume_name, continue_bit, info_level));
...
//Jesse    DEBUG(2,("dptr_num %d has no wildcard\n", dptr_num));
    return (ERROR(ERRDOS,ERRnofiles));
  }
  pstrcpy(mask, p);
...
//Jesse  DEBUG(3,("dptr_num is %d, mask = %s, attr = %x, dirptr=(0x%lX,%d)\n",
//Jesse	   dptr_num, mask, dirtype, 
//Jesse	   (long)conn->dirptr,
//Jesse	   TellDir(conn->dirptr)));
...
//Jesse  DEBUG(8,("dirpath=<%s> dontdescend=<%s>\n",conn->dirpath,lp_dontdescend(SNUM(conn))));
  ...
  if(requires_resume_key && *resume_name && !continue_bit)
  {
    ...
    {
//Jesse      DEBUG(7,("call_trans2findnext: seeking to pos %d\n", current_pos));
...
//Jesse        DEBUG(7,("call_trans2findnext: got match at pos %d\n", current_pos+1 ));
        break;
      }
    }
    ...
    if(current_pos < 0)
    {
//Jesse      DEBUG(7,("call_trans2findnext: notfound: seeking to pos %d\n", start_pos));
...
//Jesse          DEBUG(7,("call_trans2findnext: got match at pos %d\n", current_pos+1 ));
  ...
  {
//Jesse    DEBUG(5,("call_trans2findnext: closing dptr_num = %d\n", dptr_num));
...
//Jesse  DEBUG( 3, ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
//Jesse	    smb_fn_name(CVAL(inbuf,smb_com)), 
//Jesse	    mask, directory, dirtype, numentries ) );
...
//Jesse  DEBUG(3,("call_trans2qfsinfo: level = %d\n", info_level));

  if(dos_stat(".",&st)!=0) {
//Jesse    DEBUG(2,("call_trans2qfsinfo: stat of . failed (%s)\n", strerror(errno)));
...
//Jesse      DEBUG(5,("call_trans2qfsinfo : bsize=%u, id=%x, cSectorUnit=%u, cUnit=%u, cUnitAvail=%u, cbSector=%d\n",
//Jesse		 (unsigned int)bsize, (unsigned int)st.st_dev, ((unsigned int)bsize)/512, (unsigned int)dsize,
//Jesse         (unsigned int)dfree, 512));
      break;
    }
    case 2:
    { 
...
//Jesse      DEBUG(5,("call_trans2qfsinfo : time = %x, namelen = %d, name = %s\n",
//Jesse	       (unsigned)st.st_ctime, volname_len,
//Jesse	       pdata+l2_vol_szVolLabel));
      break;
    }
...
//Jesse      DEBUG(5,("call_trans2qfsinfo : SMB_QUERY_FS_VOLUME_INFO namelen = %d, vol = %s\n", 
//Jesse	       (int)strlen(vname),vname));
      break;
...
//Jesse  DEBUG( 4, ( "%s info_level = %d\n",
//Jesse            smb_fn_name(CVAL(inbuf,smb_com)), info_level) );
				...
				char **pparams, char **ppdata)
{
  ...
  int outsize;
//Jesse  DEBUG(3,("call_trans2setfsinfo\n"));
//Jesse
#if 0
  if (!CAN_WRITE(conn))
...
//Jesse    files_struct *fsp = file_fsp(params,0);
    info_level = SVAL(params,2);

//Jesse    DEBUG(3,("call_trans2qfilepathinfo: TRANSACT2_QFILEINFO: level = %d\n", info_level));
...
//Jesse        DEBUG(3,("fileinfo of %s failed (%s)\n",fname,strerror(errno)));
        if((errno == ENOENT) && bad_path)
        {
          unix_ERR_class = ERRDOS;
          unix_ERR_code = ERRbadpath;
...
//Jesse        DEBUG(3,("fstat of fnum %d failed (%s)\n",fsp->fnum, strerror(errno)));
...
//Jesse    DEBUG(3,("call_trans2qfilepathinfo: TRANSACT2_QPATHINFO: level = %d\n", info_level));
...
//Jesse  DEBUG(3,("call_trans2qfilepathinfo %s level=%d call=%d total_data=%d\n",
//Jesse	   fname,info_level,tran_call,total_data));
  ...
  if (total_data > 0 && IVAL(pdata,0) == total_data) {
    /* uggh, EAs for OS2 */
//Jesse    DEBUG(4,("Rejecting EA request with total_data=%d\n",total_data));
...
//Jesse      DEBUG(5,("SMB_QFBI - "));
      {
        time_t create_time = get_create_time(&sbuf,lp_fake_dir_create_times(SNUM(conn)));
//Jesse        DEBUG(5,("create: %s ", ctime(&create_time)));
      }
//Jesse      DEBUG(5,("access: %s ", ctime(&sbuf.st_atime)));
//Jesse      DEBUG(5,("write: %s ", ctime(&sbuf.st_mtime)));
//Jesse      DEBUG(5,("change: %s ", ctime(&sbuf.st_mtime)));
//Jesse      DEBUG(5,("mode: %x\n", mode));
...
//      CVAL(pdata,21) = (mode&aDIR)?1:0;
  ...
  send_trans2_replies( outbuf, bufsize, params, 2, *ppdata, data_size, ProcSockID);
...
//Jesse      DEBUG(3,("stat of %s failed (%s)\n", fname, strerror(errno)));
      if((errno == ENOENT) && bad_path)
      {
        unix_ERR_class = ERRDOS;
        unix_ERR_code = ERRbadpath;
...
//Jesse  DEBUG(3,("call_trans2setfilepathinfo(%d) %s info_level=%d totdata=%d\n",
//Jesse	   tran_call,fname,info_level,total_data));
...
//Jesse      DEBUG(10,("call_trans2setfilepathinfo: Set file allocation info for file %s to %.0f\n", fname, (double)newsize ));
      if(newsize == 0)
        size = 0;
      break;
    }
    ...
    case SMB_SET_FILE_END_OF_FILE_INFO:
    {
      ...
      if (IVAL(pdata,4) != 0)	/* more than 32 bits? */
         return(ERROR(ERRDOS,ERRunknownlevel));
#endif /* LARGE_SMB_OFF_T */
//Jesse      DEBUG(10,("call_trans2setfilepathinfo: Set end of file info for file %s to %.0f\n", fname, (double)size ));
      break;
    }
...
//Jesse          DEBUG(10, ("call_trans2setfilepathinfo: %s delete on close flag for fnum = %d, directory %s\n",
//Jesse                delete_on_close ? "Added" : "Removed", fsp->fnum, fsp->fsp_name ));

        }
	else if(fsp->stat_open)
	{
//Jesse          DEBUG(10, ("call_trans2setfilepathinfo: %s delete on close flag for fnum = %d, stat open %s\n",
//Jesse                delete_on_close ? "Added" : "Removed", fsp->fnum, fsp->fsp_name ));
	}
        else
        {
          ...
          if(lp_share_modes(SNUM(conn)))
          {
            ...
            if(delete_on_close && !GET_DELETE_ON_CLOSE_FLAG(fsp->share_mode))
            {
              ...
              for(i = 0; i < num_share_modes; i++)
              {
                ...
                {
//Jesse                  DEBUG(5,("call_trans2setfilepathinfo: refusing to set delete on close flag for fnum = %d, \
//Jesse file %s as a share exists that was not opened with FILE_DELETE access.\n",
//Jesse                        fsp->fnum, fsp->fsp_name ));
                  /*
                   * Release the lock.
                   */
...
//Jesse              DEBUG(10,("call_trans2setfilepathinfo: %s delete on close flag for fnum = %d, file %s\n",
//Jesse                   delete_on_close ? "Adding" : "Removing", fsp->fnum, fsp->fsp_name ));
...
//Jesse                DEBUG(10,("call_trans2setfilepathinfo: Changing share mode for fnum %d, file %s \
//Jesse dev = %x, inode = %.0f from %x to %x\n", 
//Jesse                      iterate_fsp->fnum, iterate_fsp->fsp_name, (unsigned int)dev, 
...
{
//Jesse                  DEBUG(0,("call_trans2setfilepathinfo: failed to change delete on close for fnum %d, \
//Jesse dev = %x, inode = %.0f\n", iterate_fsp->fnum, (unsigned int)dev, (double)inode));
}
              }
...
//Jesse  DEBUG(6,("actime: %s " , ctime(&tvs.actime)));
//Jesse  DEBUG(6,("modtime: %s ", ctime(&tvs.modtime)));
//Jesse  DEBUG(6,("size: %.0f ", (double)size));
//Jesse  DEBUG(6,("mode: %x\n"  , mode));
  ...
  if (st.st_mtime != tvs.modtime || st.st_atime != tvs.actime) {
    if(fsp != NULL) {
       ...
       */
       ...
       if (tvs.modtime != (time_t)0 && tvs.modtime != (time_t)-1) {
//Jesse         DEBUG(10,("call_trans2setfilepathinfo: setting pending modtime to %s\n",
//Jesse            ctime(&tvs.modtime) ));
         fsp->pending_modtime = tvs.modtime;
       }

    } else {

//Jesse      DEBUG(10,("call_trans2setfilepathinfo: setting utimes to modified values.\n"));
...
//Jesse    DEBUG(10,("call_trans2setfilepathinfo: file %s : setting dos mode %x\n",
//Jesse          fname, mode ));

    if(file_chmod(conn, fname, mode, NULL)) {
//Jesse      DEBUG(2,("chmod of %s failed (%s)\n", fname, strerror(errno)));
      return(UNIXERROR(ERRDOS,ERRnoaccess));
    }
  }
...
//Jesse    DEBUG(10,("call_trans2setfilepathinfo: file %s : setting new size to %.0f\n",
//Jesse          fname, (double)size ));
...
//Jesse  DEBUG(3,("call_trans2mkdir : name = %s\n", directory));
  ...
  if(ret < 0)
    {
//Jesse      DEBUG(5,("call_trans2mkdir error (%s)\n", strerror(errno)));
      if((errno == ENOENT) && bad_path)
      {
        unix_ERR_class = ERRDOS;
        unix_ERR_code = ERRbadpath;
...
static int call_trans2findnotifyfirst(connection_struct *conn,
				      char *inbuf, char *outbuf, 
				      int length, int bufsize,
				      char **pparams, char **ppdata, int ProcSockID)
{
  static uint16 fnf_handle = 257;
  char *params = *pparams;
  uint16 info_level = SVAL(params,4);

//Jesse  DEBUG(3,("call_trans2findnotifyfirst - info_level %d\n", info_level));
  ...
  send_trans2_replies(outbuf, bufsize, params, 6, *ppdata, 0,ProcSockID);
...
//Jesse  DEBUG(3,("call_trans2findnotifynext\n"));
  ...
  send_trans2_replies(outbuf, bufsize, params, 4, *ppdata, 0,ProcSockID);
...
//Jesse	DEBUG(3,("reply_findclose, dptr_num = %d\n", dptr_num));

	dptr_close(&dptr_num);

	outsize = set_message(outbuf,0,0,True);

//Jesse	DEBUG(3,("SMBfindclose dptr_num = %d\n", dptr_num));
#endif //0
...
//Jesse	DEBUG(3,("reply_findnclose, dptr_num = %d\n", dptr_num));
...
//Jesse	DEBUG(3,("SMB_findnclose dptr_num = %d\n", dptr_num));
...
int reply_transs2(connection_struct *conn,
		  char *inbuf,char *outbuf,int length,int bufsize)
{
//Jesse	DEBUG(4,("Ignoring transs2 of length %d\n",length));
	return(-1);
}
...
int reply_trans2(connection_struct *conn,
		 char *inbuf,char *outbuf,int length,int bufsize, int ProcSockID, int threadid)
{
	int outsize = 0;
...
//Jesse		DEBUG(2,("reply_trans2: queueing message trans2open due to being "));
//Jesse		DEBUGADD(2,( "in oplock break state.\n"));

//Jesse		push_oplock_pending_smb_message(inbuf, length);
		return -1;
	}
	...
	if (suwcnt != 1) {
//Jesse		DEBUG(2,("Invalid smb_sucnt in trans2 call\n"));
...
//Jesse		DEBUG(2,("Out of memory in reply_trans2\n"));
        if(params)
          free(params);
        if(data)
          free(data); 
...
//Jesse	if (num_params > total_params || num_data > total_data)
//Jesse		exit_server("invalid params in reply_trans2");
	...
	if(num_data_sofar < total_data || num_params_sofar < total_params)  {
		...
		while (num_data_sofar < total_data || 
			...
			if ((ret && 
				...
				outsize = set_message(outbuf,0,0,True);
//Jesse				if(ret)
//Jesse					DEBUG(0,("reply_trans2: Invalid secondary trans2 packet\n"));
//Jesse				else
//Jesse					DEBUG(0,("reply_trans2: %s in getting secondary trans2 response.\n",
//Jesse						 (smb_read_error == READ_ERROR) ? "error" : "timeout" ));
				if(params)
					free(params);
				if(data)
					free(data);
...
//Jesse			if (num_params_sofar > total_params || num_data_sofar > total_data)
//Jesse				exit_server("data overflow in trans2");
...
//Jesse	if (Protocol >= PROTOCOL_NT1) {
	if (Protocol[0] >= PROTOCOL_NT1) {
	...
	case TRANSACT2_QFILEINFO:
		outsize = call_trans2qfilepathinfo(conn, inbuf, outbuf, 
						   length, bufsize, 
						   &params, &data, total_data, ProcSockID);
		break;
	...
	default:
		/* Error in request */
//Jesse		DEBUG(2,("Unknown request %d in trans2 call\n", tran_call));
		if(params)
			free(params);
		if(data)
			free(data);
