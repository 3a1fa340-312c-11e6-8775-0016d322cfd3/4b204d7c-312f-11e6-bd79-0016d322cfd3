...
_func_enter_;
	...
	{		
		...
		memset(ptmpchar, 0, maxlen + FWBUFF_ALIGN_SZ);	//eason 20100407	
		...
		RT_TRACE(_module_hal_init_c_, _drv_notice_,("\n WT SRAM ; txpktsize = 0x%x  fwhdr.img_SRAM_size=0x%x", emem_sz,fwhdr.img_SRAM_size));	
		...
		RT_TRACE(_module_hal_init_c_, _drv_notice_,("\n WT SRAM ; txpktsize = %x", ptx_desc->txdw0));	
		...
		i = 5;
		tmp16=read16(padapter,TCR);
			RT_TRACE(_module_hal_init_c_, _drv_notice_,("TCR val = %x ", tmp16));
		...
		RT_TRACE(_module_hal_init_c_, _drv_notice_,("WT SYS_CLKR to 0x%x(ori=0x%x)", (u32)(tmp8|BIT(2)),tmp8) );
		...
		RT_TRACE(_module_hal_init_c_, _drv_notice_,("WT SYS_FUNC_EN+1 to 0x%x[ori=0x%x]",(u32)(tmp8|BIT(2)),tmp8));
...
uint	 rtl8712_hal_init(_adapter *padapter) 
{
	...
	RT_TRACE(_module_hal_init_c_,  _drv_alert_, ("\nrtl8712_hal_init #1 == SYS_FUNC_EN:0x%08x\n",read8(padapter,0x10250003)));
	...
	if(val8==_FAIL){	
        ...
        RT_TRACE(_module_hal_init_c_,  _drv_alert_, ("\nrtl8712_hal_init #2 == SYS_FUNC_EN:0x%08x\n",read8(padapter,0x10250003)));
...
//eason 20100210	printk("2 TCR=0x%x \n",  read32(padapter, TCR));
#endif	
...
//	write8(padapter, 0x1025003a, 0x72);
	...
	RT_TRACE(_module_hal_init_c_,_drv_debug_,("MAC Address = %x-%x-%x-%x-%x-%x\n", 
				 peeprompriv->mac_addr[0],	peeprompriv->mac_addr[1],
				 peeprompriv->mac_addr[2],	peeprompriv->mac_addr[3],
				 peeprompriv->mac_addr[4],	peeprompriv->mac_addr[5]));				 
