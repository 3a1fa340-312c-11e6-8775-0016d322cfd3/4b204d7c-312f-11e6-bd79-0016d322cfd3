...
//eason 20100210------------------------------
#define WPA_TASK_PRI         	20
#define WPA_TASK_STACK_SIZE  	16384//8192
static	uint8			WPA_Stack[WPA_TASK_STACK_SIZE];
static  cyg_thread		WPA_Task;
static  cyg_handle_t	WPA_TaskHdl;
	//MLME Thread initiation information	//eason
#define CMD_TASK_PRI         	20
#define CMD_TASK_STACK_SIZE  	8192
static	uint8			CMD_Stack[CMD_TASK_STACK_SIZE];
static  cyg_thread		CMD_Task;
static  cyg_handle_t	CMD_TaskHdl;

_adapter *Global_padapter = NULL;
void wpa_task(cyg_addrword_t data);
...
int low_power = 0;
char initmac[6] = {0};  // temp mac address if users want to use instead of the mac address in Efuse
...
//eason 20100210 
	...
	{
		case 3: 
			wireless_mode = WIRELESS_11BGN;
			if( mvBSSType == 2 )
				wireless_mode = WIRELESS_11BG;
			break;
		case 0:	
			wireless_mode = WIRELESS_11BG;
			break;
	...
	if(mvShortPreamble == 0)
		preamble = 1;
	else if(mvShortPreamble == 1)
		preamble = 3;
	else	
		preamble = 2;
	...
	//eason 20100210 registry_par->channel = (u8)channel;
	channel = mvChannel;
...
//eason 20100210	registry_par->frag_thresh=(u16)frag_thresh;
	registry_par->frag_thresh=(u16)mvFragmentation;
	...
	memcpy(initmac, mvEDDMAC, 6);
	memcpy(pnetdev->dev_addr, mvEDDMAC, 6);
...
struct net_device *init_netdev(void)	
{
 ...
 //eason 20100210 	pnetdev = alloc_etherdev(sizeof(_adapter));	
	pnetdev = malloc(sizeof(struct net_device));
	if (!pnetdev)
	   return NULL;
	
	memset(pnetdev, 0, sizeof (struct net_device));
	pnetdev->priv = (_adapter *) aligned_alloc (sizeof (_adapter), 16);
	//SET_MODULE_OWNER(pnetdev);
 //eason 20100210      ether_setup(pnetdev);
	
	//eason 20100210 padapter = netdev_priv(pnetdev);
	padapter = pnetdev->priv;	//eason 20100210
	if (!padapter)
	   return NULL;
	   
	memset(padapter, 0, sizeof (_adapter));
...
//	printk("register rtl8712_netdev_ops to netdev_ops\n");
	...
	//eason 20100210 netif_carrier_off(pnetdev);
	//netif_stop_queue(pnetdev);
	//eason 20100520	padapter->pid = 0;  // Initial the PID value used for HW PBC.
...
u32 start_drv_threads(_adapter *padapter)
{
	...
	cyg_thread_create(CMD_TASK_PRI,
					cmd_thread,
					0,
					"CMD",
					(void *) (CMD_Stack),
...
u8 init_default_value(_adapter *padapter)
{
	...
	pregistrypriv->rts_thresh = mvRTSThreshold;	//eason 20100210 
	//xmit_priv
...
*/
	...
	//eason 20100520	InitSwLeds(padapter);
exit:
...
u8 free_drv_sw(_adapter *padapter)
{
	...
	//eason 20100520	DeInitSwLeds(padapter);	 
...
//				efuse_pg_packet_read(padapter, offset, &pdata[i]);
				efuse_pg_packet_write(padapter, offset, 8, &pdata[offset*8]);	

//				efuse_pg_packet_write(padapter, 0x51, 1, data);	
//				efuse_one_byte_write(padapter, addr, WRegion);
//				efuse_one_byte_rw(padapter, _FALSE, addr, data);
			}
	    ...
	    {	//eason 2010730 EMI task	
         ...
         {	
	        //eason 20100407 wpa task	
	        cyg_thread_create(WPA_TASK_PRI,
						wpa_task,
						0,
						"WPA_Task",
						(void *) (WPA_Stack),
...
//eason 20100210	netif_carrier_off(pnetdev);	
//eason 20100210	netif_stop_queue(pnetdev);
...
static int netdev_close(struct net_device *pnetdev)
{
	...
	// Close LED
//eason 20100520        padapter->ledpriv.LedControlHandler(padapter, LED_CTL_POWER_OFF);
        msleep( 200 );
/*	if(!padapter->hw_init_completed)
	...
	{
//eason 20100210		printk("(2)871x_drv - drv_close, bup=%d, hw_init_completed=%d\n", padapter->bup, padapter->hw_init_completed);
	...
	RT_TRACE(_module_os_intfs_c_,_drv_info_,("-871x_drv - drv_close\n"));
//eason 20100210	printk("-871x_drv - drv_close, bup=%d\n", padapter->bup);
