...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,26))
		//eason 20100520   kill_proc(padapter->pid, SIGUSR1, 1);
#endif
...
//eason 20100210 thread_return cmd_thread(thread_context context)
thread_return cmd_thread(cyg_addrword_t data)
{
	struct cmd_obj *pcmd;
	unsigned int cmdsz, wr_sz, *pcmdbuf, *prspbuf;
	struct tx_desc *pdesc;
	void (*pcmd_callback)(_adapter *dev, struct cmd_obj	*pcmd);
    //eason 20100210   _adapter *padapter = (_adapter *)context;
...
_func_enter_;
	...
	memset(pcmd_parm, 0, sizeof(struct reg_rw_parm));	//eason 20100407
...
{	
	...
	{	
		...
		if(peventbuf)
		{
			...
			{
//eason 20100210				printk("recv_event_bh():bDriverStopped(%d) OR bSurpriseRemoved(%d)", padapter->bDriverStopped, padapter->bSurpriseRemoved);
			}
