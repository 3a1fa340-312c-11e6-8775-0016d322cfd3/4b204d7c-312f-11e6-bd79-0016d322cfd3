...
#include <circ_buf.h>
//eason 20100210------------------------------------
#define RT8712BulkRx_TASK_PRI         	20
#define RT8712BulkRx_TASK_STACK_SIZE  	8192
static	uint8			RT8712BulkRx_Stack[RT8712BulkRx_TASK_STACK_SIZE];
static  cyg_thread		RT8712BulkRx_Task;
static  cyg_handle_t	RT8712BulkRx_TaskHdl;

cyg_sem_t RT8712BulkRxThread;
//eason 20100210------------------------------------
...
int	init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{
	...
	if(precvpriv->recvbuf_drop == NULL)		//eason 20100407
		res = _FAIL;
	memset(precvpriv->recvbuf_drop, 0, sizeof(struct recv_buf));	//eason 20100407
	...
	((struct recv_buf *)precvpriv->recvbuf_drop)->pallocated_buf = _malloc(MAX_RECVBUF_SZ+4);
		...
		memset(((struct recv_buf *)precvpriv->recvbuf_drop)->pallocated_buf, 0, MAX_RECVBUF_SZ+4);	//eason 20100407
...
//#if defined (CONFIG_USB_HCI) && defined(PLATFORM_LINUX)
...
#endif
#else
	  cyg_semaphore_init(&RT8712BulkRxThread,0);
	  
	  cyg_thread_create(RT8712BulkRx_TASK_PRI,
					recv_tasklet,
					0,
					"RT8712BulkRx",
					(void *) (RT8712BulkRx_Stack),
					RT8712BulkRx_TASK_STACK_SIZE,
					&RT8712BulkRx_TaskHdl,
					&RT8712BulkRx_Task);
				
		//Start RT8712BulkRx Thread
	cyg_thread_resume(RT8712BulkRx_TaskHdl);
...
#ifdef CONFIG_PREALLOC_RECV_SKB
	{
		...
		for(i=0; i<NR_PREALLOC_RECV_SKB; i++)
		{
			...
			if(pskb)
			{
			//eason 20100210	pskb->dev = padapter->pnetdev;
...
int recvbuf2recvframe(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	do{		
		...
		//eason 20100210  pkt_copy->dev = padapter->pnetdev;
	#else			
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	for(i=0; i<nr_subframes; i++){
		...
		if (sub_skb->len >= 8 &&
			((__memcmp(sub_skb->data, rfc1042_header, SNAP_SIZE) &&
			  eth_type != ETH_P_AARP && eth_type != ETH_P_IPX) ||
			 __memcmp(sub_skb->data, bridge_tunnel_header, SNAP_SIZE) )) {
			/* remove RFC1042 or Bridge-Tunnel encapsulation and replace EtherType */
//eason 20100520			skb_pull(sub_skb, SNAP_SIZE);
//eason 20100520			_memcpy(skb_push(sub_skb, ETH_ALEN), pattrib->src, ETH_ALEN);
//eason 20100520			_memcpy(skb_push(sub_skb, ETH_ALEN), pattrib->dst, ETH_ALEN);
		} else {
			u16 len;
			/* Leave Ethernet header part of hdr and full payload */
			len = htons(sub_skb->len);
//eason 20100520			_memcpy(skb_push(sub_skb, 2), &len, 2);
//eason 20100520			_memcpy(skb_push(sub_skb, ETH_ALEN), pattrib->src, ETH_ALEN);
//eason 20100520			_memcpy(skb_push(sub_skb, ETH_ALEN), pattrib->dst, ETH_ALEN);
		}
		...
		if (sub_skb) {
			...
			//eason 20100520	sub_skb->protocol = eth_type_trans(sub_skb, padapter->pnetdev);
			//eason 20100520	sub_skb->dev = padapter->pnetdev;

#ifdef CONFIG_RTL8712_TCP_CSUM_OFFLOAD_RX
			if ( (pattrib->tcpchk_valid == 1) && (pattrib->tcp_chkrpt == 1) ) {
			//eason 20100520	sub_skb->ip_summed = CHECKSUM_UNNECESSARY;
			} else {
			//eason 20100520	sub_skb->ip_summed = CHECKSUM_NONE;
			}
#else /* !CONFIG_RTL8712_TCP_CSUM_OFFLOAD_RX */
			sub_skb->ip_summed = CHECKSUM_NONE;
#endif

			//eason 20100520	netif_rx(sub_skb);
			WLanRecv(sub_skb->data, sub_skb->len);
		}
	}
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	do{
		...
		{
		 	if ( __memcmp(psnap->oui, oui_rfc1042, WLAN_IEEE_OUI_LEN))
			{
   				bsnaphdr=_TRUE;//wlan_pkt_format = WLAN_PKT_FORMAT_SNAP_RFC1042;
  			}
  			else if (__memcmp(psnap->oui, SNAP_HDR_APPLETALK_DDP, WLAN_IEEE_OUI_LEN) &&
     					__memcmp(psnap_type, SNAP_ETH_TYPE_APPLETALK_DDP, 2) )
     			{  					
    				bsnaphdr=_TRUE;	//wlan_pkt_format = WLAN_PKT_FORMAT_APPLETALK;
  			}	
  			else if (__memcmp( psnap->oui, oui_8021h, WLAN_IEEE_OUI_LEN))
  			{
...
//#ifdef CONFIG_SKB_COPY	
	...
	if(a_len > ETH_HLEN) 
	{
		...
		{						
			...
			if(pskb_copy==NULL)
			{			
//eason 20100210				printk("amsdu_to_msdu:can not all(ocate memory for skb copy\n");
			}		
		...
		{
//eason 20100210			printk("amsdu_to_msdu:can not allocate memory for pnrframe_new\n");
		}
...
//	u8	*prspbuf;
	...
	do{
	...
	
		{
			...
			memset(pc2h, 0, sizeof(struct evt_obj));	//eason 20100407			
			_init_listhead(&pc2h->list);

			if(cmd_len<64)
			{
				pevtcmd = (u8*)_malloc(64+8);
				if(pevtcmd == NULL)		//eason 20100407
					return NULL;
				memset(pevtcmd, 0, cmd_len+8);	//eason 20100407
			}	
			else
			{				
...
static int recvbuf2recvframe_u(_adapter *padapter, struct sk_buff *pskb)
{
	...
	//	pkt_copy = dev_alloc_skb(pkt_offset>1600?pkt_offset:1600);		
		...
		pkt_copy = dev_alloc_skb_malloc(alloc_sz);	
#else			
		//pkt_copy = netdev_alloc_skb(padapter->pnetdev, pkt_offset>1600?pkt_offset:1600);
		...
		if(pkt_copy)
		{
			//eason 20100520	pkt_copy->dev = padapter->pnetdev;
			precvframe->u.hdr.pkt = pkt_copy;	
...
//eason 20100210 void recv_tasklet(void *priv)
void recv_tasklet(cyg_addrword_t data)
{
	struct sk_buff *pskb;
	//eason 20100210 _adapter *padapter = (_adapter*)priv;
	_adapter *padapter = (_adapter*)Global_padapter;
	struct recv_priv *precvpriv = &padapter->recvpriv;
	
	while(1)//eason 20100210
	{
		cyg_semaphore_wait(&RT8712BulkRxThread); 	
		...
		//eason 20100210	pskb->tail = pskb->data;
			pskb->len = 0;
...
{	
	...
	while(1)
	{
		...
		{
//eason 20100210			printk("recv_bh => bDriverStopped or bSurpriseRemoved \n");
			break;
		}
