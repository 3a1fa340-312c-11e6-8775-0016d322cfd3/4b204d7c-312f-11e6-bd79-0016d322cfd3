...
#else
static char *translate_scan(_adapter *padapter, struct wlan_network *pnetwork,
				char *start, char *stop  )
#endif	//eason 20100407				
{
	struct iw_event iwe;
	...
	//eason 20100407	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_ADDR_LEN);
	...
	//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe, pnetwork->network.Ssid.Ssid);
	...
	//eason 20100407	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_CHAR_LEN);
	...
	//eason 20100407	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_UINT_LEN);
	...
	//eason 20100407start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_FREQ_LEN);
...
//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe, pnetwork->network.Ssid.Ssid);
	...
	if(ht_cap == _TRUE)
	{
		...
		{
		//eason 20100407	printk("wx_get_scan, mcs_rate_bitmap=0x%x\n", mcs_rate);
			max_rate = (bw_40MHz) ? ((short_GI)?150:135):((short_GI)?72:65);
		}
	...
	//eason 20100407	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_PARAM_LEN);
         ...
         //parsing WPA/WPA2 IE
	{
		...
		if(wpa_len>0){
			...
			//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe,buf);
			...
			//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe, wpa_ie);
			start = iwe_stream_add_point(start, stop, &iwe, wpa_ie);
			//
		}
		if(rsn_len>0){
			p = buf;
			...
			//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe, rsn_ie);
			start = iwe_stream_add_point(start, stop, &iwe, rsn_ie);
			//
		}
	}
	 ...
	 {//parsing WPS IE
		...
		if(get_wps_ie(pnetwork->network.IEs, pnetwork->network.IELength, wps_ie, &wps_ielen)==_TRUE)
		{
			if(wps_ielen>2)
			{
				...
				//eason 20100407	start = iwe_stream_add_point(info, start, stop, &iwe, wps_ie);
				start = iwe_stream_add_point(start, stop, &iwe, wps_ie);
...
/*	
	...
	//eason 20100407	start = iwe_stream_add_event(info, start, stop, &iwe, IW_EV_QUAL_LEN);
...
#else
int wpa_set_auth_algs(u32 value)
#endif //eason 20100407
{	
	//eason 20100407	_adapter *padapter = netdev_priv(dev);
	...
	{
//eason 20100210		printk("wpa_set_auth_algs, AUTH_ALG_OPEN_SYSTEM\n");
...
#else
static int wpa_set_encryption(struct ieee_param *param, u32 param_len)
#endif //eason 20100407
{
	int ret = 0;
...
//eason 20100210		printk("wpa_set_encryption, crypt.alg = WEP\n");
...
//eason 20100210		printk("(1)wep_key_idx=%d\n", wep_key_idx);
		...
		{
//eason 20100210			printk("wep, set_tx=1\n");
		...
		{
//eason 20100210			printk("wep, set_tx=0\n");
...
static int r871x_set_wpa_ie(_adapter *padapter, char *pie, unsigned short ielen)
{
	...
	{		
		...
		memset(buf, 0, ielen);	//eason 20100407
...
/*//eason 20100210		{
		...
		{//set wps_ie	
			...
			while( cnt < ielen )
			{
				...
				if((eid==_VENDOR_SPECIFIC_IE_)&&(__memcmp(&buf[cnt+2], wps_oui, 4)==_TRUE))
				{


//eason 20100210					printk("SET WPS_IE\n");
...
//eason 20100210					printk("SET WPS_IE, wps_phase==_TRUE\n");
	...
	RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("r871x_set_wpa_ie: pairwise_cipher=%d padapter->securitypriv.ndisencryptstatus =%d padapter->securitypriv.ndisauthtype=%d\n",pairwise_cipher,padapter->securitypriv.ndisencryptstatus,padapter->securitypriv.ndisauthtype));
...
static int r8711_wx_set_freq(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_set_freq(struct iw_freq *wrqu)
#endif //eason 20100520			     
{
//eason 20100520	_adapter *padapter = (_adapter *)netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
//eason 20100520	struct iw_freq *fwrq = & wrqu->freq;
	struct iw_freq *fwrq = wrqu;
	int rc = 0;
...
#else 
int r8711_wx_set_mode( uint32 mode)
#endif //eason 20100210			     
{
//eason 20100210	_adapter *padapter = (_adapter *)netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
	...
	//eason 20100210 switch(wrqu->mode)
	switch(mode)
	{
		case IW_MODE_AUTO://0
...
//eason 20100210			printk("set_mode = IW_MODE_MASTER\n");
			break;				
...
static int r871x_wx_set_pmkid(struct net_device *dev,
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r871x_wx_set_pmkid(char *extra)
#endif //eason 20100407			     
{

	//eason 20100407	_adapter    *padapter = (_adapter *)netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
...
/*
	...
	_memcpy( strIssueBssid, pPMK->bssid.sa_data, ETH_ALEN);
        ...
        {
//eason 20100210                printk( "[r871x_wx_set_pmkid] IW_PMKSA_ADD!\n" );
                if ( __memcmp( strIssueBssid, strZeroMacAddress, ETH_ALEN ) == _TRUE )
                {
                    return( intReturn );
                }
                else
		...
		{
			if( __memcmp( psecuritypriv->PMKIDList[j].Bssid, strIssueBssid, ETH_ALEN) ==_TRUE )
			{ // BSSID is matched, the same AP => rewrite with new PMKID.
                                
//eason 20100210                                printk( "[r871x_wx_set_pmkid] BSSID exists in the PMKList.\n" );
	        ...
	        if(!blInserted)
                {
		    // Find a new entry
//eason 20100210                    printk( "[r871x_wx_set_pmkid] Use the new entry index = %d for this PMKID.\n",
//eason 20100210                            psecuritypriv->PMKIDIndex );
        ...
        {
//eason 20100210                printk( "[r871x_wx_set_pmkid] IW_PMKSA_REMOVE!\n" );
                intReturn = _TRUE;
		for(j=0 ; j<NUM_PMKID_CACHE; j++)
		{
			if( __memcmp( psecuritypriv->PMKIDList[j].Bssid, strIssueBssid, ETH_ALEN) ==_TRUE )
        ...
        {
//eason 20100210            printk( "[r871x_wx_set_pmkid] IW_PMKSA_FLUSH!\n" );
...
static int r8711_wx_get_range(struct net_device *dev, 
				...
				union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_get_range(char *extra)
#endif	//eason 20100407				
{
...
//eason 20100407	wrqu->data.length = sizeof(*range);
				...
				char *extra)
{
	...
	RT_TRACE(_module_rtl871x_ioctl_os_c, _drv_notice_,
		 ("r871x_wx_set_priv: (SIOCSIWPRIV) %s req=%s\n",
		  dev->name, ext));
#if 0
//#define MAX_WX_STRING 80
//	snprintf(ext, MAX_WX_STRING, "OK");
	snprintf(ext, 80, "OK");
...
static int r8711_wx_set_wap(struct net_device *dev,
			 ...
			 char *extra)
#else
int r8711_wx_set_wap(struct sockaddr1 *awrq)
#endif	//eason 20100407			 
{
	int ret = 0;
	//eason 20100407	_adapter *padapter = netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
	struct sockaddr1 *temp = (struct sockaddr *)awrq;
	struct	mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	_irqL irqL;
	_list	*phead;
	...
	while (1)
	 {
		...
		if (__memcmp(dst_bssid, temp->sa_data, ETH_ALEN) == _TRUE)
		{
...
static int r8711_wx_get_wap(struct net_device *dev, 
			    ...
			    union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_get_wap(struct sockaddr1 *ap_addr)
#endif	//eason 20100407			    
{
	...
	//eason 20100407	wrqu->ap_addr.sa_family = ARPHRD_ETHER;
	ap_addr->sa_family = ARPHRD_ETHER;
	//eason 20100407	_memset(wrqu->ap_addr.sa_data, 0, ETH_ALEN);
	_memset(ap_addr->sa_data, 0, ETH_ALEN);
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE|WIFI_AP_STATE) == _TRUE)
	{
		...
		//eason 20100407	_memcpy(wrqu->ap_addr.sa_data, pcur_bss->MacAddress, ETH_ALEN);
		_memcpy(ap_addr->sa_data, pcur_bss->MacAddress, ETH_ALEN);
...
int r8711_wx_set_scan(struct net_device *dev, struct iw_request_info *a,
			     union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_set_scan(struct iw_point *wrqu ,char *extra)
#endif //eason 20100210			     
{
	u8 status = _TRUE;
	int ret = 0;	
...
//eason 20100210           printk("bDriverStopped=%d\n", padapter->bDriverStopped);
		ret= -1;
		goto exit;
	}
...
#if WIRELESS_EXT >= 17
	//eason 20100520	if (wrqu->data.length == sizeof(struct iw_scan_req))
	if (wrqu->length == sizeof(struct iw_scan_req))
	{
		struct iw_scan_req *req = (struct iw_scan_req *)extra;

		//eason 20100520	if (wrqu->data.flags & IW_SCAN_THIS_ESSID)
		if (wrqu->flags & IW_SCAN_THIS_ESSID)
		{
...
static int r8711_wx_get_scan(struct net_device *dev, struct iw_request_info *a,
			     union iwreq_data *wrqu, char *extra)
#else		
int r8711_wx_get_scan(struct iw_point *data, char *extra)	     
#endif //eason 20100407			     
{
	_irqL	irqL;
	_list					*plist, *phead;
	...
	struct	wlan_network	*pnetwork = NULL;
	char *ev = extra;
	//eason 20100407	char *stop = ev + wrqu->data.length;
	...
	while(1)
	{
		...
		//eason 20100407	ev=translate_scan(padapter, a, pnetwork, ev, stop);
		ev=translate_scan(padapter, pnetwork, ev, stop);
...
/*eason 20100407       wrqu->data.length = ev-extra;
	wrqu->data.flags = 0;*/
	data->length = ev-extra;
	data->flags = 0;
...
static int r8711_wx_set_essid(struct net_device *dev, 
			      ...
			      union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_set_essid( UCHAR SsidLen, char *extra)
#endif //eason 20100210			      
{
	uint ret=0, len;
	...
	RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("r871x_wx_set_essid\n"));	
	...
	//eason 20100210 if (wrqu->essid.length > IW_ESSID_MAX_SIZE){
	if (SsidLen > IW_ESSID_MAX_SIZE){
		ret= -E2BIG;
		goto exit;
	}
	...
	//eason 20100210 if (wrqu->essid.flags && wrqu->essid.length)
	if (SsidLen)
	{
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,19))
		//eason 20100210 len = ((wrqu->essid.length-1) < IW_ESSID_MAX_SIZE) ? (wrqu->essid.length-1) : IW_ESSID_MAX_SIZE;
		len = ((SsidLen) < IW_ESSID_MAX_SIZE) ? (SsidLen) : IW_ESSID_MAX_SIZE;
#else
		...
		RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("r871x_wx_set_ssid():ssid=%s\n", ndis_ssid.Ssid));
...
#endif			
//eason 20100210				printk("scan_q is empty, set ssid to check if scanning again!\n");
			...
			RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("r871x_wx_set_ssid(): dst_ssid=%s\n", pnetwork->network.Ssid.Ssid));

			//src_ssid = ndis_ssid.Ssid;

			if (((__memcmp(dst_ssid, src_ssid, ndis_ssid.SsidLength)) == _TRUE)&&
				(pnetwork->network.Ssid.SsidLength==ndis_ssid.SsidLength))
			{
				RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("set ssid: 802_11_infra. mode\n"));
#if 0	//eason 20100713				
				if (set_802_11_infrastructure_mode(padapter, pnetwork->network.InfrastructureMode) == _FALSE)
				{
					ret = -1;
...
#endif 	//eason 20100713	
				break;			
			}

		}

		RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("set ssid: 802_11_auth. mode\n"));
		set_802_11_authentication_mode(padapter, authmode);
...
static int r8711_wx_get_essid(struct net_device *dev, 
			      ...
			      union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_get_essid(struct iw_point *wrqu, char *extra)
#endif //eason 20100407			      
{
	u32 len,ret = 0;
	//eason 20100407	_adapter *padapter = (_adapter *)netdev_priv(dev);
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)
	{
		...
		wrqu->length = len;//eason 20100407	wrqu->essid.length = len;
			
		_memcpy(extra, pcur_bss->Ssid.Ssid, len);

		wrqu->flags = 1;//eason 20100407	wrqu->essid.flags = 1;
	}
	else
	{
		ret = -1;		
...
#if 0 //eason 200100210
static int r8711_wx_set_rate(struct net_device *dev, 
			      struct iw_request_info *a,
			      union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_set_rate(u32 target_rate, u32 fixed)
#endif //eason 20100210			      
{
	int	i, ret = 0;
	//eason 20100210 _adapter *padapter = (_adapter *)netdev_priv(dev);
	...
	//eason 20100210 u32	target_rate = wrqu->bitrate.value;
	//eason 20100210 u32	fixed = wrqu->bitrate.fixed;
	u32	ratevalue = 0;
	...
	//eason 20100210 target_rate = target_rate/100000;
...
static int r8711_wx_get_rate(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r8711_wx_get_rate(struct iw_param *bitrate)
#endif	//eason 20100407			     
{	
	int i;
	u8 *p;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)
	{
		...
		while( (pcur_bss->SupportedRates[i]!=0) && (pcur_bss->SupportedRates[i]!=0xFF))
		{
			...
			//eason 20100407	wrqu->bitrate.fixed = 0;	/* no auto select */
			bitrate->fixed = 0;
			//wrqu->bitrate.disabled = 1/;
			//eason 20100407	wrqu->bitrate.value = rate*500000;
		...
		if(ht_cap == _TRUE)
		{
			...
			max_rate *= 2; // Mbps/2		
			//eason 20100407	wrqu->bitrate.value = max_rate*500000;
...
#else
int r8711_wx_set_enc(struct iw_point *wrqu, char *keybuf)
#endif //eason 20100210			    
{	
	u32 key, ret = 0;
...
//eason 20100210	struct iw_point *erq = &(wrqu->encoding);
	struct iw_point *erq = (struct iw_point *)wrqu;
//eason 20100210	_adapter *padapter = netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
//eason 20100210	printk("r8711_wx_set_enc, flags=0x%x\n", erq->flags);
	...
	{
//eason 20100210		printk("EncryptionDisabled\n");
		padapter->securitypriv.ndisencryptstatus = Ndis802_11EncryptionDisabled;
		padapter->securitypriv.dot11PrivacyAlgrthm=_NO_PRIVACY_;
		padapter->securitypriv.dot118021XGrpPrivacy=_NO_PRIVACY_;
...
//eason 20100210		printk("r871x_wx_set_enc, key=%d\n", key);
	}
	
	//set authentication mode	
	if(erq->flags & IW_ENCODE_OPEN)
	{
//eason 20100210		printk("r8711_wx_set_enc():IW_ENCODE_OPEN\n");
	...
	{		
//eason 20100210		printk("r8711_wx_set_enc():IW_ENCODE_RESTRICTED\n");
		padapter->securitypriv.ndisencryptstatus = Ndis802_11Encryption1Enabled;
#ifdef CONFIG_PLATFORM_MT53XX
	...
	{
//eason 20100210		printk("r8711_wx_set_enc():erq->flags=0x%x\n", erq->flags);
...
//eason 20100210			printk("(keyindex_provided == 1), keyid=%d, key_len=%d\n", key, padapter->securitypriv.dot11DefKeylen[key]);
...
static int r871x_wx_set_gen_ie(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r871x_wx_set_gen_ie(struct iw_point *data, char *extra)
#endif	//eason 20100407			     
{
	int ret;
	//eason 20100407	_adapter *padapter = netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
       ...
       ret = r871x_set_wpa_ie(padapter, extra, data->length);
...
static int r871x_wx_set_auth(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r871x_wx_set_auth( struct iw_param *extra)			     
#endif	//eason 20100407			     
{
	//eason 20100407	_adapter *padapter = netdev_priv(dev);
	_adapter *padapter = (_adapter *)Global_padapter;
//eason 20100407	struct iw_param *param = (struct iw_param*)&(wrqu->param);
	struct iw_param *param = (struct iw_param*)extra;
	int paramid;
	int paramval;
	int ret = 0;
	...
	switch (paramid)
	{
		...
		case IW_AUTH_80211_AUTH_ALG: // 6
			//eason 20100407	ret = wpa_set_auth_algs(dev, (u32)param->value);		
		ret = wpa_set_auth_algs((u32)param->value);	
			break;
...
static int r871x_wx_set_enc_ext(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
#else
int r871x_wx_set_enc_ext(struct iw_point *wrqu, char *extra)			     
#endif //eason 20100407			     
{
	char *alg_name;
	u32 param_len;
	struct ieee_param *param = NULL;
//eason 20100407	struct iw_point *pencoding = &wrqu->encoding;
	struct iw_point *pencoding = wrqu;
	...
	//eason 20100407	ret =  wpa_set_encryption(dev, param, param_len);	
	ret =  wpa_set_encryption(param, param_len);	
...
struct	iw_point
{
  ...
  __u16		flags;		/* Optional params */
	...
	memset(pparmbuf, 0, len);	//eason 20100407
	if(bset)//set info
	{
...
static  int r871x_mp_ioctl_hdl(struct net_device *dev, struct iw_request_info *info,
						union iwreq_data *wrqu, char *extra)
#else
int r871x_mp_ioctl_hdl( struct iw_point *data)
...
//eason 20100802	_adapter *padapter = netdev_priv(dev);
//eason 20100802	struct iw_point *p = &wrqu->data;
	_adapter *padapter = (_adapter *)Global_padapter;	
	struct iw_point *p = data;
...
#else
	memcpy(pparmbuf, p->pointer, len);	
#endif	//eason 20100802		
	poidparam = (struct mp_ioctl_param *)pparmbuf;	
	RT_TRACE(_module_rtl871x_ioctl_os_c,_drv_info_,("r871x_mp_ioctl_hdl: subcode [%d], len[%d], buffer_len[%d]\r\n",
        					 poidparam->subcode, poidparam->len, len));
...
//eason 20100210	printk("+r871x_mp_ioctl_hdl(%d)\n", poidparam->subcode);

//eason 20100802	phandler = mp_ioctl_hdl + poidparam->subcode;
	switch(poidparam->subcode)
	{
	    case 0: //MP_START
	        phandler->paramsize =	sizeof(unsigned int);
	        phandler->handler = oid_rt_pro_start_test_hdl;
	        phandler->oid = OID_RT_PRO_START_TEST;
			break;
		case 18: //CNTU_TX			
	      	phandler->paramsize = sizeof(unsigned int);
	        phandler->handler = oid_rt_pro_set_continuous_tx_hdl;
	        phandler->oid =	OID_RT_PRO_SET_CONTINUOUS_TX;
			break;
		case 21: //RESET_PHY_RX_PKT_CNT
	        phandler->paramsize = 0;
	        phandler->handler = oid_rt_reset_phy_rx_packet_count_hdl;
	        phandler->oid = OID_RT_RESET_PHY_RX_PACKET_COUNT;
			break;		
		case 22: //GET_PHY_RX_PKT_RECV
	        phandler->paramsize = sizeof(unsigned int);
	        phandler->handler = oid_rt_get_phy_rx_packet_received_hdl;
	        phandler->oid = OID_RT_GET_PHY_RX_PACKET_RECEIVED;
			break;		 			
	}
	...
	{
//eason 20100210		printk("r871x_mp_ioctl_hdl(): err!, subcode=%d, oid=%d, handler=%p\n", 
			//eason 20100210poidparam->subcode, phandler->oid, phandler->handler);
		ret = -EFAULT;
		goto _r871x_mp_ioctl_hdl_exit;
	}
...
//eason 20100210	printk("+r871x_get_aplist_info\n");
	...
	while(1)
	{
		...
		{			
//eason 20100210			printk("Invalid BSSID '%s'.\n", (u8*)pdata->pointer);
			return -EINVAL;
		}		
		
//eason 20100210		printk("BSSID:" MACSTR "\n", MAC2STR(bssid));

		if(__memcmp(bssid, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE)//BSSID match, then check if supporting wpa/wpa2
		{
...
static int r871x_wps_start(struct net_device *dev,
                               ...
                               union iwreq_data *wrqu, char *extra)
#else
int r871x_wps_start(struct iw_point *data)
#endif //eason 20100608                               
{	
	int ret = 0;	
//eason 20100608	_adapter *padapter = netdev_priv(dev);	
	_adapter *padapter = (_adapter *)Global_padapter;
	//eason 20100608	struct iw_point *pdata = &wrqu->data;
	...
	{                
       ...
       if ( u32wps_start == 0 )
       {
//eason 20100608	u32wps_start = *extra;
			u32wps_start = pdata->pointer;
       }
...
//       printk( "[%s] wps_start = %d\n", __FUNCTION__, u32wps_start );
       ...
       {
//eason 20100520           padapter->ledpriv.LedControlHandler(padapter, LED_CTL_START_WPS);
       }
	else if ( u32wps_start == 2 ) // WPS Stop because of wps success
	{
//eason 20100520           padapter->ledpriv.LedControlHandler(padapter, LED_CTL_STOP_WPS);
	}
	else if ( u32wps_start == 3 ) // WPS Stop because of wps fail
	{
//eason 20100520           padapter->ledpriv.LedControlHandler(padapter, LED_CTL_STOP_WPS_FAIL);
	}
exit:
...
//eason 20100407		ret = wpa_set_auth_algs(dev, value);
		ret = wpa_set_auth_algs(value);
		break;
			      ...
			      struct ieee_param *param, u32 plen)
{
	...
	{		
		...
		memset(buf, 0, param->u.wpa_ie.len);	//eason 20100407
		_memcpy(buf, param->u.wpa_ie.data, param->u.wpa_ie.len);

		//dump
		{
			int i;
//eason 20100210			printk("\n wpa_ie(length:%d):\n",param->u.wpa_ie.len);
			for(i=0;i<param->u.wpa_ie.len;i=i+8)
//eason 20100210				printk("0x%.2x 0x%.2x 0x%.2x 0x%.2x 0x%.2x 0x%.2x 0x%.2x 0x%.2x \n",buf[i],buf[i+1],buf[i+2],buf[i+3],buf[i+4],buf[i+5],buf[i+6],buf[i+7]);
		}
		...
		switch(pairwise_cipher)
		{
			...
			case WPA_CIPHER_TKIP:
				padapter->securitypriv.dot11PrivacyAlgrthm=_TKIP_;
				padapter->securitypriv.ndisencryptstatus = Ndis802_11Encryption2Enabled;
//eason 20100210				printk("(TKIP Pairwise)ndisencryptstatus=%d\n", padapter->securitypriv.ndisencryptstatus);
				break;
			case WPA_CIPHER_CCMP:
				padapter->securitypriv.dot11PrivacyAlgrthm=_AES_;
				padapter->securitypriv.ndisencryptstatus = Ndis802_11Encryption3Enabled;
//eason 20100210				printk("(AES Pairwise)ndisencryptstatus=%d\n",padapter->securitypriv.ndisencryptstatus);
				break;
			case WPA_CIPHER_WEP104:	
				padapter->securitypriv.dot11PrivacyAlgrthm=_WEP104_;
				padapter->securitypriv.ndisencryptstatus = Ndis802_11Encryption1Enabled;
...
int wpa_supplicant_ioctl(struct net_device *dev, struct iw_point *p)
{
	...
	memset(param, 0, p->length);	//eason 20100407
	...
	case IEEE_CMD_SET_ENCRYPTION:
		//eason 20100407	ret = wpa_set_encryption(dev, param, p->length);
		ret = wpa_set_encryption(param, p->length);
		break;
	...
	default:
//eason 20100210		printk("Unknown WPA supplicant request: %d\n", param->cmd);
		ret = -EOPNOTSUPP;
		break;
...
extern u32 Global_wps_state;	//eason 20100727	
extern u32 Global_wpa_state;	//eason 20100727	
int rtl871x_get_linkup()
{
	int ret = 0;
	_adapter *padapter = (_adapter *)Global_padapter;
	struct	mlme_priv	*pmlmepriv = &(padapter->mlmepriv);
	struct security_priv *psecuritypriv=&padapter->securitypriv;
	int state = psecuritypriv->dot118021XGrpPrivacy;
	
	if ((if_up(padapter)) == _FALSE)
	{
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)==_TRUE)
		{
			if(Global_wps_state  == 1)
				ret = 0;
			else	
				ret = 1;
		}else
		...
		if(Global_wps_state  == 1)
			ret = 0;
		else if(state == _TKIP_ || state == _AES_)	
		{
			if(Global_wpa_state == 1)
				ret = 1;
		}
		else 	
			ret = 1;
