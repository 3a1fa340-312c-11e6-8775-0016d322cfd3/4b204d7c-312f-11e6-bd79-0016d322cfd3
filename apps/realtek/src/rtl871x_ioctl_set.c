...
//ZOT			sitesurvey_cmd(padapter, &pmlmepriv->assoc_ssid);
	...
	{	
		...
		else	
		{
			...
			{ 
				...
				{
					if(__memcmp(pmlmepriv->cur_network.network.Ssid.Ssid, pmlmepriv->assoc_ssid.Ssid, pmlmepriv->assoc_ssid.SsidLength))
					{ 
...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)
	{
		...
		if (__memcmp(&pmlmepriv->cur_network.network.MacAddress, bssid, ETH_ALEN) == _TRUE)
		{		
...
_func_enter_;
	...
	RT_TRACE(_module_rtl871x_ioctl_set_c_,_drv_info_,("set_802_11_ssid:fwstate:%x\n", pmlmepriv->fw_state));
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)	
	{
		...
		if ((pmlmepriv->assoc_ssid.SsidLength == ssid->SsidLength) &&
		    (__memcmp(&pmlmepriv->assoc_ssid.Ssid, ssid->Ssid, ssid->SsidLength) == _TRUE))
		{		
...
_Abort_Set_SSID:
	...
	RT_TRACE(_module_rtl871x_ioctl_set_c_,_drv_info_,(" _Abort_Set_SSID"));
done:	
	_exit_critical(&pmlmepriv->lock, &irqL);
...
_func_enter_;
	    ...
	    (pmlmepriv->sitesurveyctrl.traffic_busy == _TRUE))	
	{
		...
		res = (u8)check_fwstate(pmlmepriv, _FW_UNDER_SURVEY);
		RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
		...
		if(Global_wps_state == 1)	//eason 20100608
			res = sitesurvey_cmd(padapter, NULL);
		else		//eason 20100608
		{
			if(scan_flag == 0)
			{
				res = sitesurvey_cmd(padapter,NULL );	//eason 20100608
				scan_flag = 1;
			}
			else
			{
			res = sitesurvey_cmd(padapter,&pmlmepriv->assoc_ssid );	//eason 20100608
				scan_flag = 0;
			}
		}
	}
