...
#include<rtl871x_byteorder.h> //eason 20100210
extern UINT    mvBeaconinterval;   //eason 20100210
extern void set_realtek_event(int event);	//eason 20100407
...
_func_enter_;
	...
	_memset(pbuf, 0, MAX_BSS_CNT * (sizeof(struct wlan_network)));	//eason 20100407
...
_func_enter_;	
	...
	if(__memcmp(zero_addr, addr, ETH_ALEN)){
		pnetwork=NULL;
		goto exit;
		}
	...
	while (plist != phead)
       {
                ...
                if (__memcmp(addr, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE)
                        break;
        }
...
_func_enter_;
	...
	pibss[1] = 0x11;
	pibss[2] = 0x87;
	pibss[3] = (u8)(((curtime) & 0xff) + mvEDDMAC[3] );//p[0];
	pibss[4] = (u8)(((curtime>>8) & 0xff) + mvEDDMAC[4] ) ;//p[1];
	pibss[5] = (u8)(((curtime>>16) & 0xff) + mvEDDMAC[5] ) ;//p[2];
_func_exit_;
	return;
}
...
_func_exit_;			
	...
	return ((src->Ssid.SsidLength == dst->Ssid.SsidLength) &&
			...
			( (__memcmp(src->MacAddress, dst->MacAddress, ETH_ALEN)) == _TRUE) &&
			( (__memcmp(src->Ssid.Ssid, dst->Ssid.Ssid, src->Ssid.SsidLength)) == _TRUE) &&
...
_func_enter_;		
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE)
	{
		...
		if(__memcmp(&(pmlmepriv->cur_network.network.MacAddress), pnetwork->MacAddress, ETH_ALEN))
		{
...
_func_enter_;			
	...
	RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("tgt_network->network.MacAddress = %x %x %x %x %x %x ssid = %s\n", 
...
*/
u32 Global_wpa_state;	//eason 20100727	
...
_func_enter_;	
	...
	set_realtek_event(2);	//eason 20100407
	Global_wpa_state = 0;
//eason 20100520	padapter->ledpriv.LedControlHandler(padapter, LED_CTL_LINK);
	...
	RT_TRACE(_module_rtl871x_mlme_c_,_drv_err_,("\n after indicate_connect, fw_state=%x\n\n", pmlmepriv->fw_state));
...
//eason 20100520	padapter->ledpriv.LedControlHandler(padapter, LED_CTL_NO_LINK);
	set_realtek_event(1);	//eason 20100407
...
_func_enter_;	
	...
	the_same_macaddr = __memcmp(pnetwork->network.MacAddress, cur_network->network.MacAddress, ETH_ALEN);
...
//ZOT 20100713			generate_random_ibss(pibss);
...
*/
u8 old_bssid[ETH_ALEN]={0};	//eason 20100720
extern u32 Global_wps_state;	//eason 20100608	
...
{	
	...
	_queue	*queue	= &(pmlmepriv->scanned_queue);
	struct	wlan_network	*pnetwork = NULL;
	struct	wlan_network	*pnetwork_source = &(pmlmepriv->cur_network);	//eason 20100713
...
//eason 20100715
		if(Global_wps_state == 0){
			if(!(pnetwork_source->network.InfrastructureMode == pnetwork->network.InfrastructureMode))
				continue ;
		}
		...
		if(pmlmepriv->assoc_by_bssid==_TRUE)
		{
			...
			if(__memcmp(dst_ssid, src_ssid, ETH_ALEN)==_TRUE)
			{
		...
		if (((__memcmp(dst_ssid, src_ssid, pmlmepriv->assoc_ssid.SsidLength)) == _TRUE)&&
			(pnetwork->network.Ssid.SsidLength==pmlmepriv->assoc_ssid.SsidLength))
		{
			...
			//eason 20100720
			if (memcmp(old_bssid, pnetwork->network.MacAddress, ETH_ALEN) != 0)
			{
				memcpy( old_bssid, pnetwork->network.MacAddress, ETH_ALEN );				
			}else{
				memset(old_bssid, 0, ETH_ALEN);
				continue;
			}
...
_func_enter_;	
	...
	_memset(pcmd, 0, sizeof(struct	cmd_obj));	//eason 20100407
...
int SecIsInPMKIDList(_adapter *Adapter, u8 *bssid)
{
	...
	do
	{
		if( ( psecuritypriv->PMKIDList[i].bUsed ) && 
                    (  __memcmp( psecuritypriv->PMKIDList[i].Bssid, bssid, ETH_ALEN ) == _TRUE ) )
		{
			break;
		}
		else
...
_func_enter_;
	...
	RT_TRACE(_module_rtl871x_mlme_c_,_drv_err_, ("restruct_sec_ie():ndisauthmode=%d, ndissecuritytype=%d\n", ndisauthmode, ndissecuritytype));
	...
	while(cnt<in_len)
	{
		...
		{
			if((authmode==_WPA_IE_ID_)&&(__memcmp(&in_ie[cnt+2], &wpa_oui[0], 4)==_TRUE))
			{
			...
			if(((authmode==_WPA_IE_ID_)&&(__memcmp(&in_ie[cnt+2], &wpa_oui[0], 4)==_TRUE))||(authmode==_WPA2_IE_ID_))
			{
	...
	if(match==_TRUE)
	{
		if(sec_ie[0]==_WPA_IE_ID_)
		{
			...
			while(_TRUE)
			{
				if(__memcmp(&sec_ie[2], &wpa_oui[0], 4) !=_TRUE)//check wpa_oui tag
				{  	 
				...
				if(__memcmp(&sec_ie[8], &wpa_oui[0], 3) ==_TRUE)
				{ 
				...
				{ 
					...
					if(__memcmp(&sec_ie[14], &uncst_oui[0], 4) !=_TRUE)
					{
		...
		if(authmode==_WPA2_IE_ID_)
		{
			...
			while(_TRUE)
			{
				...
				if(__memcmp(&sec_ie[4], &uncst_oui[0], 3) ==_TRUE)
				{ 
				...
				{ 
					//check the unicast encryption type
					if(__memcmp(&sec_ie[10], &uncst_oui[0],4) !=_TRUE)
					{
...
//eason 20100210	pdev_network->Configuration.BeaconPeriod = 100;	
	pdev_network->Configuration.BeaconPeriod = mvBeaconinterval;	
