	...
	)
{
	...
	if(pmp_wi_cntx->curractfunc != NULL)
	{
		...
		RT_TRACE(_module_rtl871x_mp_ioctl_c_,_drv_info_,("WorkItemActType: %d, time spent: %I64d us\n",
...
int init_mp_priv(struct mp_priv  *pmp_priv)
{
	...
	memset(pmp_priv->pallocated_mp_xmitframe_buf, 0, NR_MP_XMITFRAME * sizeof(struct mp_xmit_frame) + 4);	//eason 20100407
...
void _irqlevel_changed_(_irqL *irqlevel, unsigned char bLower)
{
...
u32 get_bb_reg(PADAPTER pAdapter ,u16 offset ,u32 bitmask)	
{
	...
	//RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("get_bb_reg org value :0x%08x.........\n",org_value));	
...
u8 set_bb_reg(PADAPTER pAdapter ,u16 offset ,u32 bitmask, u32 value)
{
	...
	if(bitmask!=bMaskDWord){		
		...
		RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("set_bb_reg #2  offset :0x%04x org:0x%08x  new:0x%08x.........\n",offset,org_value,new_value));	
	}
	else{
		new_value = value;
		RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("set_bb_reg #2  offset :0x%04x data:0x%08x.........\n",offset,new_value));	
	}	
...
u8 set_rf_reg(PADAPTER pAdapter ,u8 path,u8 offset,u32 bitmask,u32 value)
{
	...
	if(bitmask!=bMaskDWord){	
		...
		bit_shift = bitshift(bitmask);
		new_value =  ((org_value &  (~bitmask)) | (value << bit_shift) );
		RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("set_rf_reg #2  v:0x%08x org:0x%08x  new:0x%08x.........\n",value,org_value,new_value));	
	}
	else	{
		new_value = 	value ;
		RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("set_rf_reg #2  v:0x%08x data:0x%08x.........\n",value,new_value));	
	}
...
void SetChannel(PADAPTER pAdapter)
{
	...
	memset(pcmd, 0, sizeof(struct cmd_obj));	//eason 20100407
	...
	memset(pparm, 0, sizeof(struct SetChannel_parm));	//eason 20100407
...
void SetCCKTxPower(PADAPTER pAdapter,u8 TxPower)
{
	...
	RT_TRACE(_module_rtl871x_mp_ioctl_c_,_drv_alert_, ("SetCCKTxPower (%d).\n",TxPower));	
}
...
void SetOFDMTxPower(PADAPTER pAdapter,u8 TxPower)
{
	...
	RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("SetOFDMTxPower (%d).\n",TxPower));	
}
...
void SetTxPower(PADAPTER pAdapter)
{
	...
	memset(pparm, 0, sizeof(struct SetTxPower_parm));	//eason 20100407
...
{	
	...
	memset(pparm, 0, sizeof(struct setdatarate_parm));	//eason 20100407
...
void SwitchBandwidth(PADAPTER pAdapter)
{
	...
	memset(pparm, 0, sizeof(struct SwitchBandwidth_parm));	//eason 20100407
...
//#ifdef MP_FIRMWARE_OFFLOAD
	...
	memset(pparm, 0, sizeof(struct SwitchAntenna_parm));	//eason 20100407
...
#endif
	RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("SwitchAntenna finished...................\n"));
}
...
void SetCrystalCap(PADAPTER pAdapter)
{
	...
	memset(pparm, 0, sizeof(struct SetCrystalCap_parm));	//eason 20100407
	...
	RT_TRACE(_module_rtl871x_mp_c_,_drv_alert_, ("SetCrystalCap (%d)...................\n",pAdapter->mppriv.curr_crystalcap));	
#endif
}
...
{	
	...
	memset(pparm, 0, sizeof(struct SetSingleCarrierTx_parm));	//eason 20100407
	pparm->bStart = bStart;
...
void SetSingleToneTx(PADAPTER pAdapter,u8 bStart)
{
	...
	memset(pparm, 0, sizeof(struct SetSingleToneTx_parm));	//eason 20100407
	pparm->bStart = bStart;
	switch(pAdapter->mppriv.antenna_tx)
	{
		case ANTENNA_B:
...
void SetCarrierSuppressionTx(PADAPTER pAdapter,u8 bStart)
{
	...
	memset(pparm, 0, sizeof(struct SetCarrierSuppressionTx_parm));	//eason 20100407
	pparm->bStart = bStart;
	pparm->curr_rateidx = pAdapter->mppriv.curr_rateidx;
...
void SetContinuousTx(PADAPTER pAdapter, u8 bStart)
{
	...
	memset(pparm, 0, sizeof(struct SetContinuousTx_parm));	//eason 20100407
	pparm->bStart = bStart;
...
short ERALTEK_get_channel(void) {
	...
	if ((if_up(padapter)) == _FALSE) {
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)==_TRUE)
			return padapter->registrypriv.channel;
		else
		return 0;
	}		
	else {
...
void REALTEK_get_currssid(unsigned char huge * ssid) {
	...
	if ((if_up(padapter)) == _FALSE) {
		...
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)==_TRUE)
			memcpy(ssid, pcur_bss->Ssid.Ssid, pcur_bss->Ssid.SsidLength);	
	}		
	else {
		memset(ssid, 0, WLAN_SSID_MAXLEN);
		memcpy(ssid, pcur_bss->Ssid.Ssid, pcur_bss->Ssid.SsidLength);	
...
void REALTEK_get_currbssid(unsigned char huge * bssid) {
	...
	if ((if_up(padapter)) == _FALSE) {
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)==_TRUE)
			memcpy(bssid, pcur_bss->MacAddress, WLAN_BSSID_LEN); 
		else
...
short REALTEK_get_currrate(void) {
	...
	memset(&bitrate, 0, sizeof(struct iw_param));	
	...
	if(data_rate == 0)
	{
		r8711_wx_get_rate(&bitrate); 
		data_rate = bitrate.value/1000000;
		if((padapter->registrypriv.wireless_mode == WIRELESS_11B) && (data_rate >= 11))	
			data_rate = 11;
		if((padapter->registrypriv.wireless_mode == WIRELESS_11BG ||
			 padapter->registrypriv.wireless_mode == WIRELESS_11G) && (data_rate >= 48))	
			data_rate = 54;	
		if(padapter->registrypriv.wireless_mode == WIRELESS_11BGN)	
		{		
			 if((data_rate >= 48) && (data_rate <= 54))
			 	data_rate = 54;
...
short REALTEK_get_rssi(void) {
	...
	if ((if_up(padapter)) == _FALSE) {
		...
		return ((pcur_bss->Rssi) - 110);		
	}	
}


short REALTEK_get_linkquality(void) {
	...
	if ((if_up(padapter)) == _FALSE) {
		...
		return (signal_scale_mapping(pcur_bss->Rssi));		
	}	
}
knownbss_t* REALTEK_get_scanlist(void) {

	knownbss_t* pTmp = NULL;
	u16 cap;
   _irqL	irqL;
	_list					*plist, *phead;
	_adapter *padapter = (_adapter *)Global_padapter;
	...
	while(1)
	{
		...
		pTmp = malloc(sizeof(knownbss_t));
		if (pTmp == NULL)
			continue;
		memset(pTmp, 0, sizeof(knownbss_t));

		memcpy(pTmp->bssid, pnetwork->network.MacAddress, ETH_ALEN);
		pTmp->channel = pnetwork->network.Configuration.DSConfig;

		memcpy((u8 *)&cap, get_capability_from_ie(pnetwork->network.IEs), 2);
		cap = le16_to_cpu(cap);
	
		if(cap & (WLAN_CAPABILITY_IBSS |WLAN_CAPABILITY_BSS)){
			if (cap & WLAN_CAPABILITY_BSS)	// 0:Ad-Hoc 1:Infra
				pTmp->cap_info = 1;
			else
				pTmp->cap_info = 0;
		}
		
		if(pnetwork->network.Privacy) {
			...
			
		{
			...
			if(( wpa_len || rsn_len) > 0)
				pTmp->cap_info |= 1 << 8;
		}
		}
		
		memset(pTmp->ssid,0,WLAN_SSID_MAXLEN+2+1);
		if(strcmp(pnetwork->network.Ssid.Ssid, "<hidden>") != 0)
			memcpy(pTmp->ssid, pnetwork->network.Ssid.Ssid, pnetwork->network.Ssid.SsidLength); 
		/* Lance - Begin */
#if 0
		sprintf(buf,"Lance %s, wpa_len = %d,  rsn_len = %d\n",pTmp->ssid, wpa_len , rsn_len);
		serial_puts(buf);
		sprintf(buf, "wpa = ");
		for(i=0;i<wpa_len;i++)
		{
		...
		if(wpa_len>0)
		{
				...
				group_cipher = 0;
				pairwise_cipher = 0;				
				parse_wpa_ie(wpa_ie, wpa_len, &group_cipher, &pairwise_cipher);
				sprintf(buf,"Lance WPA IE, group_cipher = %x,  pairwise_cipher = %x\n", group_cipher , pairwise_cipher);
				serial_puts(buf);
		}
		...
		if(rsn_len>0)
		{
				...
				group_cipher = 0;
				pairwise_cipher = 0;
				parse_wpa2_ie(rsn_ie, rsn_len, &group_cipher, &pairwise_cipher);
				sprintf(buf,"Lance RSN IE, group_cipher = %x,  pairwise_cipher = %x\n", group_cipher , pairwise_cipher);
				serial_puts(buf);
		}
#else
...
#endif
		/* Lance - End */
		//pTmp->rssi = signal_scale_mapping(pnetwork->network.Rssi);
	   	pTmp->rssi = (pnetwork->network.Rssi - 110);		
