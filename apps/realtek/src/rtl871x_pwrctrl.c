...
//eason 20100210------------------------------------
#define SetPSModeWorkItem_TASK_PRI         	20
#define SetPSModeWorkItem_TASK_STACK_SIZE  	4096
static	uint8			SetPSModeWorkItem_Stack[SetPSModeWorkItem_TASK_STACK_SIZE];
static  cyg_thread		SetPSModeWorkItem_Task;
static  cyg_handle_t	SetPSModeWorkItem_TaskHdl;

#define rpwm_workitem_TASK_PRI         	20
#define rpwm_workitem_TASK_STACK_SIZE  	4096
static	uint8			rpwm_workitem_Stack[rpwm_workitem_TASK_STACK_SIZE];
static  cyg_thread		rpwm_workitem_Task;
static  cyg_handle_t	rpwm_workitem_TaskHdl;

cyg_sem_t SetPSModeWorkItem_Thread;
cyg_sem_t rpwm_workitem_Thread;
...
_func_enter_;
	...
	//eason 20100210 printk("========= Power Mode is :%d, Smart_PS = %d\n", ps_mode,smart_ps);
	...
	{		
		...
		//eason 20100210 _set_workitem(&(pwrpriv->SetPSModeWorkItem));
		cyg_semaphore_post(&SetPSModeWorkItem_Thread);
	}
...
_func_enter_;
	...
	if(pwrpriv->cpwm !=pwrpriv->rpwm){
		...
		//eason 20100210 _set_workitem(&(pwrpriv->rpwm_workitem));
		cyg_semaphore_post(&rpwm_workitem_Thread);
	}
_func_exit_;	
...
//
//-----------------------------------------------------------------------------
void SetPSModeWorkItemCallback(IN NDIS_WORK_ITEM*	pWorkItem,IN PVOID	Context)
{
	_adapter			*padapter = (_adapter *)Context;
	struct pwrctrl_priv *pwrpriv = &padapter->pwrctrlpriv;
...
//eason 20100210 void SetPSModeWorkItemCallback(struct work_struct *work)
void SetPSModeWorkItemCallback(cyg_addrword_t data)
{
#if 0 //eason 20100210
...
_func_enter_;
	...
	{
		cyg_semaphore_wait(&SetPSModeWorkItem_Thread); //eason 20100210
		RT_TRACE(_module_rtl871x_pwrctrl_c_,_drv_info_,("SetPSModeWorkItemCallback: Power Mode is :%d\n", pwrpriv->pwr_mode));
		_enter_pwrlock(&pwrpriv->lock);
...
//eason 2010210 void rpwm_workitem_callback(struct work_struct *work)
void rpwm_workitem_callback(cyg_addrword_t data)
{
#if 0 //eason 20100210
	struct pwrctrl_priv *pwrpriv = container_of(work, struct pwrctrl_priv, SetPSModeWorkItem);
	_adapter *padapter = container_of(pwrpriv, _adapter, pwrctrlpriv);
...
_func_enter_;	
	...
	{
	//	msleep_os(10);
		cyg_semaphore_wait(&rpwm_workitem_Thread); 
		_enter_pwrlock(&pwrpriv->lock);
...
#else
	 cyg_semaphore_init(&SetPSModeWorkItem_Thread,0);
	 cyg_semaphore_init(&rpwm_workitem_Thread,0);
	  
	 cyg_thread_create(SetPSModeWorkItem_TASK_PRI,
					SetPSModeWorkItemCallback,
					0,
					"SetPSModeWorkItem",
					(void *) (SetPSModeWorkItem_Stack),
					SetPSModeWorkItem_TASK_STACK_SIZE,
					&SetPSModeWorkItem_TaskHdl,
					&SetPSModeWorkItem_Task);
				
		//Start SetPSModeWorkItem Thread
	cyg_thread_resume(SetPSModeWorkItem_TaskHdl);
		  	  
	cyg_thread_create(rpwm_workitem_TASK_PRI,
					rpwm_workitem_callback,
					0,
					"rpwm_workitem",
					(void *) (rpwm_workitem_Stack),
					rpwm_workitem_TASK_STACK_SIZE,
					&rpwm_workitem_TaskHdl,
					&rpwm_workitem_Task);
				
		//Start rpwm_workitem Thread
	cyg_thread_resume(rpwm_workitem_TaskHdl);
#endif //eason 20100210	
#ifdef PLATFORM_WINDOWS
