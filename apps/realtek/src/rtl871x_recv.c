...
u8 *recvframe_pull(union recv_frame *precvframe, sint sz)
{
...
_func_enter_;		
       ...
       if((prxattrib->encrypt>0) && ((prxattrib->bdecrypted==0) ||(psecuritypriv->sw_decrypt==_TRUE)))
	{
		...
		case _TKIP_:
			...
			break;
		case _AES_:
			//eason 20100723	aes_decrypt(padapter, (u8 * )precv_frame);
			aes_decrypt_by_driver(padapter, (u8 * )precv_frame);
			break;
		default:
				break;
		}
...
_func_enter_;		
		...
		(check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE)) 
	{
	    ...
	    if (__memcmp(myhwaddr, pattrib->src, ETH_ALEN)){
		  RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,(" SA==myself \n"));
		  ret= _FAIL;
		  goto exit;
	    }		
        
	    if( (!__memcmp(myhwaddr, pattrib->dst, ETH_ALEN))	&& (!bmcast) ){
		  ret= _FAIL;
		  goto exit;
	    	}  
		
	    if( __memcmp(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		   __memcmp(mybssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		   (!__memcmp(pattrib->bssid, mybssid, ETH_ALEN)) ) {
			ret= _FAIL;
			goto exit;
	    	}
	...
	{	
		...
		if(!__memcmp(pattrib->bssid, pattrib->src, ETH_ALEN) )
		{
	 ...
	 else if(check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
	 {
		...
		else // not mc-frame
		{
			...
			if(!__memcmp(pattrib->bssid, pattrib->dst, ETH_ALEN)) {
				ret= _FAIL;
				goto exit;
			}
...
_func_enter_;
	...
	{ 
...
#endif		
		{
			...
			// For AP mode, if DA is non-MCAST, then it must be BSSID, and bssid == BSSID
			//???if( (!__memcmp(mybssid, pattrib->dst, ETH_ALEN)) || 
			if(!__memcmp(pattrib->bssid, mybssid, ETH_ALEN))
			{
					ret= _FAIL;
					goto exit;
				}
...
_func_enter_;
	...
	if((__memcmp(psnap, rfc1042_header, SNAP_SIZE) &&
		(__memcmp(psnap_type, SNAP_ETH_TYPE_IPX, 2) == _FALSE) && 
		(__memcmp(psnap_type, SNAP_ETH_TYPE_APPLETALK_AARP, 2)==_FALSE) )||
		//eth_type != ETH_P_AARP && eth_type != ETH_P_IPX) ||
		 __memcmp(psnap, bridge_tunnel_header, SNAP_SIZE)){
...
//eason 20100520	padapter->ledpriv.LedControlHandler(padapter, LED_CTL_RX);
