...
//eason 20100210	#ifdef PLATFORM_WINDOWS
#include <if_ether.h>
#include <ip.h>
//eason 20100210	#endif
//eason 20100210------------------------------------
#define RT8712BulkTx_TASK_PRI         	20
#define RT8712BulkTx_TASK_STACK_SIZE  	8192
static	uint8			RT8712BulkTx_Stack[RT8712BulkTx_TASK_STACK_SIZE];
static  cyg_thread		RT8712BulkTx_Task;
static  cyg_handle_t	RT8712BulkTx_TaskHdl;

cyg_sem_t RT8712BulkTxThread;
cyg_sem_t RT8188BulkTxThread;
//eason 20100210------------------------------------
/* eason 20100210
...
_func_enter_;   	
	...
	*/
	...
	memset(pxmitpriv->pallocated_frame_buf, 0, NR_XMITFRAME * sizeof(struct xmit_frame) + 4);	//eason 20100407
	...
	*/	
	...
	memset(pxmitpriv->pallocated_xmitbuf, 0, NR_XMITBUFF *sizeof(struct xmit_buf) + 4);	//eason 20100407
	...
	{	
		...
		memset(pxmitbuf->pallocated_buf, 0, MAX_XMITBUF_SZ + XMITBUF_ALIGN_SZ);	//eason 20100407
...
#endif
#else
	cyg_semaphore_init(&RT8712BulkTxThread,0);
	cyg_semaphore_init(&RT8188BulkTxThread,0);
	  
	cyg_thread_create(RT8712BulkTx_TASK_PRI,
					xmit_bh,
					0,
					"RT8712BulkTx",
					(void *) (RT8712BulkTx_Stack),
					RT8712BulkTx_TASK_STACK_SIZE,
					&RT8712BulkTx_TaskHdl,
					&RT8712BulkTx_Task);
				
		//Start RT8712BulkTx Thread
	cyg_thread_resume(RT8712BulkTx_TaskHdl);
...
#ifdef CONFIG_PWRCTRL
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) ||
		...
		if (pattrib->ether_type != 0x8712) {
		 	res = _FAIL;
			RT_TRACE(_module_rtl871x_xmit_c_,_drv_alert_,("IN WIFI_MP_STATE but the ether_type(0x%x) !=0x8712!!!",pattrib->ether_type));
			goto exit;
		}
	...
	if(bmcast)
	{
		...
		{	
			...
			pattrib->mac_id = 5;
			RT_TRACE(_module_rtl871x_xmit_c_,_drv_alert_,("mp xmit pkt:%d \n",padapter->mppriv.tx_pktcount));
	...
	else  // if we cannot get psta => drrp the pkt
	{
		RT_TRACE(_module_rtl871x_xmit_c_, _drv_alert_, ("\n update_attrib => get sta_info fail \n"));
		RT_TRACE(_module_rtl871x_xmit_c_, _drv_alert_, ("\n ra:%x:%x:%x:%x:%x:%x\n", 
		pattrib->ra[0], pattrib->ra[1],
	...
	RT_TRACE(_module_rtl871x_xmit_c_,_drv_info_,
		("pattrib->encrypt=%d padapter->securitypriv.sw_encrypt=%d ",
		pattrib->encrypt,padapter->securitypriv.sw_encrypt));	
	
	if((pattrib->encrypt)&&((padapter->securitypriv.sw_encrypt==_TRUE)||(psecuritypriv->hw_decrypted==_FALSE)))
	{
		pattrib->bswenc=_TRUE;
		
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,
			("pattrib->encrypt=%d padapter->securitypriv.hw_decrypted=%d pattrib->bswenc=_TRUE",
						pattrib->encrypt,padapter->securitypriv.sw_encrypt));
	}
	else
	{
		pattrib->bswenc=_FALSE;
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_info_,("pattrib->bswenc=_FALSE"));
	}
       ...
       if (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE  ){  
		...
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_alert_,("====> update_attrib  priority=%x\n", pattrib->priority));			   
       }
#endif
...
_func_enter_;
	...
	if(pattrib->encrypt ==_TKIP_)//if(psecuritypriv->dot11PrivacyAlgrthm==_TKIP_PRIVACY_) 
	{
		...
		if(stainfo!= NULL){
			...
			if(bmcst)
			{
				if(__memcmp(psecuritypriv->dot118021XGrptxmickey[psecuritypriv->dot118021XGrpKeyid].skey, null_key, 16)==_TRUE){
			...
			{
			if(__memcmp(&stainfo->dot11tkiptxmickey.skey[0],null_key, 16)==_TRUE){
				//DbgPrint("\nxmitframe_addmic:stainfo->dot11tkiptxmickey==0\n");
				//msleep_os(10);
				return _FAIL;
...
_func_enter_;
	...
	if(pattrib->bswenc)
	{
		...
		case _TKIP_:			
			...
			break;
		case _AES_:			
			//eason 20100723	aes_encrypt(padapter, (u8 * )pxmitframe);
			aes_encrypt_by_driver(padapter, (u8 * )pxmitframe);			
			break;
		default:
				break;
		}
...
_func_enter_;
	...
	while (1)
	{
		...
		if (bmcst || (endofpktfile(&pktfile) == _TRUE))
		{
			...
			RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("\n coalesce:  pattrib->last_txcmdsz=%d pxmitframe->pxmitbuf->phead=0x%p  pxmitframe->pxmitbuf->ptail=0x%p pxmitframe->pxmitbuf->len=%d\n", pattrib->last_txcmdsz,pxmitframe->pxmitbuf->phead,pxmitframe->pxmitbuf->ptail,pxmitframe->pxmitbuf->len));
			...
			RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("\n[2] coalesce:  pattrib->last_txcmdsz=%d pxmitframe->pxmitbuf->ptail=0x%p pxmitframe->pxmitbuf->len=%d\n", pattrib->last_txcmdsz,pxmitframe->pxmitbuf->ptail,pxmitframe->pxmitbuf->len));
#endif
			break;
		}else{
#ifdef CONFIG_SDIO_HCI
...
#ifdef PLATFORM_LINUX
//eason 20100210        if (netif_queue_stopped(padapter->pnetdev))		     
//eason 20100210                 netif_wake_queue(padapter->pnetdev);	
#endif	
...
_func_enter_;
	...
	{
		res= _FAIL;
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("psta == NULL"));
		goto exit;
	}
...
void alloc_hwxmits(_adapter *padapter)
{
	...
	memset(pxmitpriv->hwxmits, 0, sizeof (struct hw_xmit) * pxmitpriv->hwxmit_entry);	//eason 20100407
