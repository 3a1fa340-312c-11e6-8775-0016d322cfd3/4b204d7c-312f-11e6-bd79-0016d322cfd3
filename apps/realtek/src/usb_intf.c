...
#include <usb_hal.h>
#include "psdefine.h"	//eason 20100210
#include "wlanif.h" 	//eason 20100210
...
extern char initmac[6];

void r871x_dev_unload(_adapter *padapter);
//eason 20100210 static int r871xu_drv_init(struct usb_interface *pusb_intf,const struct usb_device_id *pdid);
//eason 20100210 static void r871xu_dev_remove(struct usb_interface *pusb_intf);
static int r871xu_drv_init(struct usb_device *dev,unsigned int pusb_intf,const struct usb_device_id *pdid);
static void r871xu_dev_remove(struct usb_device *dev,struct usb_interface *pusb_intf);
extern _adapter *Global_padapter;
...
//eason 20100210	struct	usb_host_config			*phost_conf;
//eason 20100210	struct	usb_config_descriptor 		*pconf_desc;

//eason 20100210	struct	usb_host_interface		*phost_iface;
//eason 20100210	struct	usb_interface_descriptor	*piface_desc;
...
_func_enter_;
	...
	pdvobjpriv->nr_endpoint = pintf->altsetting->bNumEndpoints;	//eason 20100210
#if 0 //eason 20100210
#if 0
...
void r871x_dev_unload(_adapter *padapter)
{
	...
	{
//eason 20100210		printk("+r871x_dev_unload\n");
		//s1.
/*		if(pnetdev)   
     		{
		...
		if(padapter->bSurpriseRemoved == _FALSE)
		{
//eason 20100210			printk("r871x_dev_unload()->rtl871x_hal_deinit()\n");
			rtl871x_hal_deinit(padapter);
...
{  	
	...
	//eason 20100210 usb_get_dev(interface_to_usbdev(pusb_intf));

	//eason 20100210 pintf = pusb_intf;
	pintf = dev->actconfig->interface + pusb_intf;
	...
	//eason 20100210 padapter = netdev_priv(pnetdev);
...
#else	
	padapter->dvobjpriv.pusbdev = dev;
#endif //eason 20100210
...
//ZOT			for( jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3 )
//ZOT			{
//ZOT            			mac[jj] = key_2char2num(initmac[kk], initmac[kk+ 1]);
//ZOT			}
			_memcpy(mac, initmac, ETH_ALEN);
		}
		else
...
//			printk("AutoLoad Fail reported from CR9346!!\n");
		...
		//eason 20100210 printk("MAC Address from efuse= %x-%x-%x-%x-%x-%x\n", 
	...
	netdev_open(pnetdev);	//eason 20100210
	...
	if(1)	
	{
		...
		{
//Jesse			IP4_ADDR( &ipaddr,0,0,0,0);
//Jesse			IP4_ADDR( &netmask, 0,0,0,0);
//Jesse			IP4_ADDR( &gw, 0,0,0,0);
			ipaddr.addr = Lanface->ip_addr.addr;
			netmask.addr = Lanface->netmask.addr;
			gw.addr = Lanface->gw.addr;
		}
		else
		{
		...
		WLanface = netif_add( netif ,&ipaddr, &netmask, &gw, 0, pk_attach, tcpip_input);
		WLanface->name[0] = 'W';
	}
...
//eason 20100210	usb_set_intfdata(pusb_intf, NULL);
...
static void r871xu_dev_remove(struct usb_device *dev,struct usb_interface *pusb_intf)
{		
	//eason 20100210 struct net_device *pnetdev=usb_get_intfdata(pusb_intf);	
       //eason 20100210 _adapter *padapter = (_adapter*)netdev_priv(pnetdev);
       struct net_device *pnetdev=(struct net_device *)pusb_intf;	
...
int r8712u_drv_entry(void)
{
	//eason 20100210 RT_TRACE(_module_hci_intfs_c_,_drv_err_,("+r8712u_drv_entry\n"));
	//eason 20100210 drvpriv.drv_registered = _TRUE;
	
	drvpriv.probe	  = r871xu_drv_init;			//eason 20100210	// wireless card plugged
	drvpriv.disconnect = r871xu_dev_remove;			//eason 20100210	// wireless card unplugged
	drvpriv.id_table   = rtl871x_usb_id_tbl;		//eason 20100210	// unused
	
	//eason 20100210 return usb_register(&drvpriv.r871xu_drv);	
