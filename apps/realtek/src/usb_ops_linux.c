...
#define usb_fill_bulk_urb(a,aa,b,c,d,e,f)	FILL_BULK_URB(a,aa,b,c,d,e,f)	//eason 2010210
#define URB_ZERO_PACKET	USB_ZERO_PACKET 									//eason 2010210
...
_func_enter_;
	...
	memset(pintfpriv->allocated_io_rwmem, 0, pintfpriv->max_iosz +4);	//eason 20100407
...
{	
	...
	if(pcontext == NULL)				//eason 20100407
		return NULL;				//eason 20100407
	memset(pcontext, 0, sizeof (struct zero_bulkout_context));	//eason 20100407
	
	pbuf = (unsigned char *)_malloc(sizeof(int));
	
	if(pbuf == NULL)				//eason 20100407
		return NULL;				//eason 20100407
	memset(pbuf, 0, sizeof (int));	//eason 20100407
...
_func_enter_;	
	...
	status = usb_submit_urb(piorw_urb);
...
extern cyg_sem_t RT8712BulkRxThread;	//eason 20100210
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 18))
		...
		{	
			precvbuf->transfer_len = purb->actual_length;
				...
				else
				{
					...
					//eason 20100210 tasklet_hi_schedule(&precvpriv->recv_tasklet);
					cyg_semaphore_post(&RT8712BulkRxThread);	
...
_func_enter_;
	...
	{	
		...
		//eason 20100210	precvbuf->phead = precvbuf->pskb->head;
			precvbuf->pdata = precvbuf->pskb->data;
		//eason 20100210	precvbuf->ptail = precvbuf->pskb->tail;
		//eason 20100210	precvbuf->pend = precvbuf->pskb->end;
...
extern cyg_sem_t RT8712BulkTxThread;	//eason 20100210
extern _adapter *Global_padapter;
//eason 20100210 void xmit_bh(void *priv)
void xmit_bh(cyg_addrword_t data)
{	
	int ret = _FALSE;
	//eaason 20100210 _adapter *padapter = (_adapter*)priv;
	_adapter *padapter = (_adapter*)Global_padapter;
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	
	while(1)
	{
		cyg_semaphore_wait(&RT8712BulkTxThread); //eason 20100210
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE))
		{
//eason 20100210			printk("xmit_bh => bDriverStopped or bSurpriseRemoved \n");
			goto error;
		}
		...
		if(ret==_FALSE)
		{
			//eason 20100608	return;
		}
		else
			cyg_semaphore_post(&RT8712BulkTxThread);
	}
error:	
	return;
}
...
//	}
	...
	//eason 20100210 tasklet_hi_schedule(&pxmitpriv->xmit_tasklet);
	cyg_semaphore_post(&RT8712BulkTxThread);
#else
