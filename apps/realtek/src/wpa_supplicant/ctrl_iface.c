					 ...
					 char *buf, size_t *resp_len)
{
	...
	os_memset(reply, 0, reply_size); 	//eason 20100407
	os_memcpy(reply, "OK\n", 3);
	reply_len = 3;
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		/*eason 20100608	reply_len = wpa_sm_get_mib(wpa_s->wpa, reply, reply_size);
		if (reply_len >= 0) {
			int res;
		...
		/*eason 20100608	reply_len = wpa_supplicant_ctrl_iface_status(
			wpa_s, buf + 6, reply, reply_size);*/
	} else if (os_strcmp(buf, "PMKSA") == 0) {
		reply_len = pmksa_cache_list(wpa_s->wpa, reply, reply_size);
	} else if (os_strncmp(buf, "SET ", 4) == 0) {
		/*eason 20100608	if (wpa_supplicant_ctrl_iface_set(wpa_s, buf + 4))
			reply_len = -1;*/
