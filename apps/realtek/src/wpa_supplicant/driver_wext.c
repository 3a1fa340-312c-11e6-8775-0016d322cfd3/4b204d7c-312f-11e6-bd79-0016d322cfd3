...
//eason 20100407	#include <net/if_arp.h>
//eason 20100407------------
...
int wpa_driver_wext_set_auth_param(struct wpa_driver_wext_data *drv,
				   int idx, u32 value)
{
	//eason 20100407	struct iwreq iwr;
	struct iw_param param;
	int ret = 0;
	
	os_memset(&param, 0, sizeof(struct iw_param));	//eason 20100407
	param.flags = idx & IW_AUTH_INDEX;		//eason 20100407
	param.value = value;					//eason 20100407
#if 0 //eason 20100407
...
int wpa_driver_wext_get_bssid(void *priv, u8 *bssid)
{
	...
	//eason 20100407	struct iwreq iwr;
	struct sockaddr1 ap_addr;	//eason 20100407
	int ret = 0;
#if 0 //eason 20100407
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
...
#else
	os_memset(&ap_addr, 0, sizeof(ap_addr));
	
	if( r8711_wx_get_wap(&ap_addr) < 0)
		ret = -1;
	os_memcpy(bssid, ap_addr.sa_data, ETH_ALEN);
#endif //eason 20100407
	return ret;
}
...
{
	//eason 20100608	struct iwreq iwr;
	...
	data.pointer = &u32wps_state;	
	data.length = sizeof( u32wps_state );
	
	r871x_wps_start(&data);
#endif //eason 20100407
}
...
int wpa_driver_wext_set_bssid(void *priv, const u8 *bssid)
{
	...
	if (bssid)
		os_memcpy(ap_addr.sa_data, bssid, ETH_ALEN);
	else
		os_memset(ap_addr.sa_data, 0, ETH_ALEN);
		
	if(r8711_wx_set_wap(&ap_addr) < 0)
		ret = -1;
#endif //eason 20100407
	return ret;
}
...
{
	struct wpa_driver_wext_data *drv = priv;
	struct iw_point essid;//eason 20100407	struct iwreq iwr;
	int ret = 0;
#if 0 //eason 20100407
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	...
	if (ioctl(drv->ioctl_sock, SIOCGIWESSID, &iwr) < 0) {
		...
		ret = -1;
#else
	os_memset(&essid, 0, sizeof(essid));
	essid.pointer = ssid;
	essid.length = 32;

	if (r8711_wx_get_essid(&essid, essid.pointer)) {
		ret = -1;	
	} else {
		ret = essid.length;//eason 20100407	ret = iwr.u.essid.length;
		if (ret > 32)
			ret = 32;
...
int wpa_driver_wext_set_ssid(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if(r8711_wx_set_essid(ssid_len, buf) < 0)
		ret = -1;
#endif //eason 20100407
	return ret;
}
...
{
	struct wpa_driver_wext_data *drv = priv;
	//eason 20100520	struct iwreq iwr;
	struct	iw_freq wrqu;
	int ret = 0;
#if 0 //eason 20100407
	os_memset(&iwr, 0, sizeof(iwr));
...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
		...
		os_memset(data.assoc_info.req_ies, 0, bytes); 	//eason 20100407
		...
		if (os_strncmp(spos, " RespIEs=", 9) == 0) {
			...
			os_memset(data.assoc_info.resp_ies, 0, bytes); 	//eason 20100407
	...
	struct wpa_driver_wext_data *drv, const char *ev, int len)
{
	...
	os_memset(drv->assoc_req_ies, 0, len); 	//eason 20100407
	...
	struct wpa_driver_wext_data *drv, const char *ev, int len)
{
	...
	os_memset(drv->assoc_resp_ies, 0, len); 	//eason 20100407
					   ...
					   void *ctx, char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case IWEVCUSTOM:
			...
			if (buf == NULL)
				return;
			os_memset(buf, 0, iwe->u.data.length + 1); 	//eason 20100407	
...
/*eason 20100407	wpa_printf(MSG_DEBUG, "RTM_NEWLINK: operstate=%d ifi_flags=0x%x "
		   "(%s%s%s%s)",
...
//ZOT	char buf[8192];
	int left;
/*eason 20100407	struct sockaddr_nl from;
	socklen_t fromlen;*/
	struct nlmsghdr *h;
	int max_events = 10;
	int32 cmd = 0;	//eason 20100407
	
#if	0	//eason 20100407
try_again:
	fromlen = sizeof(from);
	left = recvfrom(sock, buf, sizeof(buf), MSG_DONTWAIT,
			(struct sockaddr1 *) &from, &fromlen);
	if (left < 0) {
...
#else
		cmd = get_realtek_event();
		switch (cmd) {
		case 1:
			wpa_supplicant_event(sock_ctx, EVENT_DISASSOC, NULL);
			break;
		case 2:			
			wpa_driver_wext_event_assoc_ies(eloop_ctx);
			wpa_supplicant_event(sock_ctx, EVENT_ASSOC, NULL);
			break;
		case 3:
			eloop_cancel_timeout(wpa_driver_wext_scan_timeout,
					     eloop_ctx, sock_ctx);
			wpa_supplicant_event(sock_ctx, EVENT_SCAN_RESULTS, NULL);
			break;
		}
#endif	//eason 20100407	
}
...
void * wpa_driver_wext_init(void *ctx, const char *ifname)
{
	...
	if (drv == NULL)
		return NULL;
	os_memset(drv, 0, sizeof(*drv));	//eason 20100407
	drv->ctx = ctx;
	...
	if (bind(s, (struct sockaddr1 *) &local, sizeof(local)) < 0) {
		perror("bind(netlink)");
		close(s);
		close(drv->ioctl_sock);
		os_free(drv);
...
//eason 20100407	drv->ifindex = if_nametoindex(drv->ifname);
...
//eason 20100407	wpa_driver_wext_send_oper_ifla(drv, 1, IF_OPER_DORMANT);
}
...
//eason 20100407	wpa_driver_wext_send_oper_ifla(priv, 0, IF_OPER_UP);
...
int wpa_driver_wext_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if (ssid && ssid_len) {
		...
		os_memcpy(req.essid, ssid, ssid_len);
		data.pointer = &req;
		data.length = sizeof(req);
		data.flags = IW_SCAN_THIS_ESSID;
	}

	ret = r8711_wx_set_scan(&data ,&req); 
...
char scan_temp_buf[1024*8+512];	//eason 20100407
...
//eason 20100407	struct iwreq iwr;
	...
	//eason 20100407	res_buf_len = IW_SCAN_MAX_DATA;
	res_buf_len = 1024*8;
	for (;;) {
		//eason 20100407	res_buf = os_malloc(res_buf_len);
		res_buf = scan_temp_buf;
		if (res_buf == NULL)
			return NULL;
#if 0 //eason 20100407			
		os_memset(&iwr, 0, sizeof(iwr));
		...
		if (ioctl(drv->ioctl_sock, SIOCGIWSCAN, &iwr) == 0)
			break;
#else
		os_memset(res_buf, 0, res_buf_len);
		os_memset(&data, 0, sizeof(struct iw_point));
		data.pointer = res_buf;
		data.length = res_buf_len;	
		
		if(r8711_wx_get_scan(&data, res_buf) == 0)
			break;
		else
			ppause(10);
#endif //eason 20100407			
...
//eason 20100407	if (iwr.u.data.length > res_buf_len) {
	if (data.length > res_buf_len) {
		os_free(res_buf);
		return NULL;
	}
//eason 20100407	*len = iwr.u.data.length;
	*len = data.length;
					   ...
					   struct wext_scan_data *data)
{
	...
	os_memset(r, 0, sizeof(*r) + extra_len + data->ie_len);	//eason 20100407		
...
struct wpa_scan_results * wpa_driver_wext_get_scan_results(void *priv)
{
	...
	os_memset(res, 0, sizeof(*res));	//eason 20100407
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		/* Event data may be unaligned, so make a local, aligned copy
		 * before processing. */
		//eason 20100407	os_memcpy(&iwe_buf, pos, IW_EV_LCP_LEN);
		os_memcpy(&iwe_buf, pos, sizeof(struct iw_event));
		if (iwe->len <= IW_EV_LCP_LEN)
			break;
...
//eason 20100407	os_free(res_buf);
	res_buf = NULL;
	if (!first)
		wpa_driver_wext_add_scan_entry(res, &data);
	os_free(data.ie);
...
static int wpa_driver_wext_get_range(void *priv)
{
	...
	if (range == NULL)
		return -1;
	os_memset(range, 0, buflen);	//eason 20100407		
#if 0 //eason 20100407
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
				   ...
				   const u8 *psk)
{
	...
	//eason 20100407	struct iwreq iwr;
	struct iw_point	encoding;	//eason 20100407
	int ret;
	...
	os_free(ext);
#else
	os_memset(&encoding, 0, sizeof(encoding));

	ext = os_zalloc(sizeof(*ext) + PMK_LEN);
	if (ext == NULL)
		return -1;
	os_memset(ext, 0, sizeof(sizeof(*ext) + PMK_LEN));
	encoding.pointer =  ext;
	encoding.length = sizeof(*ext) + PMK_LEN;
	ext->key_len = PMK_LEN;
	os_memcpy(&ext->key, psk, ext->key_len);
	ext->alg = IW_ENCODE_ALG_PMK;
	
	ret = r871x_wx_set_enc_ext(encoding, ext);	
	os_free(ext);
#endif //eason 20100407
	return ret;
}
...
#else
	os_memset(ext, 0, sizeof(*ext) + key_len);
	os_memset(&encoding, 0, sizeof(struct iw_point));
	encoding.flags = key_idx + 1;
	encoding.flags |= IW_ENCODE_TEMP;
	if (alg == WPA_ALG_NONE)
		encoding.flags |= IW_ENCODE_DISABLED;
	
	encoding.length = sizeof(*ext) + key_len;
#endif //eason 20100407
	if (addr == NULL ||
				      ...
				      size_t ie_len)
{
	...
	ret = r871x_wx_set_gen_ie(&data, data.pointer);
#endif //eason 20100407
	return ret;
}
				  ...
				  struct wpa_driver_associate_params *params)
{
	...
	if(r8711_wx_set_enc(&encoding, encoding.pointer) < 0)
		ret = -1;
#endif //eason 20100407
	return ret;
}
...
#else
	ret = r8711_wx_set_mode(new_mode);
...
/*eason 20100407	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);*/
...
/*eason 20100407	drv->ifindex2 = if_nametoindex(ifname);
	if (drv->ifindex2 <= 0)
		return -1;*/
