...
struct eloop_timeout {
	...
	unsigned long start, timeout;	//eason 20100407
};
...
#endif	//eason 20100407
{
	...
	table->changed = 0;
	for (i = 0; i < table->count; i++) {
//eason	20100407		if (FD_ISSET(table->table[i].sock, fds)) {
			   ...
			   void *eloop_data, void *user_data)
{
	...
	if (timeout == NULL)
		return -1;
	memset(timeout, 0, sizeof(*timeout));	//eason 20100407		
/*eason 20100407	if (os_get_time(&timeout->time) < 0) {
		os_free(timeout);
		return -1;
	}*/
	timeout->start = jiffies;	//eason 20100407
	...
	//eason 20100608	timeout->timeout = timeout->start + ((secs * TICKS_PER_SEC) + ((usecs > 4000)?(usecs/4000):1));	//eason 20100407
	timeout->timeout = timeout->start + ((secs * TICKS_PER_SEC * 10) + ((usecs > 4000)?(usecs/4000):1));	//eason 20100407
	...
	tmp = eloop.timeout;
	while (tmp != NULL) {
		//eason	20100407	if (os_time_before(&timeout->time, &tmp->time))
		if (timeout->timeout < tmp->timeout)	//eason 20100407
			break;
		prev = tmp;
		tmp = tmp->next;
	}
...
/*eason 20100407		printf("ELOOP: remaining timeout: %d.%06d eloop_data=%p "
