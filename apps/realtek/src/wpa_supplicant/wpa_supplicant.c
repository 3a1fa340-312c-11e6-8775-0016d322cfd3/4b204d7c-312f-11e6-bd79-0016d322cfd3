...
extern UINT    mvRate;             //
extern UINT	   mvExtRate;		//isl3890, for 6~54Mb
//eason 20100407------------------
...
/*eason 20100407	wpa_supplicant_dbus_notify_state_change(wpa_s, state,
						wpa_s->wpa_state);*/
...
//eason 20100407 no sleep mode	wpa_drv_set_operstate(wpa_s, 1);
	} else if (state == WPA_DISCONNECTED || state == WPA_ASSOCIATING ||
		   state == WPA_ASSOCIATED) {
		wpa_s->new_connection = 1;
//eason 20100407 no sleep mode		wpa_drv_set_operstate(wpa_s, 0);
	}
...
{
//eason 20100407	#define SCAN_AP_LIMIT 128
#define SCAN_AP_LIMIT 32
	...
	os_memset(results, 0, SCAN_AP_LIMIT * sizeof(struct wpa_scan_result)); 	//eason 20100407
	...
	memset(res, 0,sizeof(*res));	//eason 20100407
	...
	memset(res->res, 0, num * sizeof(struct wpa_scan_res *));	//eason 20100407
	for (i = 0; i < num; i++) {
		...
		if (r == NULL)
			break;
		memset(r, 0, sizeof(*r) + ie_len);	//eason 20100407
...
extern struct wps_data *WPS_DATA_GLOBE;	//ZOT 20100712
			     ...
			     const u8 *buf, size_t len)
{
	...
	//eason 20100709
	if( (WPS_DATA_GLOBE != 0 ) && (WPS_DATA_GLOBE->state == RECV_M4) )
	{
		...
		plen -= 14;
		if( wps_parse_buf((buf+18), plen, &attr) == 0)
		{
			if(*attr.msg_type == WPS_M2)
		return;
		}
	}
...
{
	struct wpa_ssid *ssid;					//eason 20100407
	unsigned char	keyMaterial[40]={0};	//eason 20100407
	unsigned int 	target_rate = 0;		//eason 20100723
	unsigned int	fixed;				//eason 20100723
...
//eason 20100407------------------------------------------
	...
	if(strcmp(mvESSID,ANYESS))
	{	
		ssid->ssid = strdup(mvESSID);
		ssid->ssid_len = strlen(mvESSID);
	}
	else
	{
		ssid->ssid = strdup("");
	...
	set__adapter_mlmepriv(ssid->ssid ,ssid->ssid_len);
	...
	r8711_wx_set_rate(target_rate,fixed);
	
	//set proto
	if( mvAuthenticationType == 4)
		ssid->proto = WPA_PROTO_WPA;
	else if( mvAuthenticationType == 5)
		ssid->proto = WPA_PROTO_RSN;
	//set key_mgmt 
	...
	{	
		...
		//set psk
		ssid->passphrase = strdup(mvWPAPass);			
		cli();	
		if (strlen(ssid->passphrase) == 2 * PMK_LEN) {
			if (hexstr2bin((const char *) ssid->passphrase, ssid->psk,
				       PMK_LEN)) {				
				return -1;
			}
			ssid->psk_set = 1;
		} else if (strlen(ssid->passphrase) >= 8 && strlen(ssid->passphrase) < 2 * PMK_LEN) {
			if (ssid->passphrase) {					
	...
	else if( mvAuthenticationType == 2)
	{
		ssid->key_mgmt = WPA_KEY_MGMT_NONE;
...
//------test
	...
	if(mvWEPType == 0)
	{
		//wrqu->flags |= IW_ENCODE_DISABLED;
	}
	else if((mvWEPType == 1) || (mvWEPType == 2))
	{ 
		if(mvWEPType == 1)
		{
			switch(mvWEPKeySel)
			{	
				case 0:
					memcpy(ssid->wep_key[0], mvWEPKey1, 5);
					ssid->wep_key_len[0] = 5;
					break;	
				case 1:		
					memcpy(ssid->wep_key[1], mvWEPKey2, 5);
					ssid->wep_key_len[1] = 5;
					break;
				case 2:
					memcpy(ssid->wep_key[2], mvWEPKey3, 5);
					ssid->wep_key_len[2] = 5;
					break;
				case 3:	
					memcpy(ssid->wep_key[3], mvWEPKey4, 5);
					ssid->wep_key_len[3] = 5;
				default:				
					break;			
			}	
		}		
		else if(mvWEPType == 2)
		{
			...
			{
				case 0:
					memcpy(ssid->wep_key[0], mvWEP128Key, 13);
					ssid->wep_key_len[0] = 13;
					break;
				case 1:
					memcpy(ssid->wep_key[1], mvWEP128Key2, 13);
					ssid->wep_key_len[1] = 13;
					break;	
				case 2:
					memcpy(ssid->wep_key[2], mvWEP128Key3, 13);
					ssid->wep_key_len[2] = 13;
					break;	
				case 3:
					memcpy(ssid->wep_key[3], mvWEP128Key4, 13);
					ssid->wep_key_len[3] = 13;
					break;
				default:
					break;	
			}	
		}
		ssid->wep_tx_keyidx = mvWEPKeySel;
	}
done:	
//-------test
	...
	wpa_config_add_prio_network(wpa_s->conf, ssid);
...
extern void wpa_supplicant_ctrl_iface_receive(int sock, void *eloop_ctx,void *sock_ctx);

static int wpa_supplicant_init_iface2(struct wpa_supplicant *wpa_s)
{
	const char *ifname;
	struct wpa_driver_capa capa;
	int s = 1;	//eason 20100608
	...
	eloop_register_read_sock(s, wpa_supplicant_ctrl_iface_receive,wpa_s, NULL);	//eason 20100608
#if 0 //eason 20100407
	wpa_s->ctrl_iface = wpa_supplicant_ctrl_iface_init(wpa_s);
...
//eason 20100407	wpa_debug_open_file(params->wpa_debug_file_path);
	...
	memset(global->drv_priv, 0, global->drv_count * sizeof(void *));	//eason 20100407
	for (i = 0; wpa_supplicant_drivers[i]; i++) {
...
void wpa_supplicant_deinit(struct wpa_global *global)
{
	...
	os_free(global);
//eason 20100407	wpa_debug_close_file();
}
