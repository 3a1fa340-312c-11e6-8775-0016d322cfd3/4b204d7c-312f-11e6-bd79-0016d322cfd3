					  ...
					  const u8 *addr, const u8 *uuid_e)
{
	...
	if (!pbc) {
		...
		memset(pbc, 0, sizeof(*pbc));	//eason 20100407	
		   ...
		   const struct wps_registrar_config *cfg)
{
	...
	memset(reg, 0, sizeof(*reg));	//eason 20100407
	reg->wps = wps;
			  ...
			  const u8 *pin, size_t pin_len)
{
	...
	memset(p, 0, sizeof(*p));	//eason 20100407		
	if (uuid == NULL)
		p->wildcard_uuid = 1;
	else
	...
	memset(p->pin, 0, pin_len);	//eason 20100407
...
static int wps_get_dev_password(struct wps_data *wps)
{
	...
	if (wps->dev_password == NULL)
		return -1;
	memset(wps->dev_password, 0, pin_len);	//eason 20100407	
...
static int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		if (wps->new_psk == NULL)
			return -1;
		memset(wps->new_psk, 0, wps->new_psk_len);	//eason 20100407	
