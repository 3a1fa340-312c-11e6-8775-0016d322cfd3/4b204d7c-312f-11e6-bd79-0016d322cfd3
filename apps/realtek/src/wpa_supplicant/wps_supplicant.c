...
u32 Global_wps_state = 0;	//eason 20100608
extern Global_wpa_state = 0;//eason 20100727
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
	...
	Global_wps_state = 0;	//eason 20100608	
	Global_wpa_state = 1;//eason 20100727
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid) {
		...
		set__adapter_mlmepriv(wpa_s->conf->ssid->ssid ,wpa_s->conf->ssid->ssid_len);	//eason 20100713
		wpas_wps_reassoc(wpa_s, wpa_s->conf->ssid);	//eason 20100713				      
		return 1;
	}
				   ...
				   const struct wps_credential *cred)
{
	...
	if ((wpa_s->conf->wps_cred_processing == 1 ||
		...
		if (buf) {
			memset(buf, 0, blen);	//eason 20100407
	...
	if (ssid->ssid) {
		memset(ssid->ssid, 0, cred->ssid_len);	//eason 20100407		
	...
	if (ssid->key_mgmt == WPA_KEY_MGMT_PSK) {
		if (cred->key_len == 2 * PMK_LEN) {
			...
			//	Please see this function wlan_wsc_save_eep() in wps_dep.c.
			os_free(ssid->passphrase);
			ssid->passphrase = os_malloc(cred->key_len + 1);
			if (ssid->passphrase == NULL)
				return -1;
			memset(ssid->passphrase, 0, cred->key_len + 1);	//eason 20100407	
	...
	wpa_s->conf->ap_scan = DEFAULT_AP_SCAN;	//eason 20100727
	wlan_wsc_save_eep(cred, ssid);	//eason 20100608
...
{
       u32  u32wps_state = 3;    //   wps stop because of wps fail.
       
	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_FAIL "msg=%d", fail->msg);
	wpas_clear_wps(wpa_s);
	inform_driver_wps_state( wpa_s->drv_priv, u32wps_state );
}


static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
	u32     u32wps_state = 2;  //  wps stop because of wps success
...
static void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    u32   u32wps_state = 3;  // wps stop because of wps fail
	int   state = 0;		//eason 20100713
/*eason 20100608	wpa_printf(MSG_INFO, WPS_EVENT_TIMEOUT "Requested operation timed "
		   "out");*/
	wpas_clear_wps(wpa_s);	
	inform_driver_wps_state( wpa_s->drv_priv, u32wps_state );
	...
	if(state == FALSE)
		wpas_wps_reassoc(wpa_s, wpa_s->conf->ssid);
}
					      ...
					      int registrar, const u8 *bssid)
{
	...
	if(wpa_config_set_eap(ssid) < 0)
		return NULL;
	ssid->eap.identity = strdup(WSC_ID_ENROLLEE);		
	ssid->eap.identity_len = strlen(WSC_ID_ENROLLEE);	
	...
	if (bssid) {
		...
		for (i = 0; i < wpa_s->scan_res->num; i++) {
			...
			memset(ssid->ssid, 0, ie[1]);	//eason 20100407	
...
{
	struct wpa_ssid *ssid;
       u32   u32wps_state = 1;  // wps start
	   
	wpas_clear_wps(wpa_s);
	ssid = wpas_wps_add_network(wpa_s, 0, bssid);
	if (ssid == NULL)
		return -1;
//eason 20100608	wpa_config_set(ssid, "phase1", "\"pbc=1\"", 0);
	ssid->eap.phase1 = strdup("pbc=1");		//eason 20100608
			
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wpas_wps_timeout,
			       wpa_s, NULL);
	
	Global_wps_state = 1;	//eason 20100608			       
	inform_driver_wps_state( wpa_s->drv_priv, u32wps_state );
...
int wpas_wps_init(struct wpa_supplicant *wpa_s)
{
	...
	memset(wps, 0, sizeof(*wps));	//eason 20100407
...
//ZOT	wps->dev.rf_bands = WPS_RF_24GHZ | WPS_RF_50GHZ; /* TODO: config */
