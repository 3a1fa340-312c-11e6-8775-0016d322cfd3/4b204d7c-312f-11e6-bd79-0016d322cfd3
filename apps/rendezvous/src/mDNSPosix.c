...
#include "network.h" //Ron
#include "if.h" //Ron
extern int if_nametoindex(const char *ifname); //Ron
extern uint32 msclock( void ); //Ron
void swapIP ( struct sockaddr * ); //Ron
void swapPort ( struct sockaddr * ); //Ron
...
//#define ARM //Ron
#ifdef ARM
	swapIP((struct sockaddr*)&to);
	swapPort((struct sockaddr*)&to);
	to.sa_data[2]=251;
	to.sa_data[3]=0;
	to.sa_data[4]=0;
	to.sa_data[5]=224;
...
void SocketDataReady(mDNS *const m, PosixNetworkInterface *intf, int skt)
{
...
//Ron	packetLen = recvfrom_flags(skt, &packet, sizeof(packet), &flags, (struct sockaddr *) &from, &fromLen, &packetInfo);
	
	//Ron
    packetLen=-1;
    memset(&from,0,sizeof(from));
    memset(&packet,0,sizeof(packet));
    packetLen = recvfrom(skt, &packet,sizeof(packet),  0, (struct sockaddr *) &from, &fromLen);
...
//Ron	if (packetLen >= 0)
	...
	{
		//Ron
//    	swapIP((struct sockaddr*)&from);
//    	swapPort((struct sockaddr*)&from);

///////   we need emulate unix's packetInfo struct for recvfrom_flags function
    	memcpy(&packetInfo.ipi_addr,&from,sizeof(from));
    	strcpy(packetInfo.ipi_ifname,"eth9");
    	packetInfo.ipi_ifindex=if_nametoindex(packetInfo.ipi_ifname); // just test , dont use
...
//Ron		#if HAVE_BROKEN_RECVDSTADDR || (!defined(IP_PKTINFO) && !defined(IP_RECVDSTADDR))
...
{
	MakeDomainLabelFromLiteralString(namelabel, _BoxName );
}
...
mDNSlocal void GetUserSpecifiedRFC1034ComputerName(domainlabel *const namelabel)
{
    int len = 0;
   	...
   	if ( EEPROM_Data.RENVServiceName[0]==NULL) {
        strcpy(EEPROM_Data.RENVServiceName,EEPROM_Data.BoxName);
        strcpy(mvRENVServiceName,EEPROM_Data.BoxName);
        WriteToEEPROM(&EEPROM_Data);
    }
    strcpy(&namelabel->c[1],_BoxName);
...
if (intf->multicastSocket   != -1) close(intf->multicastSocket);
if (intf->multicastSocketv6 != -1) close(intf->multicastSocketv6);
	free(intf);
}

// Grab the first interface, deregister it, free it, and repeat until done.
//static void ClearInterfaceList(mDNS *const m)
extern void ClearInterfaceList(mDNS *const m)
{
	assert(m != NULL);
...
//		#else
//Ron			#error This platform has no way to avoid address busy errors on multicast.
//		#endif
...
//Ron				err = setsockopt(*sktPtr, IPPROTO_IP, IP_PKTINFO, &kOn, sizeof(kOn));
...
//Ron			err = setsockopt(*sktPtr, IPPROTO_IP, IP_ADD_MEMBERSHIP, &imr, sizeof(imr));
...
//		if (err == 0)
//		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IP, IP_MULTICAST_IF, &((struct sockaddr_in*)intfAddr)->sin_addr, sizeof(struct in_addr));
...
//		if (err == 0)
//		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IP, IP_TTL, &kIntTwoFiveFive, sizeof(kIntTwoFiveFive));
...
//		if (err == 0)
//		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IP, IP_MULTICAST_TTL, &kByteTwoFiveFive, sizeof(kByteTwoFiveFive));
...
//		}
		...
		if (err == 0)
		{	
			memset((int8 *) &bindAddr, 0, sizeof(bindAddr));
...
//Ron				err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_PKTINFO, &kOn, sizeof(kOn));
...
//Ron			err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_JOIN_GROUP, &imr6, sizeof(imr6));
			if (err < 0)
			{
				err = errno;
				verbosedebugf("IPV6_JOIN_GROUP %.16a on %d failed.\n", &imr6.ipv6mr_multiaddr, imr6.ipv6mr_interface);
//Ron				perror("setsockopt - IPV6_JOIN_GROUP");
			}
		}
...
//Ron			err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_MULTICAST_IF, &multicast_if, sizeof(multicast_if));
//			if (err < 0) { err = errno; perror("setsockopt - IPV6_MULTICAST_IF"); }
		...
		if (err == 0)
		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_V6ONLY, &kOn, sizeof(kOn));
//			if (err < 0) { err = errno; perror("setsockopt - IPV6_V6ONLY"); }
		...
		if (err == 0)
		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &kIntTwoFiveFive, sizeof(kIntTwoFiveFive));
//			if (err < 0) { err = errno; perror("setsockopt - IPV6_UNICAST_HOPS"); }
		...
		if (err == 0)
		{
//Ron			err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &kByteTwoFiveFive, sizeof(kByteTwoFiveFive));
//			if (err < 0 && errno == EINVAL)
...
//			bindAddr6.sin6_addr.s_addr = IN6ADDR_ANY_INIT; // Want to receive multicasts AND unicasts on this socket
	...
	if (err == 0)
	{
//Ron		err = fcntl(*sktPtr, F_GETFL, 0);
		if (err < 0) err = errno;
		else
		{
...
//	if (err != 0 && *sktPtr != -1) { assert(close(*sktPtr) == 0); *sktPtr = -1; }
	if (err != 0 && *sktPtr != -1) { close(*sktPtr);  *sktPtr = -1; }
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
	...
	GetUserSpecifiedFriendlyComputerName(&m->nicelabel);
        if (m->nicelabel.c[0] == 0) MakeDomainLabelFromLiteralString(&m->nicelabel, "ZOT PRINTER SERVER");

	// Set up the RFC 1034-compliant label
	m->hostlabel.c[0] = 0;
	GetUserSpecifiedRFC1034ComputerName(&m->hostlabel);
        if (m->hostlabel.c[0] == 0) MakeDomainLabelFromLiteralString(&m->hostlabel, "ZOT PRINTER SERVER");
...
//Ron	gettimeofday(&tv, NULL);
    ...
    tempTime = msclock();	
    tv.tv_sec = (tempTime/1000); //Ron
    tv.tv_usec = (tempTime%1000)*1000; //Ron
	
	//return( (tv.tv_sec << 10) | (tv.tv_usec * 16 / 15625) );
	return( (tv.tv_sec << 10) | (tv.tv_usec * 16 / 13200 ) ); //13200
}
...
void swapIP ( struct sockaddr * a){
         char temp;

         temp=a->sa_data[2];
         a->sa_data[2]=a->sa_data[5];
         a->sa_data[5]=temp;

         temp=a->sa_data[3];
         a->sa_data[3]=a->sa_data[4];
         a->sa_data[4]=temp;
}

void swapPort (struct sockaddr *a){
         char temp;
         temp=a->sa_data[0];
         a->sa_data[0]=a->sa_data[1];
         a->sa_data[1]=temp;
}
