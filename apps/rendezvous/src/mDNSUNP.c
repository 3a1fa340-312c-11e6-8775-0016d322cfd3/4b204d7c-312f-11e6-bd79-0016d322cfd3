...
{
    struct ifi_info   *ifihead, **ifipnext;
    struct sockaddr  *addr,*brdaddr,*dstaddr;

    ifipnext = &ifihead;

        ifihead=(struct ifi_info *) malloc(sizeof(struct ifi_info));   memset(ifihead,0,sizeof(struct ifi_info));

        addr   = (struct sockaddr *) malloc(sizeof(struct sockaddr));  memset(addr,0,sizeof(struct sockaddr));
        brdaddr= (struct sockaddr *) malloc(sizeof(struct sockaddr));  memset(brdaddr,0,sizeof(struct sockaddr));
        dstaddr= (struct sockaddr *) malloc(sizeof(struct sockaddr));  memset(dstaddr,0,sizeof(struct sockaddr));
        memcpy(addr->sa_data+2,EEPROM_Data.BoxIPAddress, 4);

        ifihead->ifi_addr=addr;
        ifihead->ifi_brdaddr=brdaddr;
        ifihead->ifi_dstaddr=dstaddr;
        ifihead->ifi_next =NULL;

        strcpy(ifihead->ifi_name,"eth9");
        strcpy(ifihead->ifi_haddr,"test");

        ifihead->ifi_addr->sa_family=AF_INET;
        ifihead->ifi_index=1;
        ifihead->ifi_flags=4163;
//        ifihead->ifi_myflags=0;
...
//Ron    if ( (n = recvmsg(fd, &msg, *flagsp)) < 0)
//       return(n);
...
int if_nametoindex(const char *ifname){
	struct ifi_info *intfList      = get_ifi_info(AF_INET, 1);
    struct ifi_info *intf;
    int interFacesIndex=1;
    
    for (intf = intfList; intf; intf = intf->ifi_next,interFacesIndex++){
    	if (strncmp(intfList->ifi_name[0],*ifname,strlen(intfList->ifi_name))==0) {
//      	if (strcmp(intfList->ifi_name[0],*ifname)==0) {
