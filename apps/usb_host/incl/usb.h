...
//ZOT716u2 #define HZ								900
#define HZ								1000	//ZOT716u2

#define DECLARE_WAIT_QUEUE_HEAD( q )	volatile int q = 0
#define DECLARE_MUTEX( m )	cyg_mutex_t m //ZOT==> #define DECLARE_MUTEX( m )				int m = 0
#define init_MUTEX			__spin_lock_init	//ZOT==> #define init_MUTEX( m )					*m = 0;

#define cpu_to_le16p( v )				NGET16( (BYTE *)v )
#define cpu_to_le16( v )				( v )
#define cpu_to_le32( v )				( v )
#define le16_to_cpu( v )				( v )
#define le16_to_cpus( v )
#define le16_to_cpup( s )				( *s )

#define atomic_set( a, v )				(*a = v)
#define atomic_inc( cnt )				((*cnt)++)
#define atomic_dec( cnt )				((*cnt)--)
#define atomic_dec_and_test( cnt )		( --(*cnt) == 0 )
//#define down( s )						{ while( *s == 1 ) kwait( NULL ); *s = 1; }	//635u2
//#define up( s )							*s = 0	//635u2
//ZOT==> #define __spin_lock_init( s )				*s = 0		//635u2
//ZOT==> #define spin_lock_irqsave( s, f )		{ f = dirps(); *s = 1; }
//ZOT==> #define spin_unlock_irqrestore( s, f )  { *s = 0; restore(f); }
//ZOT==> #define spin_lock						down
//ZOT==> #define spin_unlock						up
#define init_waitqueue_head( e )		*e = 0;
#define wake_up_interruptible( e )		{ *e = 1; }
#define wake_up( e )					{ *e = 1; }
//#define interruptible_sleep_on( e )		{ kwait( NULL ); while( *e == 0 ) kwait(NULL); *e = 0; }
//#define sleep_on( e )					{ kwait( NULL ); while( *e == 0 ) kwait(NULL); *e = 0; }
//take off kwait(NULL) from wmb() and mb() to speed-up
#define wmb()							
#define mb()							

#define get_free_page( flag )			malloc( 4096 )
#define free_page( ptr )				free( ptr )
#define virt_to_bus( addr )				((uint32)( addr ))
#define bus_to_virt( addr )				((uint32)( addr ))
#define copy_from_user					memcpy
#define copy_to_user					memcpy
#define ioremap( ptr, size )			( ptr | 0xC0000000 )	//635u2 for pci interface

typedef uint8				   			__u8;
typedef uint16                  		__u16;
typedef uint32							__u32;
typedef spinlock_t __spinlock_t;//ZOT==> typedef uint32							__spinlock_t;	//635u2
typedef uint32							__wait_queue_head_t;	//635u2
typedef int								__atomic_t;	//635u2
...
typedef struct e_devrequest devrequest;
...
#define USB_DEVICE_INFO(cl,sc,pr) \
	USB_DEVICE_ID_MATCH_DEV_INFO, 0, 0, 0, 0, (cl), (sc), (pr)
...
struct usb_driver {
	...
	cyg_mutex_t serialize;	//ZOT==> int serialize;
...
#define URB_NO_INTERRUPT	0x0080	/* HINT: no non-error interrupt needed */	//635u2 for usb2.0
...
typedef struct urb
{
	__spinlock_t lock;		// lock for the URB	//635u2
...
struct usb_bus {
	...
	char *bus_name;			/* stable id (PCI slot_name etc) */	//635u2 for usb2.0
...
struct usb_device {
	...
	struct usb_tt *tt;		/* usb1.1 device on usb2.0 bus */	//635u2 for usb2.0
	int ttport;			/* device/hub port on that tt */
//ZOT
	int		hub_level_count;
	int		printer_port;

	__atomic_t refcnt;		/* Reference count */
	cyg_mutex_t serialize;	//ZOT==>	int serialize;
	int slow;
