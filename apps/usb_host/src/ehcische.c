 ...
 */
//original Source	//635u2 //define in u2hcd.h already
...
extern int ehci_get_frame (struct usb_hcd *hcd);	//635u2
...
//original Source	//635u2	
//			if (q->sitd->hw_uframe & cpu_to_le32 (1 << (8 + uframe))) {
			if (q->sitd->hw_uframe & (1 << (8 + uframe))) {
				if (temp)
					usecs += HS_USECS (0);
				else
					usecs += HS_USECS (188);
...
//			dbg ("interrupt urb already queued");
			if (qh->qh_state==QH_STATE_LINKED) ppause(2);
//			status = -EBUSY;
			spin_unlock_irqrestore (&ehci->lock, &flags);	//ZOT==> spin_unlock_irqrestore (&ehci->lock, flags);
			...
			if (unlikely (!usb_gettoggle (urb->dev, epnum & 0x0f, !(epnum & 0x10)))) {
				qh->hw_token |=__constant_cpu_to_le32 (QTD_TOGGLE);
				usb_settoggle (urb->dev, epnum & 0x0f, !(epnum & 0x10), 1);
			}
...
intr_complete (
	...
	period = (urb->dev->speed == USB_SPEED_HIGH) ? urb->interval : (urb->interval << 3);	//719AW
...
//719AW
	if (unlikely (list_empty (&qh->qtd_list)))
	{
		spin_unlock_irqrestore (&ehci->lock, &flags);	//eason 20100819
		intr_deschedule (ehci, frame, qh, period);
		spin_lock_irqsave (&ehci->lock, &flags);		//eason 20100819
	}
	return flags;
}
...
//		itd->hw_bufp_hi [i] = cpu_to_le32 ((u32)(temp >> 32));
		itd->hw_bufp [i] = ((u32) temp);
		itd->hw_bufp_hi [i] = ((u32)(temp >> 32));
		temp += 0x1000;
	}
	itd->buf_dma = dma;
...
//	itd->hw_bufp [2] |= cpu_to_le32 (multi);
	itd->hw_bufp [0] |= ((epnum << 8) | urb->dev->devnum);
	itd->hw_bufp [1] |= (buf1);
	itd->hw_bufp [2] |= (multi);
...
//		    ? PCI_DMA_FROMDEVICE
//		    : PCI_DMA_TODEVICE);
	buf_dma = urb->transfer_buffer;	//635u2
	if (buf_dma == 0)
		return -ENOMEM;
...
//		itd = pci_pool_alloc (ehci->itd_pool, mem_flags, &itd_dma);\
		itd = ehci_kmalloc (sizeof(struct ehci_itd));	//635u2
		if (!itd) {
			status = -ENOMEM;
			goto fail;
		}
...
//	ehci->periodic [frame] = cpu_to_le32 (itd->itd_dma) | Q_TYPE_ITD;
	ehci->periodic [frame] = (itd->itd_dma) | Q_TYPE_ITD;
}
...
int itd_submit (struct ehci_hcd *ehci, struct urb *urb, int mem_flags)
{
	int		status;
	unsigned long	flags;
#ifdef EHCI_DEBUG	//635u2	
...
//	sitd = pci_pool_alloc (ehci->sitd_pool, mem_flags, &dma);
	sitd = ehci_kmalloc (sizeof (struct ehci_sitd));
	if (!sitd)
		return sitd;
	sitd->urb = urb;
	length = urb->iso_frame_desc [index].length;
...
//	ptr = cpu_to_le32 (sitd->sitd_dma | 2);	// type 2 == sitd
	ptr = (sitd->sitd_dma | 2);	// type 2 == sitd
	if (ehci->pshadow [frame].ptr) {
...
int sitd_submit (struct ehci_hcd *ehci, struct urb *urb, int mem_flags)
{
...
void scan_periodic (struct ehci_hcd *ehci)
{
