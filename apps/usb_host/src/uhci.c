...
//PCI module
extern uint32 * pci_alloc_framelist(uint32 * framelist_dma);
extern uint32 * pci_alloc_uhci_desc(uint32 * dma_addr, int iso_init);
extern void pci_free_uhci_desc(uint32 addr);
...
extern uint32 pciHost_ConfigRead(uint32 addr,uint8 func);
extern void pciHost_ConfigWrite(uint32 addr, uint32 vale,uint8 func);
void pciHost_IORead(uint32 addr, uint32* buf, uint32 size, uint32 bar);
void pciHost_IOWrite(uint32 addr, uint32* buf, uint32 size, uint32 bar);
extern inline int dirps();
extern inline void restore(cyg_uint32  old_intr);
extern NSET32( uint8 *pDest, uint32 value );
_static int rh_init_int_timer (struct urb *urb);
// How much URBs with ->next are walked
#define MAX_NEXT_COUNT 2048

static uhci_t *GLOBAL_UHCI = NULL;

/* used by userspace UHCI data structure dumper */
uhci_t **uhci_devices = &GLOBAL_UHCI;

cyg_handle_t URH_SysClk;
cyg_handle_t URH_Counter;
cyg_handle_t URH_Alarm; 
cyg_alarm URH_timerAlarm;
struct urb *GLOBAL_UHCI_RH_URB;
#if 0	//ZOT716u2
#define __SQRAM__   __attribute__ ((section (".sqbuf")))
uint16 UHCI_PCI_Word_Buf __SQRAM__;
uint32 UHCI_PCI_DWord_Buf __SQRAM__;
#endif	//ZOT716u2
uint32 UHCI_IO_BASE_ADDR;
static uint16 inw (uint32 addr)
{	
	uint32 offset = addr - UHCI_IO_BASE_ADDR;
	pciHost_IORead(offset, (uint32*)&UHCI_PCI_Word_Buf, 2, 4);
	return (NGET16((uint8*)&UHCI_PCI_Word_Buf));
}

static void outw (uint16 data, uint32 addr)
{
	uint32 offset = addr - UHCI_IO_BASE_ADDR;
	UHCI_PCI_Word_Buf = data;
	pciHost_IOWrite(offset, (uint32*)&UHCI_PCI_Word_Buf, 2, 4);
}

static uint32 inl (uint32 addr)
{	
	uint32 offset = addr - UHCI_IO_BASE_ADDR;
	pciHost_IORead(offset, (uint32*)&UHCI_PCI_DWord_Buf, 4, 4);
	return (NGET32((uint8*)&UHCI_PCI_DWord_Buf));
}

static void outl (uint32 data, uint32 addr)
{
	uint32 offset = addr - UHCI_IO_BASE_ADDR;
	UHCI_PCI_DWord_Buf = data;
	pciHost_IOWrite(offset, (uint32*)&UHCI_PCI_DWord_Buf, 4, 4);
}

static __inline__ void uhci_wait_ms(unsigned int ms)
{
//ZOT716u2	volatile int delay;
//ZOT716u2	for( delay = 0; delay < ms * 1500; delay++ );
	udelay(ms * 1000);
}

//************************************************************
//
//For Marvell 88w8510 PCI API
//
//************************************************************

static void * uhci_td_alloc(uint32 *dma_handle , int iso_init)
{
	uhci_desc_t * new;
	new = kmalloc(sizeof(uhci_desc_t),GFP_DMA | GFP_ATOMIC);
	if (!new)
		return -1;
	new->hw.td = pci_alloc_uhci_desc(dma_handle,iso_init);
	
	return new;
}

static void * uhci_qh_alloc(uint32 *dma_handle)
{
	uhci_desc_t * new;
	new = kmalloc(sizeof(uhci_desc_t),GFP_DMA | GFP_ATOMIC);
	if (!new)
		return -1;
	new->hw.qh = pci_alloc_uhci_desc(dma_handle,0);
	
	return new;
}

static void * uhci_desc_free(uhci_desc_t *element)
{
	pci_free_uhci_desc(element->hw.td);
	kfree(element, 0);
}
...
_static void enable_desc_loop(uhci_t *s, struct urb *urb)
{
	...
	spin_lock_irqsave (&s->qh_lock, &flags);		//ZOT==> spin_lock_irqsave (&s->qh_lock, flags);
//pci	s->chain_end->hw.qh.head&=cpu_to_le32(~UHCI_PTR_TERM);
	s->chain_end->hw.qh->head&=cpu_to_le32(~UHCI_PTR_TERM);
	mb();
	s->loop_usage++;
	((urb_priv_t*)urb->hcpriv)->use_loop=1;
	spin_unlock_irqrestore (&s->qh_lock, &flags);//ZOT==>			spin_unlock_irqrestore (&s->qh_lock, flags);
}
/*-------------------------------------------------------------------*/
...
#endif
//	armond_printf("UHCI Set URB status:EINPROGRESS\n");
...
_static void queue_urb (uhci_t *s, struct urb *urb)
{
	...
	spin_lock_irqsave (&s->urb_list_lock, &flags);	//ZOT==> spin_lock_irqsave (&s->urb_list_lock, flags);
	queue_urb_unlocked(s,urb);
	spin_unlock_irqrestore (&s->urb_list_lock, &flags);//ZOT==>	spin_unlock_irqrestore (&s->urb_list_lock, flags);
}
/*-------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------*/
_static int alloc_td (uhci_t *s, uhci_desc_t ** new, int flags, int iso_init)
{
	dma_addr_t dma_handle;

//os	*new = pci_pool_alloc(s->desc_pool, GFP_DMA | GFP_ATOMIC, &dma_handle);
//os	*new = kmalloc(sizeof(uhci_desc_t),GFP_DMA | GFP_ATOMIC);
	*new =  uhci_td_alloc(&dma_handle, iso_init);
	if (!*new)
		return -ENOMEM;	
...
_static void append_qh(uhci_t *s, uhci_desc_t *td, uhci_desc_t* qh, int  flags)
{
	...
	spin_lock_irqsave (&s->td_lock, &xxx);	//ZOT==> spin_lock_irqsave (&s->td_lock, xxx);

	set_td_link(td, qh->dma_addr | (flags & UHCI_PTR_DEPTH) | UHCI_PTR_QH);
       
	mb();
	spin_unlock_irqrestore (&s->td_lock, &xxx);	//ZOT==>spin_unlock_irqrestore (&s->td_lock, xxx);
}
/*-------------------------------------------------------------------*/
...
_static int insert_td_horizontal (uhci_t *s, uhci_desc_t *td, uhci_desc_t* new)
{
	...
	spin_lock_irqsave (&s->td_lock, &flags);	//ZOT==> spin_lock_irqsave (&s->td_lock, flags);

	next = list_entry (td->horizontal.next, uhci_desc_t, horizontal);
	list_add (&new->horizontal, &td->horizontal);
//pci	new->hw.td.link = td->hw.td.link;
	new->hw.td->link = td->hw.td->link;
	set_td_link(td, new->dma_addr);
	mb();
	spin_unlock_irqrestore (&s->td_lock, &flags);//ZOT==> 	spin_unlock_irqrestore (&s->td_lock, flags);	
...
//pci			prev->hw.td.link = element->hw.td.link;
			prev->hw.td->link = element->hw.td->link;
		else
//pci			prev->hw.qh.element = element->hw.td.link;
...
{
//os	pci_pool_free(s->desc_pool, element, element->dma_addr);
//os	kfree (element);
	uhci_desc_free(element);
	return 0;
}
/*-------------------------------------------------------------------*/
...
//pci		if (!(old->hw.qh.head & cpu_to_le32(UHCI_PTR_TERM)))
...
_static int unlink_qh (uhci_t *s, uhci_desc_t *element)
{
	...
	prev = list_entry (element->horizontal.prev, uhci_desc_t, horizontal);
//pci	prev->hw.qh.head = element->hw.qh.head;
...
{
//pci	td->hw.td.status = cpu_to_le32(status);
//pci	td->hw.td.info = cpu_to_le32(info);
//pci	td->hw.td.buffer = cpu_to_le32(buffer);
...
//	dbg("cleanup_skel");
	...
	if (s->framelist)
//os		pci_free_consistent(s->uhci_pci, PAGE_SIZE,
//os				    s->framelist, s->framelist_dma);
//os			kfree(s->framelist);
		uhci_desc_free(s->framelist);
	...
	if (s->desc_pool) {
//os		pci_pool_destroy(s->desc_pool);
		s->desc_pool = NULL;
	}
...
//os	s->framelist = pci_alloc_consistent(s->uhci_pci, PAGE_SIZE,
//os					    &s->framelist_dma);

//os	s->framelist = kmalloc(PAGE_SIZE,0);

	s->framelist = pci_alloc_framelist(&s->framelist_dma);
...
//os	s->desc_pool = pci_pool_create("uhci_desc", s->uhci_pci,
//os				       sizeof(uhci_desc_t), 16, 0,
//os				       GFP_DMA | GFP_ATOMIC);	
...
//	dbg("allocating qh: chain_end");
	...
	if (alloc_td (s, &td, 0,0))
		goto init_skel_cleanup;
	
	fill_td (td, 0 * TD_CTRL_IOC, 0, 0); // generate 1ms interrupt (enabled on demand)
	insert_td (s, qh, td, 0);
//pci	qh->hw.qh.element &= cpu_to_le32(~UHCI_PTR_TERM); // remove TERM bit
...
//		dbg("framelist[%i]=%x",n,le32_to_cpu(s->framelist[n]));
		if ((n&127)==127) 
//pci			((uhci_desc_t*) s->iso_td[n])->hw.td.link = cpu_to_le32(s->int_chain[0]->dma_addr);
...
//	dbg("uhci_submit_control start");
	...
	if (alloc_td (s, &td, UHCI_PTR_DEPTH * depth_first,0))		// get td for setup stage
	{
		delete_qh (s, qh);
		return -ENOMEM;
	}
...
//		    sp[0],sp[1],sp[2],sp[3],sp[4],sp[5],sp[6],sp[7]);
	...
	while (len > 0) {
		...
		if (alloc_td (s, &td, UHCI_PTR_DEPTH * depth_first,0))
	...
	if (alloc_td (s, &td, UHCI_PTR_DEPTH,0))
		goto fail_unmap_enomem;
...
//pci	qh->hw.qh.element &= cpu_to_le32(~UHCI_PTR_TERM);
...
//pci		qh->hw.qh.element &= cpu_to_le32(~UHCI_PTR_TERM);    // arm QH
...
//os		pci_dma_sync_single(s->uhci_pci, urb_priv->setup_packet_dma,
//os				    sizeof(struct usb_ctrlrequest), PCI_DMA_TODEVICE);
			pci_data_sync(urb_priv->setup_packet_dma, sizeof(devrequest),
						PCI_DMA_TODEVICE,1,0);

	if (urb_priv->transfer_buffer_dma)
//os		pci_dma_sync_single(s->uhci_pci, urb_priv->transfer_buffer_dma,
//os				    urb->transfer_buffer_length,
//os				    usb_pipein(urb->pipe) ?
//os				    PCI_DMA_FROMDEVICE :
//os				    PCI_DMA_TODEVICE);
			pci_data_sync(urb_priv->transfer_buffer_dma, urb->transfer_buffer_length,
						(usb_pipein(urb->pipe) ?
				    	PCI_DMA_FROMDEVICE :
				    	PCI_DMA_TODEVICE),0,0);
...
{
	if (urb_priv->setup_packet_dma) {
//os		pci_unmap_single(s->uhci_pci, urb_priv->setup_packet_dma,
//os				 sizeof(struct usb_ctrlrequest), PCI_DMA_TODEVICE);
		urb_priv->setup_packet_dma = pci_data_unmap(urb_priv->setup_packet_dma, sizeof(devrequest),
						PCI_DMA_TODEVICE,1,0);
	}
	if (urb_priv->transfer_buffer_dma) {
//os		pci_unmap_single(s->uhci_pci, urb_priv->transfer_buffer_dma,
//os				 urb->transfer_buffer_length,
//os				 usb_pipein(urb->pipe) ?
//os				 PCI_DMA_FROMDEVICE :
//os				 PCI_DMA_TODEVICE);
		urb_priv->transfer_buffer_dma = pci_data_unmap(urb_priv->transfer_buffer_dma, urb->transfer_buffer_length,
						(usb_pipein(urb->pipe) ?
				    	PCI_DMA_FROMDEVICE :
				    	PCI_DMA_TODEVICE),0,0);
	}
...
_static int uhci_submit_urb (struct urb *urb)
{
	...
	if (!s->running)
		return -ENODEV;
//	armond_printf("UHCI submit urb\n");
//_____________________________________________________________	
	if (s->rh.send) {	
//		armond_printf("my stop UHCI Detcet\n");
		cyg_alarm_delete(URH_Alarm);
		cyg_clock_delete(URH_SysClk);
		cyg_counter_delete(URH_Counter);
	}
//_____________________________________________________________	
...
//_____________________________________________________________
		if (s->rh.send) {
//			armond_printf("my start UHCI Detcet\n");
			rh_init_int_timer(GLOBAL_UHCI_RH_URB);
		}
//_____________________________________________________________		
		return -EMSGSIZE;
	}
...
//os		urb_priv->setup_packet_dma = pci_map_single(s->uhci_pci, urb->setup_packet,
//os							    sizeof(struct usb_ctrlrequest), PCI_DMA_TODEVICE);
			urb_priv->setup_packet_dma = pci_data_map(urb->setup_packet, sizeof(devrequest),
									PCI_DMA_TODEVICE, 1, 0);
//			armond_printf("UHCI urb_priv->setup_packet_dma = %x\n",urb_priv->setup_packet_dma);
		}
	if (urb->transfer_buffer_length){
//os		urb_priv->transfer_buffer_dma = pci_map_single(s->uhci_pci,
//os							       urb->transfer_buffer,
//os							       urb->transfer_buffer_length,
//os							       usb_pipein(urb->pipe) ?
//os							       PCI_DMA_FROMDEVICE :
//os							       PCI_DMA_TODEVICE);
			urb_priv->transfer_buffer_dma = pci_data_map(urb->transfer_buffer,
									urb->transfer_buffer_length,
									(usb_pipein(urb->pipe) ?
							    	PCI_DMA_FROMDEVICE :
							    	PCI_DMA_TODEVICE), 0, 0);
//			armond_printf("UHCI urb_priv->transfer_buffer_dma = %x\n",urb_priv->transfer_buffer_dma);
		}
	if (type == PIPE_BULK) {
...
//		armond_printf("Start UHCI submit BULK urb\n");
		...
		case PIPE_INTERRUPT:
			...
			break;
		case PIPE_CONTROL:
//			armond_printf("Start UHCI submit CONTRO urb\n");		
...
_static int rh_send_irq (struct urb *urb)
{
	...
	__u16 data = 0;
	uint32 temp32_1,temp32_2;

	//________________###test###_____________________//
	//Read Frame List number
	temp32_1 = inw (io_addr + USBPORTSC1);
	
	temp32_2 = inw (io_addr + USBPORTSC2);
...
//os _static void rh_int_timer_do (unsigned long ptr)
_static void rh_int_timer_do (cyg_handle_t handle, cyg_addrword_t ptr)
{
	int len;
	struct urb *urb = (struct urb*) ptr;
	uhci_t *uhci;
	uint32 uhci_c_state;
	uint16 stsval,cmdval;
	uint32 temp32;
	
	cli();
	
	/* Ron Add, for monitor GPIO13 6/7/2004*/
	if ((*(volatile unsigned long *)(0x8000d010) & 0x2000) ==0){ //GPIO13 still Pull low 
		ApEvtMsg_t *dummymsg;
		static int errcounter =0;
		
		uhci_interrupt(dummymsg);
		errcounter++;	
	}
	
	//________________###test###_____________________//
	//Read Frame List number
	temp32 = inw (UHCI_IO_BASE_ADDR + USBFRNUM);
	
	uhci_c_state = pciHost_ConfigRead(0x04, 0);
	
	stsval = inw (UHCI_IO_BASE_ADDR + USBSTS);
	cmdval = inw (UHCI_IO_BASE_ADDR + USBCMD);
	//_______________________________________________//
...
//os	init_timer (&uhci->rh.rh_int_timer);
//os	uhci->rh.rh_int_timer.function = rh_int_timer_do;
//os	uhci->rh.rh_int_timer.data = (unsigned long) urb;
//os	uhci->rh.rh_int_timer.expires = jiffies + (HZ * 20) / 1000;
//os	add_timer (&uhci->rh.rh_int_timer);

    /* Attach the timer to the real-time clock */    
    URH_SysClk = cyg_real_time_clock();

	GLOBAL_UHCI_RH_URB = urb;

    cyg_clock_to_counter(URH_SysClk, &URH_Counter);

    cyg_alarm_create(URH_Counter, (cyg_alarm_t *)rh_int_timer_do,
                     (cyg_addrword_t) urb,
                     &URH_Alarm, &URH_timerAlarm);

    /* This creates a periodic timer */
    cyg_alarm_initialize(URH_Alarm, cyg_current_time() + interval, 0); //every setting trigger once
...
_static int rh_submit_urb (struct urb *urb)
{
	...
	unsigned int pipe = urb->pipe;
//os	struct usb_ctrlrequest *cmd = (struct usb_ctrlrequest *) urb->setup_packet;
...
//		dbg("Root-Hub submit IRQ: every %d ms", urb->interval);
		...
		uhci->rh.send = 1;
		uhci->rh.interval = urb->interval;
//		armond_printf("UHCI start RH Detect\n");
		rh_init_int_timer (urb);
		return 0;
	}

//	armond_printf("UHCI RH submit\n");

//os	bmRType_bReq = cmd->bRequestType | cmd->bRequest << 8;
//os	wValue = le16_to_cpu (cmd->wValue);
//os	wIndex = le16_to_cpu (cmd->wIndex);
//os	wLength = le16_to_cpu (cmd->wLength);
...
//		*(__u16 *) data = cpu_to_le16 (1);
		NSET16( data, cpu_to_le16 (1) );
		OK (2);
	case RH_GET_STATUS | RH_INTERFACE:
//		*(__u16 *) data = cpu_to_le16 (0);
		NSET16( data, cpu_to_le16 (0) );
		OK (2);
...
//		*(__u16 *) data = cpu_to_le16 (status);
		NSET16( data, cpu_to_le16 (status) );
//		*(__u16 *) ((uint8 *)data + 2) = cpu_to_le16 (cstatus);
		NSET16( ((uint8 *)data + 2), cpu_to_le16 (cstatus) );
		OK (4);
...
//		dbg("Root-Hub unlink IRQ");
		uhci->rh.send = 0;
//os		del_timer (&uhci->rh.rh_int_timer);
	}
	return 0;
}
/*-------------------------------------------------------------------*/
...
//		dbg("URB to correct %p\n", urb);
		...
		for (; p != &qh->vertical; p = p->next) {
			td = list_entry (p, uhci_desc_t, vertical);
//pci			td->hw.td.info^=cpu_to_le32(1<<TD_TOKEN_TOGGLE);
...
//pci	    ((qh->hw.qh.element == cpu_to_le32(UHCI_PTR_TERM)) || !is_td_active(desc)))
	...
	for (; p != &qh->vertical; p = p->next) {
		...
		if (is_td_active(desc)) {	// do not process active TDs
			if (mode == CLEAN_TRANSFER_DELETION_MARK) // if called from async_unlink
				uhci_clean_transfer(s, urb, qh, CLEAN_TRANSFER_DELETION_MARK);
//			armond_printf("UHCI Process control urb but have active decriptor\n");	
			return ret;
		}
	
//pci		actual_length = uhci_actual_length(le32_to_cpu(desc->hw.td.status));		// extract transfer parameters from TD
//pci		maxlength = (((le32_to_cpu(desc->hw.td.info) >> 21) & 0x7ff) + 1) & 0x7ff;
//pci		status = uhci_map_status (uhci_status_bits (le32_to_cpu(desc->hw.td.status)), usb_pipeout (urb->pipe));
...
//pci		else if ((le32_to_cpu(desc->hw.td.info) & 0xff) != USB_PID_SETUP)
...
//pci				if (uhci_packetid(le32_to_cpu(last_desc->hw.td.info)) == USB_PID_OUT) {
...
//					armond_printf("UHCI Process control urb is out pid\n");
					return 0;
				}
			}
			// all other cases: short read is OK
//pci			data_toggle = uhci_toggle (le32_to_cpu(desc->hw.td.info));
...
//pci			if (data_toggle == uhci_toggle (le32_to_cpu(first_td->hw.td.info))) {
...
//	armond_printf("UHCI Set URB status:%d\n",status);
	urb->status = status;
...
//pci		if (!(desc->hw.td.status & cpu_to_le32(TD_CTRL_IOC))) {
...
//pci		actual_length = uhci_actual_length(le32_to_cpu(desc->hw.td.status));
//pci		status = uhci_map_status (uhci_status_bits (le32_to_cpu(desc->hw.td.status)), usb_pipeout (urb->pipe));
		actual_length = uhci_actual_length(le32_to_cpu(desc->hw.td->status));
		status = uhci_map_status (uhci_status_bits (le32_to_cpu(desc->hw.td->status)), usb_pipeout (urb->pipe));
...
//pci				desc->hw.td.info &= cpu_to_le32(~(1 << TD_TOKEN_TOGGLE));
				desc->hw.td->info &= cpu_to_le32(~(1 << TD_TOKEN_TOGGLE));				
				if (status==0) {
					((urb_priv_t*)urb->hcpriv)->started=jiffies;
//pci					desc->hw.td.info |= cpu_to_le32((usb_gettoggle (urb->dev, usb_pipeendpoint (urb->pipe),
//pci									    usb_pipeout (urb->pipe)) << TD_TOKEN_TOGGLE));
...
//pci				desc->hw.td.status= cpu_to_le32((urb->pipe & TD_CTRL_LS) | TD_CTRL_ACTIVE | TD_CTRL_IOC |
//pci					(urb->transfer_flags & USB_DISABLE_SPD ? 0 : TD_CTRL_SPD) | (3 << 27));
...
//pci			desc->hw.td.status &= cpu_to_le32(~TD_CTRL_ACTIVE);
...
//pci		urb->iso_frame_desc[i].actual_length = uhci_actual_length(le32_to_cpu(desc->hw.td.status));
//pci		urb->iso_frame_desc[i].status = uhci_map_status (uhci_status_bits (le32_to_cpu(desc->hw.td.status)), usb_pipeout (urb->pipe));
...
//os	_static void uhci_interrupt (int irq, void *__uhci, struct pt_regs *regs)
void uhci_interrupt(ApEvtMsg_t *msg)
{
//os	uhci_t *s = __uhci;
	uhci_t *s = GLOBAL_UHCI;
	 ...
	 * interrupt cause
	 */
//	armond_printf("UHCI Interrupt\n");
	
	status = inw (io_addr + USBSTS);

//	armond_printf("UHCI interrupt status:%x\n", status);
...
//		uhci_check_timeouts(s);
	...
	if ((jiffies - s->timeout_check) > (HZ/10)) 
		uhci_check_timeouts(s);
...
//			err("USBCMD_HCRESET timed out!");
	...
	//________________###test###_____________________//
	//Read interrutp enable
	val = inw (io_addr + USBINTR);
	//_______________________________________________//
	/* Start at frame 0 */
	...
	//________________###test###_____________________//
	//Read Frame List address
	temp32 = inl (io_addr + USBFLBASEADD);
	
	//Read PCI USB0 status and command
	temp32 = pciHost_ConfigRead(0x04, 0);
	//_______________________________________________//
	...
	//________________###test###_____________________//
	//Read command
	val = inw (io_addr + USBCMD);
	//_______________________________________________//
	
	//________________###test###_____________________//
	//Read status
	val = inw (io_addr + USBSTS);
...
//	armond_printf("UHCI - 0 Root Hub New start\n");

	if (usb_new_device (usb_dev) != 0) {
		usb_free_dev (usb_dev);
		return -1;
	}
//	armond_printf("UHCI - 0 Root Hub New OK end\n");
	return 0;
}
...
_static int __devinit alloc_uhci ( int irq, unsigned int io_addr, unsigned int io_size)
{
	uhci_t *s;
	struct usb_bus *bus;
	char buf[8], *bufp = buf;
...
//os	request_region (s->io_addr, io_size, MODNAME);
	reset_hc (s);
...
//	pci_write_config_word (dev, USBLEGSUP, USBLEGSUP_DEFAULT);
	...
	val = pciHost_ConfigRead(0xc0,0);	//USBLEGSUP
	val=val&0xffff0000;
	pciHost_ConfigWrite(0xc0, val|USBLEGSUP_DEFAULT,0);
	//________________###test###_____________________//
	//Read USBLEGSUP
	val = pciHost_ConfigRead(0xc0,0);
	//_______________________________________________//

//	s->irq = irq;

	if(uhci_start_usb (s) < 0) {
//os		uhci_pci_remove(dev);
		return -1;
	}

	//chain new uhci device into global list
//os	pci_set_drvdata(dev, s);
	GLOBAL_UHCI=s;
...
uhci_pci_probe (unsigned int sz, unsigned int addr)
{
	int i;
#if 0
...
#endif
		unsigned int io_addr = addr;
		unsigned int io_size = sz;
		unsigned int irq = 0;	//GPIO do
		uint32 val;
				
		UHCI_IO_BASE_ADDR = addr;	//615wu for read/write	
		
//		armond_printf("UHCI - 0 Probe\n");
		
		//set master and IO space to enable UHCI
   		val = pciHost_ConfigRead(0x4,0);
		pciHost_ConfigWrite(0x04, val|0x15,0);
	
		/* disable legacy emulation */
		val = pciHost_ConfigRead(0xc0,0);	//USBLEGSUP
		val=val&0xffff0000;
		pciHost_ConfigWrite(0xc0, val|0x0,0);
		//________________###test###_____________________//
		//Read USBLEGSUP
		val = pciHost_ConfigRead(0xc0,0);
		//_______________________________________________//
		
		return alloc_uhci(irq, io_addr, io_size);
		
//		armond_printf("UHCI - 0 Probe end\n");
//	return -ENODEV;
}
...
int __init uhci_hcd_init (unsigned int sz, unsigned int io_addr) 
{
	int retval;
//os	retval = pci_module_init (&uhci_pci_driver);	
	uhci_pci_probe(sz, io_addr);
