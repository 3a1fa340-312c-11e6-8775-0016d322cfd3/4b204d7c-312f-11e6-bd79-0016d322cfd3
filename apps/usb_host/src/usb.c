...
//}}MARK_DEBUG
		...
		init_MUTEX(&new_driver->serialize);//ZOT==>	new_driver->serialize = 0;
...
//{{MARK_DEBUG
//	dbg("deregistering driver %s", driver->name);
//}}MARK_DEBUG
	if (driver->fops != NULL)
 ...
 */
static long usb_calc_bus_time (int speed, int input_dir, int isoc, int bytecount)
{
	unsigned long	tmp;

	switch (speed) 		/* no isoc. here */
	{
...
//{{MARK_DEBUG
//	dbg("new USB bus registered, assigned bus number %d", bus->busnum);
//}}MARK_DEBUG
}
...
{
//{{MARK_DEBUG
//	dbg("USB bus %d deregistered", bus->busnum);
//}}MARK_DEBUG
...
//	api_wrapper_data *awd = (api_wrapper_data *)urb->context;
	cyg_sem_t * sem = (cyg_sem_t *)urb->context;	//ZOT716u2

	cyg_semaphore_post(sem);	//ZOT716u2
...
//ZOT716u2	api_wrapper_data awd;
//ZOT716u2	uint32 start;
	int status;
//eason 20100813	cyg_sem_t * sem;	//ZOT716u2
	
//	awd.wakeup = &wqh;
//	init_waitqueue_head(&wqh); 	
//	current->state = TASK_INTERRUPTIBLE;
//	add_wait_queue(&wqh, &wait);
//ZOT716u2	urb->context = &awd;
//eason 20100813	sem = malloc (sizeof(cyg_sem_t));	//ZOT716u2
//eason 20100813	cyg_semaphore_init(sem, 0);		//ZOT716u2
//eason 20100813	urb->context = sem;	//ZOT716u2
	cyg_semaphore_init(&urb_sem, 0);		
	urb->context = &urb_sem;	
...
//		remove_wait_queue(&wqh, &wait);
		//eason 20100813	cyg_semaphore_destroy(sem);	//ZOT716u2
		//eason 20100813	free(sem);	//ZOT716u2
		cyg_semaphore_destroy(&urb_sem);
		return status;
	}
	...
	if (timeout && (urb->status == -EINPROGRESS))
	{
		//eason 20100813	timeout = cyg_semaphore_timed_wait(sem, cyg_current_time() + ((timeout / MSPTICK)?(timeout / MSPTICK):1));
		timeout = cyg_semaphore_timed_wait(&urb_sem, cyg_current_time() + ((timeout / MSPTICK)?(timeout / MSPTICK):1));	
	}
...
{
	//eason 20100407	devrequest *dr = kmalloc(sizeof(devrequest), GFP_KERNEL);
	devrequest *dr = kaligned_alloc(sizeof(devrequest), 4096);
	int ret;
...
void usb_destroy_configuration(struct usb_device *dev)
{
	...
	if (dev->rawdescriptors) {
		for (i = 0; i < dev->descriptor.bNumConfigurations; i++)
			kfree(dev->rawdescriptors[i], 0);

		kfree(dev->rawdescriptors, 0);
	}
			...
			for (j = 0; j < ifp->num_altsetting; j++) {
				...
				for(k = 0; k < as->bNumEndpoints; k++) {
					if(as->endpoint[k].extra) {
						kfree(as->endpoint[k].extra, 0);
					}
				}	
				kfree(as->endpoint, 0);
			}

			kfree(ifp->altsetting, 0);
		}
		kfree(cf->interface, 0);
	}
	kfree(dev->config, 0);
}
...
//{{MARK_DEBUG
//	dbg("USB disconnect on device %d", dev->devnum);
//}}MARK_DEBUG
...
#else 	//ZOT176u2
#define GET_TIMEOUT 5	//LINUX 2.4.22 : 5 seconds
#define SET_TIMEOUT 5	//LINUX 2.4.22 : 5 seconds
#endif	//ZOT716u2
...
//}}MARK_DEBUG
//				armond_printf("usb_get_DC_8_descriptor Error: %d\n",result);
...
//		armond_printf("usb_get_DC_8_descriptor Length: %d\n",length);
...
//}}MARK_DEBUG
//			armond_printf("usb_get_DC_all_descriptor Error: %d\n",result);
			kfree(bigbuffer, 0);
			goto err;
		}	
//		armond_printf("usb_get_DC_all_descriptor OK \n");
...
int usb_string(struct usb_device *dev, int index, char *buf, size_t size)
{
	...
	if (!dev->have_langid) {
		errno = usb_get_string(dev, 0, 0, tbuf, 4);
		if (errno < 0) {
//{{MARK_DEBUG
//			err("error getting string descriptor 0 (error=%d)", errno);
//}}MARK_DEBUG
			goto errout;
		} else if (tbuf[0] < 4) {
//{{MARK_DEBUG
...
//}}MARK_DEBUG
	 ...
	 * that was returned.
	 */
	errno = usb_get_string(dev, dev->string_langid, index, tbuf, 255);
	if (errno < 0)
		goto errout;

	size--;		/* leave room for trailing NULL char in output buffer */
	for (idx = 0, u = 2; u < errno; u += 2) {
		if (idx >= size)
			break;
...
*/
	...
	errno = usb_set_address(dev);
	if (errno < 0) {
//{{MARK_DEBUG
//		err("USB device not accepting new address=%d (error=%d)",
//			dev->devnum, errno);
//}}MARK_DEBUG
//		armond_printf("usb_set_address Error : %d\n",errno);
		clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
		dev->devnum = -1;
		wait_ms(100);
		return 1;
...
//	armond_printf("New Device Number is:%d\n",dev->devnum);
	
	wait_ms(10);	/* Let the SET_ADDRESS settle */

	errno = usb_get_descriptor(dev, USB_DT_DEVICE, 0, &dev->descriptor, 8);

	if (errno < 8) {
//{{MARK_DEBUG
//		if (errno < 0)
//			err("USB device not responding, giving up (error=%d)", errno);
//		else
//			err("USB device descriptor short read (expected %i, got %i)", 8, errno);
//}}MARK_DEBUG
//		armond_printf("usb_get_DT_8_descriptor Error : %d\n",errno);
		clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
		dev->devnum = -1;
		wait_ms(100);
		return 1;
...
//	armond_printf("usb_get_DT_8_descriptor OK MPSize: %d\n",dev->descriptor.bMaxPacketSize0);
	dev->epmaxpacketin [0] = dev->descriptor.bMaxPacketSize0;
	dev->epmaxpacketout[0] = dev->descriptor.bMaxPacketSize0;

	errno = usb_get_device_descriptor(dev);
	if (errno < (signed)sizeof(dev->descriptor)) {
//{{MARK_DEBUG
//		if (errno < 0)
//			err("unable to get device descriptor (error=%d)", errno);
//		else
//			err("USB device descriptor short read (expected %Zi, got %i)",
//				sizeof(dev->descriptor), errno);
//}}MARK_DEBUG
//		armond_printf("usb_get_device_all_descriptor Error : %d\n",errno);
		clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
		dev->devnum = -1;
		wait_ms(100);
		return 1;
	}
//	armond_printf("usb_get_device_all_descriptor OK\n");
	
	errno = usb_get_configuration(dev);
	if (errno < 0) {
//{{MARK_DEBUG
//		err("unable to get device %d configuration (error=%d)",
//			dev->devnum, errno);
//}}MARK_DEBUG
//		armond_printf("usb_get_configuration Error : %d\n",errno);
		clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
		dev->devnum = -1;
		wait_ms(100);
		return 1;
	}
//	armond_printf("usb_get_configuration OK \n");
	
	/* we set the default configuration here */
	errno = usb_set_configuration(dev, dev->config[0].bConfigurationValue);
	if (errno) {
//{{MARK_DEBUG
//		err("failed to set device %d default configuration (error=%d)",
//			dev->devnum, errno);
//}}MARK_DEBUG
//		armond_printf("usb_set_configuration Error : %d\n",errno);
		clear_bit(dev->devnum, &dev->bus->devmap.devicemap);
		dev->devnum = -1;
		wait_ms(100);
		return 1;
...
//	armond_printf("usb_set_configuration OK \n");
