...
#include "eap.h"
#include "config.h"
#include "eap_wsc_common.h"		//eason 20100608
...
static char * wpa_config_parse_string(const char *value, size_t *len)
{
	if (*value == '"') {
		...
		*len = pos - value;
		...
		if (str == NULL)
			return NULL;
		os_memset(str, 0, *len + 1); 	//eason 20100407	
		...
		if (str == NULL)
			return NULL;
		os_memset(str, 0, tlen + 1); 	//eason 20100407	
				...
				const char *value)
{
	if (*value == '"') {
		...
		if (ssid->passphrase == NULL)
			return -1;
		os_memset(ssid->passphrase, 0, len + 1); 	//eason 20100407		
...
//set eap
int wpa_config_set_eap(struct wpa_ssid *ssid)
{
	int errors = 0;
	...
	size_t num_methods = 0;	
		...
		methods[num_methods].method = EAP_VENDOR_TYPE_WSC;
		methods[num_methods].vendor	= EAP_VENDOR_WFA;
		if (methods[num_methods].vendor == EAP_VENDOR_IETF &&
		    methods[num_methods].method == EAP_TYPE_NONE) {		
			errors++;
		} else if (methods[num_methods].vendor == EAP_VENDOR_IETF &&
				     ...
				     const char *value)
{
	...
	os_memset(hash, 0, 16); 	//eason 20100407	
...
struct wpa_ssid * wpa_config_add_network(struct wpa_config *config)
{
	...
	memset(ssid, 0, sizeof(*ssid));	//eason 20100407	
	ssid->id = id;
	if (last)
		last->next = ssid;
	else
...
void wpa_config_update_psk(struct wpa_ssid *ssid)
{
	...
	pbkdf2_sha1(ssid->passphrase,
		    ...
		    ssid->psk, PMK_LEN);
/*eason 20100407	wpa_hexdump_key(MSG_MSGDUMP, "PSK (from passphrase)",
			ssid->psk, PMK_LEN);*/
	ssid->psk_set = 1;
#endif /* CONFIG_NO_PBKDF2 */
}
					   ...
					   const char *driver_param)
{
	...
	if (config == NULL)
		return NULL;
	memset(config, 0, sizeof(*config));		//eason 20100407
