...
#include "includes.h"
/*eason 20100407	#include <sys/un.h>
#include <sys/stat.h>
#include <grp.h>
#include <stddef.h>*/
					    ...
					    socklen_t fromlen)
{
	...
	os_memset(dst, 0, sizeof(*dst));	//eason 20100407	
...
//eason 20100608	struct ctrl_iface_priv *priv = sock_ctx;
	char buf[256];
	int res;
/*eason 20100608	struct sockaddr_un from;
	socklen_t fromlen = sizeof(from);*/
	char *reply = NULL;
	size_t reply_len = 0;
	int new_attached = 0;
	int cmd = 0;	//eason 20100608
	...
	if (reply) {
		sendto(sock, reply, reply_len, 0, (struct sockaddr1 *) &from,
		       fromlen);
		os_free(reply);
	} else if (reply_len == 1) {
		sendto(sock, "FAIL\n", 5, 0, (struct sockaddr1 *) &from,
		       fromlen);
	} else if (reply_len == 2) {
		sendto(sock, "OK\n", 3, 0, (struct sockaddr1 *) &from,
		       fromlen);
	}
#else
		cmd = get_realtek_wps();
...
static char * wpa_supplicant_ctrl_iface_path(struct wpa_supplicant *wpa_s)
{
	...
	os_memset(buf, 0, len); 	//eason 20100407
					     ...
					     const char *txt, size_t len)
{
	...
	if (wpa_s == NULL || wpa_s->ctrl_iface == NULL)
		return;
//eason 20100407	wpa_supplicant_ctrl_iface_send(wpa_s->ctrl_iface, level, txt, len);
}
...
wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s)
{
	...
	if (bind(priv->sock, (struct sockaddr1 *) &addr, sizeof(addr)) < 0) {
		wpa_printf(MSG_DEBUG, "ctrl_iface bind(PF_UNIX) failed: %s",
			   strerror(errno));
		if (connect(priv->sock, (struct sockaddr1 *) &addr,
			    sizeof(addr)) < 0) {
			...
			if (bind(priv->sock, (struct sockaddr1 *) &addr,
				 sizeof(addr)) < 0) {
				perror("bind(PF_UNIX)");
				goto fail;
			}
