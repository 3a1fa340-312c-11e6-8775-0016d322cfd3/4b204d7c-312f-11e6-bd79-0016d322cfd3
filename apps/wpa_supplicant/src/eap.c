				...
				struct eap_peer_config *conf)
{
	...
	os_memset(conf->identity, 0, 1 + imsi_len); 	//eason 20100407
...
static void eap_sm_processNotify(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	if (msg == NULL)
		return;
	os_memset(msg, 0, msg_len + 1); 	//eason 20100407	
	for (i = 0; i < msg_len; i++)
				 ...
				 void *msg_ctx, struct eap_config *conf)
{
	...
	if (sm == NULL)
		return NULL;
	os_memset(sm, 0, sizeof(*sm));	//eason 20100407	
			   ...
			   const char *msg, size_t msglen)
{
	...
	case TYPE_OTP:
		...
		if (msg) {
			...
			os_memset(tmp, 0, msglen + 3); 	//eason 20100407	
			tmp[0] = '[';
					      ...
					      size_t *count)
{
	...
	*count = 0;
	...
	os_memset(buf, 0, mcount * sizeof(struct eap_method_type)); 	//eason 20100407
	for (m = methods; m; m = m->next) {
		vendor = m->vendor;
		method = m->method;
