					   ...
					   struct wpa_eapol_key *key, u16 ver)
{
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4) {
		...
		os_memset(buf, 0, keydatalen); 	//eason 20100407
		    ...
		    const u8 *buf, size_t len)
{
	...
	os_memset(tmp, 0, len); 	//eason 20100407	
...
struct wpa_sm * wpa_sm_init(struct wpa_sm_ctx *ctx)
{
	...
	memset(sm, 0,sizeof(*sm));	//eason 20100407		
	sm->renew_snonce = 1;
	sm->ctx = ctx;
				    ...
				    size_t *wpa_ie_len)
{
	...
	*wpa_ie_len = res;
	...
	if (sm->assoc_wpa_ie == NULL) {
		...
		os_memset(sm->assoc_wpa_ie, 0, *wpa_ie_len); 	//eason 20100407
...
int wpa_sm_set_assoc_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	if (ie == NULL || len == 0) {
		...
		os_memset(sm->assoc_wpa_ie, 0, len); 	//eason 20100407
...
int wpa_sm_set_ap_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	if (ie == NULL || len == 0) {
		...
		os_memset(sm->ap_wpa_ie, 0, len); 	//eason 20100407
...
int wpa_sm_set_ap_rsn_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	if (ie == NULL || len == 0) {
		...
		os_memset(sm->ap_rsn_ie, 0, len); 	//eason 20100407
