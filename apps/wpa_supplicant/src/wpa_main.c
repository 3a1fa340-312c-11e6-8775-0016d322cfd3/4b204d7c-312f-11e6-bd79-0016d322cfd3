...
void wpa_task(cyg_addrword_t data)
{
	...
	struct wpa_params params;
	struct wpa_global *global;
	unsigned char driver[] = "wext";
	unsigned char ifname[] = "eth1"; 
	unsigned char confname[] = "conf";
	
/*eason no yet	while(1)
	{*/
	os_memset(&params, 0, sizeof(params));
	params.wpa_debug_level = MSG_INFO;
	...
	WPA_START = 0;	
//eason no yet	kwait( (char *)wpa_task );
	WPA_START = 1;
