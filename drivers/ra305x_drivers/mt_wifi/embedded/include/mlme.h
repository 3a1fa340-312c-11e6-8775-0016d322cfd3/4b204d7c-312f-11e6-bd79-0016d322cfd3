...
typedef struct {
	QBSS_STA_INFO_PARM	QosInfo GNU_ALIGNED(4);
	UCHAR	Rsv;
	UCHAR	Q_AC_BE[4];
	UCHAR	Q_AC_BK[4];
	UCHAR	Q_AC_VI[4];
...
typedef struct _BSS_ENTRY{
	...
	/* New for WPA2 */
	CIPHER_SUITE WPA GNU_ALIGNED(4);			/* AP announced WPA cipher suite */
	CIPHER_SUITE WPA2 GNU_ALIGNED(4);			/* AP announced WPA2 cipher suite */
#ifdef WAPI_SUPPORT
	CIPHER_SUITE WAPI GNU_ALIGNED(4);			/* AP announced WAPI cipher suite */
#endif /* WAPI_SUPPORT */
...
typedef struct _IE_lists {
	...
	ULONG RalinkIe;
	EXT_CAP_INFO_ELEMENT ExtCapInfo GNU_ALIGNED(4);
	UCHAR ht_cap_len;
	HT_CAPABILITY_IE HTCapability;
}IE_LISTS;
...
typedef struct _bcn_ie_list {
	...
	HT_CAPABILITY_IE HtCapability;
	UCHAR AddHtInfoLen;
	ADD_HT_INFO_IE AddHtInfo GNU_ALIGNED(4);
	UCHAR NewExtChannelOffset;
#ifdef CONFIG_STA_SUPPORT
