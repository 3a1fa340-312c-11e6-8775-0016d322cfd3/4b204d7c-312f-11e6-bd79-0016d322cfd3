...
//PortInfo:
	...
	u8    ctr; 	  // Device Control Register
//	UINT   ieee1284_mode;
//	enum   ieee1284_phase ieee1284_phase;
	u8   PrnMode; 		//Current printing mode :(NO, SPP printing, ECP printing)
	u8   PrnReadBackMode; //Current printing mode :(NO, SPP printing, ECP printing)
//DeviceNofo:
	u8   HasGetDeviceID; //0: not yet , 1: at least get once
	u8   SupportLang;    //Printer support language (PJL, PCL, POSTSCRIPT)
	u8  *Manufacture;    //MANUFACTURER
	u8  *Model;		   //MODEL
	u8  *CommandSet;	   //COMMAND SET
...
#define NUM_OF_PRN_PORT 1
#define USBLP_MINORS    6
...
static ssize_t usblp_read(struct usblp *usblp/*struct file *file*/, char __user *buffer, size_t len, loff_t *ppos);
static int usblp_open(struct usblp *usblp/*struct inode *inode, struct file *file*/)
{
	// int minor = iminor(inode);
	// struct usblp *usblp;
    int minor = USBLP_MINOR_BASE;
...
static int usblp_release(struct usblp *usblp, int nPort/*struct inode *inode, struct file *file*/)
{
...
ssize_t usblp_write(struct usblp *usblp/*struct file *file*/, const char __user *buffer, size_t count, int *ppos)
{
	// struct usblp *usblp = file->private_data;
	struct urb *writeurb;
	int rv;
...
static ssize_t usblp_read(struct usblp *usblp/*struct file *file*/, char __user *buffer, size_t len, loff_t *ppos)
{
    ...
    rv = usblp_rwait_and_lock(usblp, 0);
	if (rv < 0)
		return rv;
	...
	if (count != 0 &&
	    /*copy_to_user*/memcpy(buffer, usblp->readbuf + usblp->readcount, count)) {
		count = -EFAULT;
		goto done;
	}
		       ...
		       const struct usb_device_id *id)
{
    ...
    usblp = kmalloc(sizeof(struct usblp), GFP_KERNEL);
	if (!usblp) {
		retval = -ENOMEM;
		goto abort_ret;
	}
	usblp->dev = dev;
    mutex_init(&usblp_mutex);
	...
	if(IS_HP_GDI_PRINTER_HP_1020 == 1)
	{
		...
		usblp_release( usblp, usblp->minor );
	}
...
#ifdef USE_PS_LIBS
	if( PortIO[nPort].base && usblp_table[PortIO[nPort].base-1] )
		usblp_release( usblp_table[PortIO[nPort].base-1], usblp_table[PortIO[nPort].base-1]->minor );
#endif /* USE_PS_LIBS */
	return 0;
}
