...
/*
	mark[6]; 	//"ZOTIPS"
	bintype; 	//0xA0:ALL 0xA1: Code2 0xA2: WEB 0xA3: EEPROM 
	reserved[1];
	date
	time
	Product
...
void main(int nArgc, char *szArgv[])
{
	...
	FILE			*VERFile = NULL;
	char			*IMGMem = NULL;
	char			*IMGHeaderPtr = NULL;
	char			*IMGSrcPtr = NULL;
	char   			 *IMGHeaderDatePtr = NULL;
	char			*IMGHeaderVersionPtr = NULL;
	DWORD			*IMGHeaderGZLenPtr = NULL;
	DWORD			*IMGHeaderCRCPtr = NULL;
	...
	char			*pszValue = NULL;
	char 			*tempVersionPtr = NULL;
	

	if( nArgc >= 3 )
	{
	//open Src file
		SrcFile = fopen( szArgv[1], "rb" );
		if( SrcFile == NULL )
		{
	...
	//alloc temp memory, the first 256 bytes for Image Header 
		IMGMem = malloc( MEM_SIZE );
		memset(IMGMem , 0, MEM_SIZE);
		if( IMGMem == NULL  )
		{
	...
	//point to temp memory
		IMGHeaderPtr = IMGMem;
		TotalSize = SrcSize + HEADER_SIZE;
	...
	//reserved two bytes	
		IMGHeaderDatePtr = &IMGHeaderPtr[8];
		IMGHeaderVersionPtr = &IMGHeaderPtr[30];	
		
		IMGHeaderGZLenPtr = &IMGHeaderPtr[252];
		IMGHeaderCRCPtr = &IMGHeaderPtr[248];
	...
	//write date and time
		sprintf( IMGHeaderDatePtr, "%s  %s  ",szTemp1, szTemp2 );
		
	//open web_info.def		
		VERFile = fopen( szArgv[3], "rb" );
		if( VERFile == NULL )
		{
			printf( "failed to open %s !\n", szArgv[3] );
			goto Exit;
	...
	//write web_info	
		tempVersionPtr = IMGHeaderVersionPtr;
		while( fgets( szLine, MAX_CHARS_PER_LINE, VERFile ) )
		{
			if( strncmp( szLine, "Product", strlen("Product") ) == 0 )
			{
				pszValue = strstr( szLine, "=" );
				if( pszValue )
				{
					pszValue += 2;
					sscanf( pszValue, "%s  ", szTemp3 );
					sprintf( tempVersionPtr, "%s  ", szTemp3);
					tempVersionPtr += strlen(tempVersionPtr);
				}
			}
			if( strncmp( szLine, "WEBVersion", strlen("WEBVersion") ) == 0 )
			{
				pszValue = strstr( szLine, "=" );
				if( pszValue )
				{
			...
			if( strncmp( szLine, "WEBLanguage", strlen("WEBLanguage") ) == 0 )
			{
				pszValue = strstr( szLine, "=" );
				if( pszValue )
				{
	...
	//do checksum
		*IMGHeaderCRCPtr = crc32( IMGSrcPtr, SrcSize, 0xFFFFFFFFL );
	
	//write length	
		*IMGHeaderGZLenPtr = SrcSize;
	
	//write img.bin
		TotalSize = ( TotalSize + BLOCK_SIZE - 1 ) / BLOCK_SIZE * BLOCK_SIZE;
		fwrite( IMGMem, 1, TotalSize, DestFile );
	}
